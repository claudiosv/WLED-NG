...
class AnalogClockUsermod : public Usermod {
 private:
  ...
  struct Segment {
    ...
    void validateAndUpdate() {
      if (firstLed < 0 || firstLed >= strip.getLengthTotal() || lastLed < firstLed ||
          lastLed >= strip.getLengthTotal()) {
        *this = {};
        return;
      }
  ...
  int16_t adjustToSegment(double progress, Segment const& segment) {
    ...
    return led > segment.lastLed ? segment.firstLed + led - segment.lastLed - 1 : led;
  }
  ...
  static inline int16_t dec(int16_t n, int16_t i, Segment const& seg) {
    return n - seg.firstLed >= i ? n - i : seg.lastLed - seg.firstLed - i + n + 1;
  }
 ...
 public:
  ...
  bool readFromConfig(JsonObject& root) override {
    ...
    configComplete &= getJsonValue(top["Hour Mark Color (RRGGBB)"], color, "161616") &&
                      hexStringToColor(color, hourMarkColor, 0x161616);
    configComplete &=
        getJsonValue(top["Hour Color (RRGGBB)"], color, "0000FF") && hexStringToColor(color, hourColor, 0x0000FF);
    configComplete &=
        getJsonValue(top["Minute Color (RRGGBB)"], color, "00FF00") && hexStringToColor(color, minuteColor, 0x00FF00);
    ...
    configComplete &=
        getJsonValue(top["Second Color (RRGGBB)"], color, "FF0000") && hexStringToColor(color, secondColor, 0xFF0000);
