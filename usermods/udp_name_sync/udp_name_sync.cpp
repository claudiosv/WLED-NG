...
class UdpNameSync : public Usermod {
 ...
 public:
  ...
  void loop() override {
    ...
    if (segmentName[0] == '\0' && !mainseg.name) {
      return;  // name was never set, do nothing
    }
  ...
  bool onUdpPacket(uint8_t* payload, size_t len) override {
    ...
    if (len < 2) {
      return false;  // need type + at least 1 byte for name (can be 0)
    }
    if (payload[0] != kPacketType) {
      return false;
    }
    Segment& mainseg                   = strip.getMainSegment();
    char     tmp[WLED_MAX_SEGNAME_LEN] = {0};
    size_t   copyLen                   = len - 1;
    if (copyLen > sizeof(tmp) - 1) {
