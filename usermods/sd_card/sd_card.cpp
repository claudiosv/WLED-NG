...
class UsermodSdCard : public Usermod {
 ...
 public:
  ...
  bool readFromConfig(JsonObject &root) {
    ...
    if (configSdEnabled && (oldPinSourceSelect != configPinSourceSelect || oldPinSourceClock != configPinSourceClock ||
                            oldPinPoci != configPinPoci || oldPinPico != configPinPico)) {
      DEBUG_PRINTF("[%s] Init SD card based of config\n", _name);
      DEBUG_PRINTF("[%s] Config changes \n - SS: %d -> %d\n - MI: %d -> %d\n - MO: %d -> %d\n - En: %d -> %d\n", _name,
                   oldPinSourceSelect, configPinSourceSelect, oldPinSourceClock, configPinSourceClock, oldPinPoci,
                   configPinPoci, oldPinPico, configPinPico);
      reinit_SD_SPI();
    }
#endif
...
const char UsermodSdCard::_name[]         = "SD Card";
...
#ifdef WLED_USE_SD_SPI
  if (!UsermodSdCard::configSdEnabled) {
    return false;
  }
#endif
