...
class RotaryEncoderBrightnessColor : public Usermod {
 ...
 public:
  ...
  void loop() {
    ...
    if (currentTime >= (loopTime + 2))  // 2ms since last check of encoder = 500Hz
    {
      ...
      if ((!Enc_A) && (Enc_A_prev)) {  // A has gone from high to low
        ...
        // call for notifier -> 0: init 1: direct change 2: button 3: notification 4: nightlight 5: other (No
        // notification)
   ...
   * - The getJsonValue() function copies the value to the variable only if the key requested is present, returning
   * false with no copy if the value isn't present
   ...
   *
   * This ensures that missing values are added to the config, with their default values, in the rare but plausible
   * cases of:
   ...
   *
   * If configComplete is false, the default values are already set, and by returning false, WLED now knows it needs to
   * save the defaults by calling addToConfig()
   */
  bool readFromConfig(JsonObject& root) {
    // set defaults here, they will be set before setup() is called, and if any values parsed from ArduinoJson below are
    // missing, the default will be used instead
    fadeAmount = 5;
    pins[0]    = -1;
    pins[1]    = -1;
    pins[2]    = -1;
