...
static const char _data_FX_MODE_DIFFUSIONFIRE[] =
    "Diffusion Fire@!,Spark rate,Diffusion Speed,Turbulence,,Use palette;;Color;;2;pal=35";

/*
 * Spinning Wheel effect - LED animates around 1D strip (or each column in a 2D matrix), slows down and stops at random
 * position Created by Bob Loeffler and claude.ai First slider (Spin speed) is for the speed of the moving/spinning LED
 * (random number within a narrow speed range). If value is 0, a random speed will be selected from the full range of
 * values. Second slider (Spin slowdown start time) is for how long before the slowdown phase starts (random number
 * within a narrow time range). If value is 0, a random time will be selected from the full range of values. Third
 * slider (Spinner size) is for the number of pixels that make up the spinner. Fourth slider (Spin delay) is for how
 * long it takes for the LED to start spinning again after the previous spin. The first checkbox allows the spinner to
 * spin. If it's enabled, the spinner will do its thing. If it's not enabled, it will wait for the user to enable it
 * either by clicking the checkbox or by pressing a physical button (e.g. using a playlist to run a couple presets that
 * have JSON API codes). The second checkbox sets "color per block" mode. Enabled means that each spinner block will be
 * the same color no matter what its LED position is. The third checkbox enables synchronized restart (all spinners
 * restart together instead of individually). aux0 stores the settings checksum to detect changes aux1 stores the color
...
static void mode_spinning_wheel(void) {
  ...
  uint32_t settingssum =
      SEGMENT.speed + SEGMENT.intensity + SEGMENT.custom1 + SEGMENT.custom3 + SEGMENT.check1 + SEGMENT.check3;
...
static const char _data_FX_MODE_SPINNINGWHEEL[] =
    "Spinning Wheel@Speed (0=random),Slowdown (0=random),Spinner size,,Spin delay,Spin me!,Color per block,Sync "
    "restart;!,!;!;;m12=1,c1=1,c3=8,o1=1,o3=1";
...
*   The Attract checkbox sets the attraction of blobs (checked will make the blobs attract other close blobs
horizontally)
*   The Keep Color Ratio checkbox sets whether we preserve the color ratio when displaying pixels that are in 2 or more
overlapping blobs
...
static const char _data_FX_MODE_2D_LAVALAMP[] =
    "Lava Lamp@,# of blobs,Blob size,H. Damping,,,Attract,Keep Color Ratio;;!;2;ix=64,c2=192,o2=1,o3=1,pal=47";
...
*   Third slider (lava bombs) is for the number of lava bombs (particles).  The max # is 1/2 the number of columns on
the 2D matrix.
...
static void drawLavaBombs(const uint16_t width, const uint16_t height, float* particleData, float gravity,
...
static void mode_2D_magma(void) {
  ...
  const uint16_t dataSize =
      (MAGMA_MAX_PARTICLES * 4 + 2) * sizeof(float) + height * sizeof(uint8_t) + SETTINGS_SUM_BYTES;
  if (!SEGENV.allocateData(dataSize)) {
    FX_FALLBACK_STATIC;  // allocation failed
  }
...
static const char _data_FX_MODE_2D_MAGMA[] =
    "Magma@Flow rate,Magma height,Lava bombs,Gravity,,,Bombs in front;;!;2;ix=192,c2=32,o2=1,pal=35";
...
*     When gathering food, the Pass By option will automatically be enabled so they can drop off their food easier (and
look for more food).
...
static void renderAntPixel(int pixelIndex, int pixelOffset, int antSize, const Ant& ant, uint32_t antColor,
                           uint32_t backgroundColor, bool gatherFood) {
  bool isMovingBackward = (ant.velocity < 0);
  bool isFoodPixel      = gatherFood && ant.hasFood &&
  ...
  for (int i = 0; i < numAnts; i++) {
    ...
    if (!passBy) {
      for (int j = i + 1; j < numAnts; j++) {
        ...
        float collisionTime =
            (timeConversionFactor * (ants[i].position - ants[j].position) + ants[i].velocity * timeOffset) /
        ...
        if (collisionTime > MIN_COLLISION_TIME_MS && collisionTime < timeSinceJ) {
          ...
          float adjustedTime =
              (collisionTime + float(int(ants[j].lastBumpUpdate - ants[i].lastBumpUpdate))) / timeConversionFactor;
          ...
          newPosition = ants[i].position +
                        ants[i].velocity * float(int(strip.now - ants[i].lastBumpUpdate)) / timeConversionFactor;
        }
      }
    }
...
static const char _data_FX_MODE_ANTS[] =
    "Ants@Ant speed,# of ants,Ant size,Blur,,Gathering food,Smear,Pass by;!,!,!;!;1;sx=192,ix=255,c1=32,c2=0,o1=1,o3=1";
...
*   This effect uses a bit array, instead of bool array, for efficient storage - 8x memory reduction (128 bytes vs 1024
bytes)
*
*   Morse Code rules:
...
static void build_morsecode_pattern(const char* morse_code, uint8_t* pattern, uint8_t* wordIndex, uint16_t& index,
                                    uint8_t currentWord, int maxSize) {
...
static void mode_morsecode(void) {
  ...
  // Allocate per-segment storage for pattern (1023 bits = 127 bytes) + word index array (1024 bytes) + word count (1
  // byte)
  ...
  uint8_t* wordCountPtr =
      reinterpret_cast<uint8_t*>(SEGENV.data + MORSECODE_PATTERN_BYTES + MORSECODE_WORD_INDEX_BYTES);
  ...
  bool needsRebuild =
      (SEGENV.call == 0) || textChanged || (SEGMENT.check2 != lastCheck2) || (SEGMENT.check3 != lastCheck3);
  ...
  if (needsRebuild) {
    ...
    for (char* c = text; *c; c++) {
      ...
      if (*c >= 'A' && *c <= 'Z') {
        build_morsecode_pattern(letters[*c - 'A'], morsecodePattern, wordIndexArray, patternLength, currentWordIndex,
                                MORSECODE_MAX_PATTERN_SIZE);
      } else if (*c >= '0' && *c <= '9') {
        build_morsecode_pattern(numbers[*c - '0'], morsecodePattern, wordIndexArray, patternLength, currentWordIndex,
                                MORSECODE_MAX_PATTERN_SIZE);
      } else if (*c == ' ') {
        ...
        if (punctuationCode) {
          build_morsecode_pattern(punctuationCode, morsecodePattern, wordIndexArray, patternLength, currentWordIndex,
                                  MORSECODE_MAX_PATTERN_SIZE);
        }
      }
    }
    ...
    if (SEGMENT.check3) {
      build_morsecode_pattern(".-.-.", morsecodePattern, wordIndexArray, patternLength, currentWordIndex,
                              MORSECODE_MAX_PATTERN_SIZE);
    }
    ...
    *wordCountPtr = currentWordIndex + 1;
    ...
    SEGENV.aux1 =
        patternLength | (currentHashBits << 10) | (SEGMENT.check2 ? 0x8000 : 0) | (SEGMENT.check3 ? 0x4000 : 0);
...
static const char _data_FX_MODE_MORSECODE[] =
    "Morse Code@Speed,,,,Color mode,Color by Word,Punctuation,EndOfMessage;;!;1;sx=192,c3=8,o1=1,o2=1";
