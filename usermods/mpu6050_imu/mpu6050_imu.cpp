...
/* This driver reads quaternion data from the MPU6060 and adds it to the JSON
  ...
  4. You also need to change lib_compat_mode from strict to soft in platformio.ini (This ignores that I2Cdevlib-MPU6050
  doesn't list platform compatibility)
...
class MPU6050Driver : public Usermod {
 ...
 public:
  inline bool initDone() {
    return um_data.u_size != 0;
  };  // recycle this instead of storing an extra variable
  ...
  void setup() {
    ...
    if (i2c_scl < 0 || i2c_sda < 0) {
      DEBUG_PRINTLN("MPU6050: I2C is no good.");
      return;
    }
    ...
    if (config.interruptPin >= 0) {
      ...
      if (!irqBound) {
        DEBUG_PRINTLN("MPU6050: IRQ pin already in use.");
        return;
      }
  ...
  void loop() {
    ...
    if (!config.enabled || !dmpReady || strip.isUpdating()) {
      return;
    }
  ...
  bool getUMData(um_data_t** data) {
    if (!data || !config.enabled || !dmpReady) {
...
const char MPU6050Driver::_name[]          = "MPU6050_IMU";
const char MPU6050Driver::_enabled[]       = "enabled";
const char MPU6050Driver::_interrupt_pin[] = "interrupt_pin";
const char MPU6050Driver::_x_acc_bias[]    = "x_acc_bias";
const char MPU6050Driver::_y_acc_bias[]    = "y_acc_bias";
const char MPU6050Driver::_z_acc_bias[]    = "z_acc_bias";
const char MPU6050Driver::_x_gyro_bias[]   = "x_gyro_bias";
const char MPU6050Driver::_y_gyro_bias[]   = "y_gyro_bias";
const char MPU6050Driver::_z_gyro_bias[]   = "z_gyro_bias";
