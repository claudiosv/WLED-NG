...
class St7789DisplayUsermod : public Usermod {
 ...
 public:
  ...
  void setup() override {
    ...
    tft.init();
    tft.setRotation(0);  // Rotation here is set up for the text to be readable with the port on the left. Use 1 to
                         // flip.
  ...
  void loop() override {
    ...
    if ((((apActive) ? String(apSSID) : WiFi.SSID()) != knownSsid) ||
        (knownIp != (apActive ? IPAddress(4, 3, 2, 1) : Network.localIP())) || (knownBrightness != bri) ||
        (knownEffectSpeed != strip.getMainSegment().speed) ||
        (knownEffectIntensity != strip.getMainSegment().intensity) || (knownMode != strip.getMainSegment().mode) ||
  ...
  bool readFromConfig(JsonObject& root) override {
    ...
    // userVar0 = top["great"] | 42; //The value right of the pipe "|" is the default value in case your setting was not
    // present in cfg.json (e.g. first boot)
    return true;
  }
