...
class RgbRotaryEncoderUsermod : public Usermod {
 private:
  ...
  inline uint32_t colorFromRgb(byte *rgb) {
    return uint32_t((byte(rgb[0]) << 16) | (byte(rgb[1]) << 8) | (byte(rgb[2])));
  }
  ...
  void initLedBus() {
    ...
    busCfg.iType =
        BusManager::getI(busCfg.type, busCfg.pins, busCfg.driverType);  // assign internal bus type and output driver
  ...
  bool readFromConfig(JsonObject &root) {
    ...
    // Config has been changed, so adopt to changes
      ...
      if (!enabled) {
        ...
        if (eaIo != oldEaIo || ebIo != oldEbIo || stepsPerClick != oldStepsPerClick ||
            incrementPerClick != oldIncrementPerClick) {
