...
#endif
RTC_DATA_ATTR bool powerup =
    true;  // this is first boot after power cycle. note: variable in RTC data persists on a reboot
RTC_DATA_ATTR uint8_t wakeupPreset =
    0;  // preset to apply after deep sleep wakeup (0 = none), set to timer macro preset
...
class DeepSleepUsermod : public Usermod {
 private:
  ...
  int     delaycounter = 10;  // delay deep sleep at bootup until preset settings are applied, force wake up if offmode
                              // persists after bootup
...
#ifdef CONFIG_IDF_TARGET_ESP32  // ESP32: GPIOs 0,2,4, 12-15, 25-39 can be used for wake-up. note: input-only GPIOs
                                // 34-39 do not have internal pull resistors
    if (wakePin == 0 || wakePin == 2 || wakePin == 4 || (wakePin >= 12 && wakePin <= 15) ||
        (wakePin >= 25 && wakePin <= 27) || (wakePin >= 32 && wakePin <= 39)) {
      return true;
    }
#endif
#if defined(CONFIG_IDF_TARGET_ESP32S3) || \
    defined(CONFIG_IDF_TARGET_ESP32S2)  // ESP32 S3 & S3: GPIOs 0-21 can be used for wake-up
    if (wakePin <= 21) {
      return true;
    }
#endif
  ...
  int findNextTimerInterval() {
    ...
    for (size_t i = 0; i < timers.size(); i++) {
      ...
      if (!t.isEnabled() || !t.isRegular()) {
        continue;
      }
      if (!isTodayInDateRange(t.monthStart, t.dayStart, t.monthEnd, t.dayEnd)) {
        continue;
      }
      ...
      for (int dayOffset = 0; dayOffset < 7; dayOffset++) {
        ...
        if ((t.weekdays >> checkWeekday) & 0x01) {
          if (dayOffset == 0 &&
              (targetHour < currentHour || (targetHour == currentHour && targetMinute <= currentMinute))) {
            continue;  // skip if time has already passed today
          }

          int timeDifference =
              calculateTimeDifference(currentHour, currentMinute, targetHour + (dayOffset * 24), targetMinute);
...
// rtc_gpio_deinit(wakeupPin);
  ...
  void loop() {
    ...
    if (!offMode) {      // LEDs are on
      ...
      if (delaycounter) {
        ...
        applyPreset(
            wakeupPreset);  // apply preset if set, this ensures macro is applied even if we missed the wake-up time
      }
      return;
    }
    ...
    if (powerup == false &&
        delaycounter) {  // delay sleep in case a preset is being loaded and turnOnAtBoot is disabled (beginStrip() /
                         // handleIO() does enable offMode temporarily in this case)
      delaycounter--;
      if (delaycounter == 1 && offMode) {  // force turn on, no matter the settings (device is bricked if user set
                                           // sleepDelay=0, no bootup preset and turnOnAtBoot=false)
        if (briS == 0) {
          bri = 10;  // turn on and set low brightness to avoid automatic turn off
        } else {
          bri = briS;
        }
        strip.setBrightness(
            bri);  // needed to make handleIO() not turn off LEDs (really? does not help in bootup preset)
        offMode = false;
        applyPresetWithFallback(0, CALL_MODE_INIT, FX_MODE_STATIC, 0);  // try to apply preset 0, fallback to static
        if (rlyPin >= 0) {
          digitalWrite(
              rlyPin,
              (rlyMde ? HIGH : LOW));  // turn relay on TODO: this should be done by wled, what function to call?
        }
      }
      return;
    }
    ...
    // note: on C3 calling esp_deep_sleep_enable_gpio_wakeup() automatically enables pullup/pulldown unless we call
    // gpio_hold_en() which overrides that
    ...
    gpio_hold_en((gpio_num_t)wakeupPin);  // hold the configured GPIO state during deep sleep, overrides the automatic
                                          // pullup/pulldown, see note above
#else                                     // ESP32, S2, S3
    ...
    if (wakeWhenHigh) {
      halerror = esp_sleep_enable_ext1_wakeup(
          1ULL << wakeupPin, ESP_EXT1_WAKEUP_ANY_HIGH);  // use ext1 as ext0 does not work with touch wakeup
    } else {
  ...
  void appendConfigData() override {
    ...
    oappend(SET_F(
        "addInfo('DeepSleep:pull',1,'','-up/down disable: ');"));  // first string is suffix, second string is prefix
    ...
    oappend(
        SET_F("addInfo('DeepSleep:delaySleep',1,'seconds <i>(0 = sleep at powerup)<i>');"));  // first string is suffix,
                                                                                              // second string is prefix
  }
...
const char DeepSleepUsermod::_name[]    = "DeepSleep";
const char DeepSleepUsermod::_enabled[] = "enabled";
