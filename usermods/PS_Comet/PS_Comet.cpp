...
void mode_pscomet() {
  ...
  auto has_fallen_off_screen = [PartSys](uint32_t particleIndex) {
    return particleIndex < PartSys->numSources ? PartSys->sources[particleIndex].source.y < PartSys->maxY * -1 : true;
  };
  ...
  if (strip.now < nextCometCreationTime || !has_fallen_off_screen(chosenIndex - 1) ||
      !has_fallen_off_screen(chosenIndex) || !has_fallen_off_screen(chosenIndex + 1)) {
  ...
  uint8_t canLargeCometSpawn =
      ...
      SEGMENT.custom1 > hw_random8(254) && chosenIndex != 0 && chosenIndex != numComets - 1;
  ...
  for (i = 0; i < numComets; i++) {
    ...
    bool isChosenSideComet = canLargeCometSpawn && (i == chosenIndex - 1 || i == chosenIndex + 1);
...
static const char _data_FX_MODE_PSCOMET[] =
    "PS Comet@Falling Speed,Comet Frequency,Large Comet Probability,Comet "
    "Length;;!;2;pal=35,sx=128,ix=255,c1=32,c2=128";
