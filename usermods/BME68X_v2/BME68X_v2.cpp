...
class UsermodBME68X : public Usermod {
 public:
  ...
  void     addToConfig(
      JsonObject& root);  // Extends the settings/user module settings page to include the user module requirements. The
                          // settings are written from the wled core to the configuration file.
 ...
 private:
  ...
  void MQTT_PublishHASensor(const String& name, const String& deviceClass, const String& unitOfMeasurement,
                            const int8_t& digs, const uint8_t& option = 0);
  void MQTT_publish(const char* topic, const float& value, const int8_t& dig);
  void onMqttConnect(bool sessionPresent);
  void checkIaqSensorStatus();
  void InfoHelper(JsonObject& root, const char* name, const float& sensorvalue, const int8_t& decimals,
                  const char* unit);
  ...
  struct settings_t {
    ...
    bool publishAfterCalibration;  // The IAQ/CO2/VOC/GAS value are only valid after the sensor has been calibrated. If
                                   // this switch is active, the values are only sent after calibration
  ...
  struct values_t {
    ...
    uint8_t iaqAccuracy;    // IAQ accuracy -  IAQ Accuracy = 1 means value is inaccurate, IAQ Accuracy = 2 means sensor
                            // is being calibrated, IAQ Accuracy = 3 means sensor successfully calibrated.
    ...
    bool    stabStatus;  // Indicates if the sensor is undergoing initial stabilization during its first use after
                         // production
    bool runInStatus;    // Indicates when the sensor is ready after after switch-on
  } valuesA, valuesB, *ValuesPtr, *PrevValuesPtr,
      *swap;  // Data Scructur A, Data Structur B, Pointers to switch between data channel A & B
  ...
  bsec_virtual_sensor_t sensorList[13] = {
      BSEC_OUTPUT_IAQ,  // Index for Air Quality estimate [0-500] Index for Air Quality (IAQ) gives an indication of the
                        // relative change in ambient TVOCs detected by BME680.
      ...
      BSEC_OUTPUT_RAW_TEMPERATURE,  // Temperature sensor signal [degrees Celsius] Temperature directly measured by
                                    // BME680 in degree Celsius. This value is cross-influenced by the sensor heating
                                    // and device specific heating.
      BSEC_OUTPUT_RAW_PRESSURE,     // Pressure sensor signal [Pa] Pressure directly measured by the BME680 in Pa.
      BSEC_OUTPUT_RAW_HUMIDITY,     // Relative humidity sensor signal [%] Relative humidity directly measured by the
                                    // BME680 in %. This value is cross-influenced by the sensor heating and device
                                    // specific heating.
      BSEC_OUTPUT_RAW_GAS,          // Gas sensor signal [Ohm] Gas resistance measured directly by the BME680 in Ohm.The
                            // resistance value changes due to varying VOC concentrations (the higher the concentration
                            // of reducing VOCs, the lower the resistance and vice versa).
      BSEC_OUTPUT_STABILIZATION_STATUS,  // Gas sensor stabilization status [boolean] Indicates initial stabilization
                                         // status of the gas sensor element: stabilization is ongoing (0) or
                                         // stabilization is finished (1).
      BSEC_OUTPUT_RUN_IN_STATUS,  // Gas sensor run-in status [boolean] Indicates power-on stabilization status of the
                                  // gas sensor element: stabilization is ongoing (0) or stabilization is finished (1)
      BSEC_OUTPUT_SENSOR_HEAT_COMPENSATED_TEMPERATURE,  // Sensor heat compensated temperature [degrees Celsius]
                                                        // Temperature measured by BME680 which is compensated for the
                                                        // influence of sensor (heater) in degree Celsius. The self
                                                        // heating introduced by the heater is depending on the sensor
                                                        // operation mode and the sensor supply voltage.
      BSEC_OUTPUT_SENSOR_HEAT_COMPENSATED_HUMIDITY,  // Sensor heat compensated humidity [%] Relative measured by BME680
                                                     // which is compensated for the influence of sensor (heater) in %.
                                                     // It converts the ::BSEC_INPUT_HUMIDITY from temperature
                                                     // ::BSEC_INPUT_TEMPERATURE to temperature
...
const char UsermodBME68X::_enabled[]  = "Enabled";
const char UsermodBME68X::_hadtopic[] = "homeassistant/sensor/";

const char UsermodBME68X::_nameI2CAdr[]        = "i2C Address";
const char UsermodBME68X::_nameInterval[]      = "Interval";
const char UsermodBME68X::_nameMaxAge[]        = "Max Age";
const char UsermodBME68X::_namePublishChange[] = "Pub changes only";
const char UsermodBME68X::_namePubAc[]         = "Pub Accuracy";
const char UsermodBME68X::_namePubSenState[]   = "Pub Calib State";
const char UsermodBME68X::_namePubAfterCalib[] = "Pub After Calib";
const char UsermodBME68X::_nameTempScale[]     = "Temp Scale";
const char UsermodBME68X::_nameTempOffset[]    = "Temp Offset";
const char UsermodBME68X::_nameHADisc[]        = "HA Discovery";
const char UsermodBME68X::_nameDelCalib[]      = "Del Calibration Hist";
const char UsermodBME68X::_namePauseOnActWL[]  = "Pause while WLED active";

/* Private: Sensor names / Sensor short name */
const char UsermodBME68X::_nameTemp[]          = "Temperature";
const char UsermodBME68X::_nameHum[]           = "Humidity";
const char UsermodBME68X::_namePress[]         = "Pressure";
const char UsermodBME68X::_nameGasRes[]        = "Gas-Resistance";
const char UsermodBME68X::_nameAHum[]          = "Absolute-Humidity";
const char UsermodBME68X::_nameDrewP[]         = "Drew-Point";
const char UsermodBME68X::_nameIaq[]           = "IAQ";
const char UsermodBME68X::_nameIaqVerb[]       = "IAQ-Verbal";
const char UsermodBME68X::_nameStaticIaq[]     = "Static-IAQ";
const char UsermodBME68X::_nameStaticIaqVerb[] = "Static-IAQ-Verbal";
const char UsermodBME68X::_nameCo2[]           = "CO2";
const char UsermodBME68X::_nameVoc[]           = "VOC";
const char UsermodBME68X::_nameGasPer[]        = "Gas-Percentage";
const char UsermodBME68X::_nameIaqAc[]         = "IAQ-Accuracy";
const char UsermodBME68X::_nameStaticIaqAc[]   = "Static-IAQ-Accuracy";
const char UsermodBME68X::_nameCo2Ac[]         = "CO2-Accuracy";
const char UsermodBME68X::_nameVocAc[]         = "VOC-Accuracy";
const char UsermodBME68X::_nameGasPerAc[]      = "Gas-Percentage-Accuracy";
const char UsermodBME68X::_nameStabStatus[]    = "Stab-Status";
const char UsermodBME68X::_nameRunInStatus[]   = "Run-In-Status";

/* Private Units */
const char UsermodBME68X::_unitTemp[]      = " ";  // NOTE - Is set with the selectable temperature unit
const char UsermodBME68X::_unitHum[]       = "%";
const char UsermodBME68X::_unitPress[]     = "hPa";
const char UsermodBME68X::_unitGasres[]    = "kΩ";
const char UsermodBME68X::_unitAHum[]      = "g/m³";
const char UsermodBME68X::_unitDrewp[]     = " ";  // NOTE - Is set with the selectable temperature unit
const char UsermodBME68X::_unitIaq[]       = " ";  // No unit
const char UsermodBME68X::_unitStaticIaq[] = " ";  // No unit
const char UsermodBME68X::_unitCo2[]       = "ppm";
const char UsermodBME68X::_unitVoc[]       = "ppm";
const char UsermodBME68X::_unitGasPer[]    = "%";
const char UsermodBME68X::_unitNone[]      = "";

const char UsermodBME68X::_unitCelsius[]    = "°C";  // Symbol for Celsius
const char UsermodBME68X::_unitFahrenheit[] = "°F";  // Symbol for Fahrenheit
...
void UsermodBME68X::setup() {
  ...
  stringbuff = "BSEC library version " + String(iaqSensor.version.major) + "." + String(iaqSensor.version.minor) + "." +
               String(iaqSensor.version.major_bugfix) + "." + String(iaqSensor.version.minor_bugfix);
  DEBUG_PRINT(UMOD_NAME);
  DEBUG_PRINTLN(stringbuff.c_str());

  /* Init Sensor*/
  iaqSensor.setConfig(bsec_config_iaq);
  iaqSensor.updateSubscription(sensorList, 13, BSEC_SAMPLE_RATE_LP);
  iaqSensor.setTPH(BME68X_OS_2X, BME68X_OS_16X,
                   BME68X_OS_1X);                       // Set the temperature, Pressure and Humidity over-sampling
  ...
  DEBUG_PRINTLN(INFO_COLUMN GOGAB_DONE);
}
...
void UsermodBME68X::loop() {
  if (!settings.enabled || strip.isUpdating() || !flags.InitSuccessful) {
    return;  // Leave if not enabled or string is updating or init failed
  }

  if (settings.pauseOnActiveWled && strip.getBrightness()) {
    return;  // Workarround Known Issue: handing led update - Leave once pause on activ wled is active and wled is
             // active
  }
  ...
  if (timer.actual - timer.lastRun >= settings.Interval * 1000) {
    ...
    if (iaqSensor.run()) {  // iaqSensor.run()
      ...
      if (ValuesPtr->temperature != PrevValuesPtr->temperature ||
          !settings.PublischChange) {  // NOTE - negative dig means inactive
      ...
      if (ValuesPtr->iaq != PrevValuesPtr->iaq || !settings.PublischChange) {
        ...
        if (settings.pubAcc) {
          MQTT_publish(_nameIaqAc, ValuesPtr->iaqAccuracy, 0);
        }
        if (settings.decimals.iaq > -1) {
          if (settings.PublishIAQVerbal) {
            if (ValuesPtr->iaq <= 50) {
              cvalues.iaqVerbal = "Excellent";
            } else if (ValuesPtr->iaq <= 100) {
              cvalues.iaqVerbal = "Good";
            } else if (ValuesPtr->iaq <= 150) {
              cvalues.iaqVerbal = "Lightly polluted";
            } else if (ValuesPtr->iaq <= 200) {
              cvalues.iaqVerbal = "Moderately polluted";
            } else if (ValuesPtr->iaq <= 250) {
              cvalues.iaqVerbal = "Heavily polluted";
            } else if (ValuesPtr->iaq <= 350) {
              cvalues.iaqVerbal = "Severely polluted";
            } else {
              cvalues.iaqVerbal = "Extremely polluted";
            }
            snprintf(charbuffer, 127, "%s/%s", mqttDeviceTopic, _nameIaqVerb);
            if (WLED_MQTT_CONNECTED) {
              mqtt->publish(charbuffer, 0, false, cvalues.iaqVerbal.c_str());
            }
          }
        }
      }
      if (ValuesPtr->staticIaq != PrevValuesPtr->staticIaq || !settings.PublischChange) {
        ...
        if (settings.pubAcc) {
          MQTT_publish(_nameStaticIaqAc, ValuesPtr->staticIaqAccuracy, 0);
        }
        if (settings.decimals.staticIaq > -1) {
          if (settings.PublishIAQVerbal) {
            if (ValuesPtr->staticIaq <= 50) {
              cvalues.staticIaqVerbal = "Excellent";
            } else if (ValuesPtr->staticIaq <= 100) {
              cvalues.staticIaqVerbal = "Good";
            } else if (ValuesPtr->staticIaq <= 150) {
              cvalues.staticIaqVerbal = "Lightly polluted";
            } else if (ValuesPtr->staticIaq <= 200) {
              cvalues.staticIaqVerbal = "Moderately polluted";
            } else if (ValuesPtr->staticIaq <= 250) {
              cvalues.staticIaqVerbal = "Heavily polluted";
            } else if (ValuesPtr->staticIaq <= 350) {
              cvalues.staticIaqVerbal = "Severely polluted";
            } else {
              cvalues.staticIaqVerbal = "Extremely polluted";
            }
            snprintf(charbuffer, 127, "%s/%s", mqttDeviceTopic, _nameStaticIaqVerb);
            if (WLED_MQTT_CONNECTED) {
              mqtt->publish(charbuffer, 0, false, cvalues.staticIaqVerbal.c_str());
            }
          }
        }
      }
      ...
      if (ValuesPtr->gasPerc != PrevValuesPtr->gasPerc || !settings.PublischChange) {
        ...
        if (settings.pubAcc) {
          MQTT_publish(_nameGasPerAc, ValuesPtr->gasPercAccuracy, 0);
        }
      }

      /**** Publish Sensor State Entrys *****/
      if ((ValuesPtr->stabStatus != PrevValuesPtr->stabStatus || !settings.PublischChange) &&
          settings.publishSensorState) {
        MQTT_publish(_nameStabStatus, ValuesPtr->stabStatus, 0);
      }
      if ((ValuesPtr->runInStatus != PrevValuesPtr->runInStatus || !settings.PublischChange) &&
          settings.publishSensorState) {
        MQTT_publish(_nameRunInStatus, ValuesPtr->runInStatus, 0);
      }

      /* Check accuracies - if accurasy level 3 is reached -> save calibration data */
      if ((ValuesPtr->iaqAccuracy != PrevValuesPtr->iaqAccuracy) && ValuesPtr->iaqAccuracy == 3) {
        flags.SaveState = true;  // Save after calibration / recalibration
      }
      if ((ValuesPtr->staticIaqAccuracy != PrevValuesPtr->staticIaqAccuracy) && ValuesPtr->staticIaqAccuracy == 3) {
        flags.SaveState = true;
      }
      if ((ValuesPtr->co2Accuracy != PrevValuesPtr->co2Accuracy) && ValuesPtr->co2Accuracy == 3) {
        flags.SaveState = true;
      }
      if ((ValuesPtr->VocAccuracy != PrevValuesPtr->VocAccuracy) && ValuesPtr->VocAccuracy == 3) {
        flags.SaveState = true;
      }
      if ((ValuesPtr->gasPercAccuracy != PrevValuesPtr->gasPercAccuracy) && ValuesPtr->gasPercAccuracy == 3) {
        flags.SaveState = true;
      }

      if (flags.SaveState) {
        saveState();  // Save if the save state flag is set
      }
    }
  }
}
...
void UsermodBME68X::getValues() {
  ...
  /* Float Values */
  ValuesPtr->temperature =
      roundf(iaqSensor.temperature * powf(10, settings.decimals.temperature)) / powf(10, settings.decimals.temperature);
  ValuesPtr->humidity =
      roundf(iaqSensor.humidity * powf(10, settings.decimals.humidity)) / powf(10, settings.decimals.humidity);
  ValuesPtr->pressure      = roundf(iaqSensor.pressure * powf(10, settings.decimals.pressure)) /
                             powf(10, settings.decimals.pressure) / 100;  // Pa 2 hPa
  ValuesPtr->gasResistance = roundf(iaqSensor.gasResistance * powf(10, settings.decimals.gasResistance)) /
                             powf(10, settings.decimals.gasResistance) / 1000;  // Ohm 2 KOhm
  ValuesPtr->iaq           = roundf(iaqSensor.iaq * powf(10, settings.decimals.iaq)) / powf(10, settings.decimals.iaq);
  ValuesPtr->staticIaq =
      roundf(iaqSensor.staticIaq * powf(10, settings.decimals.staticIaq)) / powf(10, settings.decimals.staticIaq);
  ValuesPtr->co2 = roundf(iaqSensor.co2Equivalent * powf(10, settings.decimals.co2)) / powf(10, settings.decimals.co2);
  ValuesPtr->Voc =
      roundf(iaqSensor.breathVocEquivalent * powf(10, settings.decimals.Voc)) / powf(10, settings.decimals.Voc);
  ValuesPtr->gasPerc =
      roundf(iaqSensor.gasPercentage * powf(10, settings.decimals.gasPerc)) / powf(10, settings.decimals.gasPerc);

  /* Calculate Absolute Humidity [g/m³] */
  if (settings.decimals.absHumidity > -1) {
    const float mw = 18.01534;    // molar mass of water g/mol
    const float r  = 8.31447215;  // Universal gas constant J/mol/K
    ValuesPtr->absHumidity =
        (6.112 * powf(2.718281828, (17.67 * ValuesPtr->temperature) / (ValuesPtr->temperature + 243.5)) *
         ValuesPtr->humidity * mw) /
        ((273.15 + ValuesPtr->temperature) * r);  // in ppm
  }
  /* Calculate Drew Point (C°) */
  if (settings.decimals.drewPoint > -1) {
    ValuesPtr->drewPoint =
        (243.5 *
         (log(ValuesPtr->humidity / 100) + ((17.67 * ValuesPtr->temperature) / (243.5 + ValuesPtr->temperature))) /
         (17.67 - log(ValuesPtr->humidity / 100) -
          ((17.67 * ValuesPtr->temperature) / (243.5 + ValuesPtr->temperature))));
  }
...
/**
 * @brief MQTT initialization to generate the mqtt topic strings. This initialization also creates the HomeAssistat
 * device configuration (HA Discovery), which home assinstant automatically evaluates to create a device.
 */
void UsermodBME68X::HomeAssistantDiscovery() {
  if (!settings.HomeAssistantDiscovery || !flags.InitSuccessful || !settings.enabled) {
    return;  // Leave once HomeAssistant Discovery is inactive
  }
  ...
  MQTT_PublishHASensor(
      _nameGasRes, "GAS", _unitGasres,
      settings.decimals.gasResistance);  // There is no device class for resistance in HA yet:
  ...
  MQTT_PublishHASensor(_nameIaqVerb, "", _unitNone, settings.PublishIAQVerbal,
                       2);  // IAQ Verbal / Set Option 2 (text sensor)
  MQTT_PublishHASensor(_nameStaticIaq, "AQI", _unitNone, settings.decimals.staticIaq);  // Static IAQ
  MQTT_PublishHASensor(_nameStaticIaqVerb, "", _unitNone, settings.PublishStaticIAQVerbal,
                       2);                                                 // IAQ Verbal / Set Option 2 (text sensor
   ...
   */
  MQTT_PublishHASensor(_nameIaqAc, "AQI", _unitNone, settings.pubAcc - 1 + settings.decimals.iaq * settings.pubAcc,
                       1);  // Option 1: Diagnostics Sektion
  MQTT_PublishHASensor(_nameStaticIaqAc, "", _unitNone,
                       settings.pubAcc - 1 + settings.decimals.staticIaq * settings.pubAcc, 1);
  ...
  MQTT_PublishHASensor(_nameGasPerAc, "", _unitNone, settings.pubAcc - 1 + settings.decimals.gasPerc * settings.pubAcc,
                       1);
...
/**
 * @brief These MQTT entries are responsible for the Home Assistant Discovery of the sensors. HA is shown here where to
 * look for the sensor data. This entry therefore only needs to be sent once. Important note: In order to find
 * everything that is sent from this device to Home Assistant via MQTT under the same device name, the
 * "device/identifiers" entry must be the same. I use the MQTT device name here. If other user mods also use the HA
 * Discovery, it is recommended to set the identifier the same. Otherwise you would have several devices, even though it
 * is one device. I therefore only use the MQTT client name set in WLED here.
 ...
 */
void UsermodBME68X::MQTT_PublishHASensor(const String& name, const String& deviceClass, const String& unitOfMeasurement,
                                         const int8_t& digs, const uint8_t& option) {
  ...
  String basetopic = String(_hadtopic) + mqttClientID + "/" + name +
                     "/config";  // This is the place where Home Assinstant Discovery will check for new devices
  ...
  if (digs < 0) {  // if digs are set to -1 -> entry deactivated
    ...
    /* Create all the necessary HAD MQTT entrys - see:
    ...
    avail["topic"]   = mqttDeviceTopic +
                       String("/status");  // An MQTT topic subscribed to receive availability (online/offline) updates.
    avail["payload_available"]     = "online";
    avail["payload_not_available"] = "offline";
    JsonObject device =
        jdoc.createNestedObject("device");  // Information about the device this sensor is a part of to tie it into the
                                            // device registry. Only works when unique_id is set. At least one of
    ...
    if (deviceClass != "") {
      jdoc["device_class"] =
          deviceClass;  // The type/class of the sensor to set the icon in the frontend. The device_class can be null
    }
    if (option == 1) {
      jdoc["entity_category"] = "diagnostic";  // Option 1: The category of the entity | When set, the entity category
                                               // must be diagnostic for sensors.
    }
    if (option == 2) {
      jdoc["mode"] = "text";  // Option 2: Set text mode |
    }
    jdoc["expire_after"] =
        1800;  // If set, it defines the number of seconds after the sensor’s state expires, if it’s not updated. After
               // expiry, the sensor’s state becomes unavailable. Default the sensors state never expires.
    jdoc["name"] = name;  // The name of the MQTT sensor. Without server/module/device name. The device name will be
                          // added by HomeAssinstant anyhow
    if (unitOfMeasurement != "") {
      jdoc["state_class"] =
          "measurement";  // NOTE: This entry is missing in some other usermods. But it is very important. Because only
                          // with this entry, you can use statistics (such as statistical graphs).
    }
    jdoc["state_topic"] =
        charbuffer;  // The MQTT topic subscribed to receive sensor values. If device_class, state_class,
                     // unit_of_measurement or suggested_display_precision is set, and a numeric value is expected, an
                     // empty value '' will be ignored and will not update the state, a 'null' value will set the sensor
                     // to an unknown state. The device_class can be null.
    jdoc["unique_id"] =
        String(mqttClientID) + "-" + name;  // An ID that uniquely identifies this sensor. If two sensors have the same
                                            // unique ID, Home Assistant will raise an exception.
    if (unitOfMeasurement != "") {
      jdoc["unit_of_measurement"] = unitOfMeasurement;  // Defines the units of measurement of the sensor, if any. The
...
void UsermodBME68X::addToJsonInfo(JsonObject& root) {
  ...
  if (!flags.InitSuccessful) {
    ...
    if (settings.pubAcc) {
      if (settings.decimals.iaq >= 0) {
        InfoHelper(user, _nameIaqAc, ValuesPtr->iaqAccuracy, 0, " ");
      }
      if (settings.decimals.staticIaq >= 0) {
        InfoHelper(user, _nameStaticIaqAc, ValuesPtr->staticIaqAccuracy, 0, " ");
      }
      if (settings.decimals.co2 >= 0) {
        InfoHelper(user, _nameCo2Ac, ValuesPtr->co2Accuracy, 0, " ");
      }
      if (settings.decimals.Voc >= 0) {
        InfoHelper(user, _nameVocAc, ValuesPtr->VocAccuracy, 0, " ");
      }
      if (settings.decimals.gasPerc >= 0) {
        InfoHelper(user, _nameGasPerAc, ValuesPtr->gasPercAccuracy, 0, " ");
      }
    }
 ...
 */
void UsermodBME68X::InfoHelper(JsonObject& root, const char* name, const float& sensorvalue, const int8_t& decimals,
                               const char* unit) {
  if (decimals > -1) {
...
void UsermodBME68X::appendConfigData() {
  // snprintf(charbuffer, 127, "addInfo('%s:%s',1,'read interval [seconds]';"), UMOD_NAME, _nameInterval);
  // oappend(charbuffer); snprintf(charbuffer, 127, "addInfo('%s:%s',1,'only if value changes';"), UMOD_NAME,
  // _namePublishChange); oappend(charbuffer); snprintf(charbuffer, 127, "addInfo('%s:%s',1,'maximum age of a message in
  // seconds';"), UMOD_NAME, _nameMaxAge); oappend(charbuffer); snprintf(charbuffer, 127, "addInfo('%s:%s',1,'Gas
  // related values are only published after the gas sensor has been calibrated';"), UMOD_NAME, _namePubAfterCalib);
  // oappend(charbuffer); snprintf(charbuffer, 127, "addInfo('%s:%s',1,'* Set to minus to deactivate (all sensors)');"),
  // UMOD_NAME, _nameTemp); oappend(charbuffer);
...
/**
 * @brief Called by WLED: Read Usermod Config Settings default settings values could be set here (or below using the
 * 3-argument getJsonValue()) instead of in the class definition or constructor setting them inside readFromConfig() is
 * slightly more robust, handling the rare but plausible use case of single value being missing after boot (e.g. if the
 * cfg.json was manually edited and a value was removed) This is called whenever WLED boots and loads cfg.json, or when
 * the UM config page is saved. Will properly re-instantiate the SHT class upon type change and publish HA discovery
 * after enabling. NOTE: Here are the default settings of the user module
 * @param JsonObject Pointer
 * @return bool
...
bool UsermodBME68X::readFromConfig(JsonObject& root) {
  ...
  configComplete &= getJsonValue(top[_nameI2CAdr], settings.I2cadress,
                                 0x77);  // Defalut IC2 adress set to 0x77 (some modules are set to 0x76)
  configComplete &= getJsonValue(top[_nameInterval], settings.Interval, 1);  // Executed every second
  configComplete &=
      getJsonValue(top[_namePublishChange], settings.PublischChange, false);  // Publish changed values only
  configComplete &=
      getJsonValue(top[_nameTempScale], settings.tempScale, 0);  // Temp sale set to Celsius (1=Fahrenheit)
  ...
  configComplete &=
      getJsonValue(top[_nameHADisc], settings.HomeAssistantDiscovery,
                   true);  // Activate HomeAssistant Discovery (this Module will be shown as MQTT device in HA)
  configComplete &= getJsonValue(top[_namePauseOnActWL], settings.pauseOnActiveWled,
                                 false);  // Pause on active WLED not activated per default
  configComplete &=
      getJsonValue(top[_nameDelCalib], flags.DeleteCaibration,
                   false);  // IF checked the calibration file will be delete when the save button is pressed
  ...
  configComplete &=
      getJsonValue(top["Sensors"][_nameIaq], settings.decimals.iaq, 0);  // Index for Air Quality Number is active
  configComplete &= getJsonValue(top["Sensors"][_nameIaqVerb], settings.PublishIAQVerbal,
                                 -1);  // deactivated - Index for Air Quality (IAQ) verbal classification
  configComplete &= getJsonValue(top["Sensors"][_nameStaticIaq], settings.decimals.staticIaq,
                                 0);  // activated - Static IAQ is better than IAQ for devices that are not moved
  ...
  if (settings.Interval < 1) {
    settings.Interval = 1;  // Correct interval on need (A number less than 1 is not permitted)
  }
...
/**
 * @brief Returns the current static iaq (Indoor Air Quallity) (NOTE: Static iaq is the better choice than iaq for fixed
 * devices such as the wled module)
...
void UsermodBME68X::saveState() {
  ...
  if (!file) {
    ...
    if (WLED_MQTT_CONNECTED) {
      mqtt->publish(charbuffer, 0, false, contbuffer);
    }
