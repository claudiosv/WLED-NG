 ...
 * - on/off - just swipe a hand below your sensor ("shortPressAction" is called and can be customized through WLED
 macros)
 ...
 * 2. Add `-D USERMOD_VL53L0X_GESTURES` to your build flags at platformio.ini (plaformio_override.ini) for needed
 environment.
...
#define VL53L0X_MIN_RANGE_OFFSET \
  60  // minimal range in millimeters that sensor can detect. Used in long motions to correct brightness calculation.
#endif
    ...
    if (millis() - lastTime > VL53L0X_DELAY_MS) {
      ...
      if (range < VL53L0X_MAX_RANGE_MM) {
        ...
        if (millis() - motionStartTime > VL53L0X_LONG_MOTION_DELAY_MS)  // long motion
        {
          ...
          bri = (VL53L0X_MAX_RANGE_MM - max(range, VL53L0X_MIN_RANGE_OFFSET)) * 255 /
                (VL53L0X_MAX_RANGE_MM - VL53L0X_MIN_RANGE_OFFSET);
          DEBUG_PRINTF("new brightness: %d", bri);
          stateUpdated(1);
        }
      } else if (wasMotionBefore) {  // released
        if (!isLongMotion) {         // short press
