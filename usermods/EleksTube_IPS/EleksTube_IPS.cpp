...
class ElekstubeIPSUsermod : public Usermod {
 private:
  ...
  void updateClockDisplay(TFTs::show_t show = TFTs::yes) {
    ...
    for (uint8_t i = 0; i < 6; i++) {
      ...
      if (c >= '0' && c <= '9') {
        tfts.setDigit(5 - i, c - '0', show);
        set[i] = true;
      } else if (c >= 'A' && c <= 'G') {
        tfts.setDigit(5 - i, c - 'A' + 10, show);
        set[i] = true;  // 10.bmp to 16.bmp static display
      } else if (c == '-' || c == '_' || c == ' ') {
        tfts.setDigit(5 - i, 255, show);
        set[i] = true;  // blank
      } else {
 ...
 public:
  ...
  bool readFromConfig(JsonObject& root) {
    ...
    if (tfts.digitOffset > 240) {
      tfts.digitOffset = 240;
    }
    if (tfts.digitOffset != digitOffsetPrev) {
      fshow = TFTs::force;
    }
