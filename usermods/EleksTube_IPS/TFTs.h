...
class TFTs : public TFT_eSPI {
 private:
  ...
  bool drawBmp(const char *filename) {
    ...
    if (bitDepth <= 8)  // 1,4,8 bit bitmap: read color palette
    {
      (void)read32(bmpFS);
      (void)read32(bmpFS);
      (void)read32(bmpFS);  // size, w resolution, h resolution
      paletteSize = read32(bmpFS);
      if (paletteSize == 0) {
        paletteSize = 1 << bitDepth;  // if 0, size is 2^bitDepth
      }
    ...
    for (row = h - 1; row >= 0; row--) {
      if ((row & 0b00000111) == serviceStrip) {
        strip.service();  // still refresh backlight to mitigate stutter every few rows
      }
      bmpFS.read(lineBuffer, sizeof(lineBuffer));
 ...
 public:
  ...
  void showDigit(uint8_t digit) {
    ...
    if (!digitColor && digitToDraw == bufferedDigit) {
      drawBuffer();
    }
    digitR = R(digitColor);
    digitG = G(digitColor);
    ...
    if (WLED_FS.exists(file_name)) {
      if (drawBin(file_name)) {
        bufferedDigit = digitToDraw;
      }
      return;
    }
