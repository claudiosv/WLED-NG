...
class QuinLEDAnPentaUsermod : public Usermod {
 private:
  ...
  void initOledDisplay() {
    ...
    oledDisplay = (U8G2 *)new U8G2_SSD1306_128X64_NONAME_2_4W_SW_SPI(U8G2_R0, oledSpiClk, oledSpiData, oledSpiCs,
                                                                     oledSpiDc, oledSpiRst);
  ...
  bool oledCheckForNetworkChanges() {
    if (lastKnownNetworkConnected != Network.isConnected() || lastKnownIp != Network.localIP() ||
        lastKnownWiFiConnected != WiFi.isConnected() || lastKnownSsid != WiFi.SSID() || lastKnownApActive != apActive ||
        lastKnownApSsid != apSSID || lastKnownApPass != apPass || lastKnownApChannel != apChannel) {
  ...
  void oledShowPage(byte page, bool updateLastTimePageChange = false) {
    ...
    if (updateLastTimePageChange) {
      oledLastTimePageChange = oledLastTimeUpdated;
    }
  }
      ...
      switch (oledCurrentPage) {
        ...
        case 1: {
          ...
          if (oledUseProgressBars) {
            ...
            // There is no method to draw a filled box with rounded corners. So draw the rounded frame first, then fill
            // that frame accordingly to LED percentage
            ...
            for (int8_t clp = 0; clp <= 4; clp++) {
              if (anPentaLEDPins[app] == currentLedPins[clp]) {
                ...
                if (oledUseProgressBars) {
                  ...
                  oledDisplay->drawBox(39, oledRow - 5 + (drawnLines * 8),
                                       int(round(88 * getPercentageForBrightness(currentLedcReads[clp])) / 100), 5);
                } else {
                  sprintf(charCurrentLedcReads, "%s %d%%", charCurrentLedcReads,
                          getPercentageForBrightness(currentLedcReads[clp]));
                  oledDisplay->drawStr(0, oledRow + (drawnLines * 8), charCurrentLedcReads);
                }
        ...
        case 2: {
          if (isShtReady() && shtReadDataSuccess) {
            char charShtCurrentTemp[charPerRow + 4];  // Reserve 3 more bytes than usual as we gonna have one UTF8 char
                                                      // which can be up to 4 bytes.
        ...
        case 3:
          ...
          // If WLED AP is active. Theoretically, it can even be active with ETH being connected, but we don't mind...
            ...
            sprintf(charCurrentApStatus, "WLED AP: %s (Ch: %d)", (lastKnownApActive ? "On" : "Off"),
                    lastKnownApChannel);
      ...
      if (millis() - oledLastTimeUpdated > 150) {
        ...
        switch (oledCurrentPage) {
          case 1:
            ...
            if (bri != lastKnownBri || lastKnownLedcReads[0] != currentLedcReads[0] ||
                lastKnownLedcReads[1] != currentLedcReads[1] || lastKnownLedcReads[2] != currentLedcReads[2] ||
                lastKnownLedcReads[3] != currentLedcReads[3] || lastKnownLedcReads[4] != currentLedcReads[4]) {
              lastKnownLedcReads[0] = currentLedcReads[0];
              lastKnownLedcReads[1] = currentLedcReads[1];
              lastKnownLedcReads[2] = currentLedcReads[2];
              lastKnownLedcReads[3] = currentLedcReads[3];
              lastKnownLedcReads[4] = currentLedcReads[4];
...
// strings to reduce flash memory usage (used more than twice)
// Config settings
const char QuinLEDAnPentaUsermod::_name[]                = "QuinLED-An-Penta";
const char QuinLEDAnPentaUsermod::_enabled[]             = "Enabled";
const char QuinLEDAnPentaUsermod::_oledEnabled[]         = "Enable-OLED";
const char QuinLEDAnPentaUsermod::_oledUseProgressBars[] = "OLED-Use-Progress-Bars";
const char QuinLEDAnPentaUsermod::_oledFlipScreen[]      = "OLED-Flip-Screen-180";
const char QuinLEDAnPentaUsermod::_oledSecondsPerPage[]  = "OLED-Seconds-Per-Page";
const char QuinLEDAnPentaUsermod::_oledFixBuggedScreen[] = "OLED-Fix-Bugged-Screen";
const char QuinLEDAnPentaUsermod::_shtEnabled[]          = "Enable-SHT30-Temp-Humidity-Sensor";
// Other strings

const unsigned char QuinLEDAnPentaUsermod::quinLedLogo[] = {
    ,
