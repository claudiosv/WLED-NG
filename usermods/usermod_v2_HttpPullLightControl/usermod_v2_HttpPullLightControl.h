 ...
 *           number of lights that should turn on. Each light is a friend who did contact your server in the past 5
 * minutes. So on each of your friends LED rings, the number of lights will be the number of friends who have it turned
 * on. Features: It sends a unique ID (has of MAC and salt) to the URL, so you can define each client without a need to
 * map their IP address. Tested: Tested on WLED v0.14 with ESP32-S3 (M5Stack Atom S3 Lite), but should also workd for
 * other ESPs.
 */

#include "wled.h"

// Use the following for SHA1 computation of our HASH, unfortunatelly PlatformIO doesnt recognize Hash.h while its
// already in the Core. We use mbedtls/sha256.h for ESP32 (in the core).
#ifdef ESP32
...
class HttpPullLightControl : public Usermod {
 private:
...
#else
  String url = "http://example.org/example.php";  // Default-URL (http only!), can also be url with IP address in it.
                                                  // HttpS urls are not supported (yet) because of AsyncTCP library
#endif
...
#else
  String salt =
      "1just_a_very-secret_salt2";  // Salt for generating a unique ID when requesting the URL (in this way you can give
                                    // different answers based on the WLED device who does the request)
#endif
  // NOTE THAT THERE IS ALSO A #ifdef HTTP_PULL_LIGHT_CONTROL_HIDE_URL and a HTTP_PULL_LIGHT_CONTROL_HIDE_SALT IF YOU DO
  // NOT WANT TO SHOW THE OPTIONS IN THE USERMOD SETTINGS

  // Define constants
  static const uint8_t myLockId =
      USERMOD_ID_HTTP_PULL_LIGHT_CONTROL;              // Used for the requestJSONBufferLock(id) function
  ...
  static const unsigned long inactivityTimeout =
      30000;  // When the AsyncClient is inactive (hanging) for this many milliseconds, we kill it
 ...
 public:
  ...
  uint16_t getId() {
    return USERMOD_ID_HTTP_PULL_LIGHT_CONTROL;
  }
  ...
  inline bool isEnabled() {
    return enabled;
  }  // Get usermod enabled or disabled state
