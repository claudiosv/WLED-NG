 ...
 public:
  void setup() {
    ...
    if ((ldrPin >= 0) && (digitalPinToAnalogChannel(ldrPin) >= 0)) {
      if (!PinManager::allocatePin(ldrPin, false, PinOwner::UM_LDR_DUSK_DAWN)) {
  ...
  void loop() {
    ...
    if (millis() - lastMillis > 10000) {
      if ((ldrEnabled == true) && (ldrPin >= 0) &&
          (digitalPinToAnalogChannel(ldrPin) >= 0)) {  // make sure that pin is valid for analogread()
