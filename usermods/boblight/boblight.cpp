...
class BobLightUsermod : public Usermod {
  ...
  */
  ...
  void fillBobLights(int bottom, int left, int top, int right, float pct_scan) {
    ...
    for (int i = 0; i < numLights; i++) {
      DEBUG_PRINTF_P(" light %s scan %2.1f %2.1f %2.1f %2.1f\n", lights[i].lightname, lights[i].vscan[0],
                     lights[i].vscan[1], lights[i].hscan[0], lights[i].hscan[1]);
    }
#endif
  }
  ...
  void BobSync() {
    yield();
  }  // allow other tasks, should also be used to force pixel redraw (not with WLED)
  void BobClear() {
 ...
 public:
  void setup() override {
    ...
    if (lights) {
      fillBobLights(bottom, left, top, right, float(pct));  // will fill numLights
    } else {
      enable(false);
    }
    initDone = true;
  ...
  uint16_t getId() override {
    return USERMOD_ID_BOBLIGHT;
  }
};

// strings to reduce flash memory usage (used more than twice)
const char BobLightUsermod::_name[]    = "BobLight";
const char BobLightUsermod::_enabled[] = "enabled";
