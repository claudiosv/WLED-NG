...
#define FFT_PREFER_EXACT_PEAKS  // use Blackman-Harris FFT windowing instead of Flat Top -> results in "sharper" peaks
                                // and less "leaking" into other frequencies (credits to @softhack)
...
// #define UM_AUDIOREACTIVE_USE_INTEGER_FFT // use integer FFT if using ESP-IDF DSP library, always used on S2 and C3
// (UM_AUDIOREACTIVE_USE_ARDUINO_FFT takes priority) #define UM_AUDIOREACTIVE_USE_ARDUINO_FFT // default on ESP32 and S3
...
static volatile bool disableSoundProcessing =
    false;  // if true, sound processing (FFT, filters, AGC) will be suspended. "volatile" as its shared between tasks.
...
#endif
// static float    volumeSmth = 0.0f;              // either sampleAvg or sampleAgc depending on soundAgc; smoothed
// sample
static float FFT_MajorPeak = 1.0f;   // FFT: strongest (peak) frequency
static float FFT_Magnitude = 0.0f;   // FFT: volume (magnitude) of peak frequency
static bool  samplePeak    = false;  // Boolean flag for peak - used in effects. Responding routine may reset this flag.
                                     // Auto-reset after strip.getFrameTime()
static bool udpSamplePeak =
    false;  // Boolean flag for peak. Set at the same time as samplePeak, but reset by transmitAudioData
static unsigned long timeOfPeak            = 0;    // time of last sample peak detection.
static uint8_t fftResult[NUM_GEQ_CHANNELS] = {0};  // Our calculated freq. channel result table to be used by effects

// TODO: probably best not used by receive nodes
// static float agcSensitivity = 128;            // AGC sensitivity estimation, based on agc gain (multAgc). calculated
// by getSensitivity(). range 0..255
...
static void detectSamplePeak(
    void);  // peak detection function (needs scaled FFT results in vReal[]) - no used for 8266 receive-only mode
#endif
static void    autoResetPeak(void);  // peak auto-reset function
static uint8_t maxVol =
    31;  // (was 10) Reasonable value for constant volume for 'peak detector', as it won't always trigger  (deprecated)
...
#define UM_AUDIOREACTIVE_USE_ARDUINO_FFT  // use ArduinoFFT library for FFT instead of ESP-IDF DSP library by default on
                                          // ESP32 and S3
#endif
...
const double agcSampleDecay[AGC_NUM_PRESETS] = {
    0.9994f, 0.9985f, 0.9997f};  // decay factor for sampleMax, in case the current sample is below sampleMax
...
const double agcFollowSlow[AGC_NUM_PRESETS] = {1 / 6144.f, 1 / 4096.f,
...
const float  agcSampleSmooth[AGC_NUM_PRESETS] = {
    1 / 12.f, 1 / 6.f,
    1 / 16.f};  // smoothing factor for sampleAgc (use rawSampleAgc if you want the non-smoothed value)
// AGC presets end

static AudioSource* audioSource       = nullptr;
static bool         useBandPassFilter = false;  // if true, enables a hard cutoff bandpass filter. Applies after FFT.
static bool         useMicFilter =
    false;  // if true, enables a IIR bandpass filter 80Hz-20Khz to remove noise. Applies before FFT.
////////////////////
// Begin FFT Code //
////////////////////
...
static float fftResultMax[NUM_GEQ_CHANNELS] = {
    0.0f};  // A table used for testing to determine how our post-processing is working.
#endif

// audio source parameters and constant
constexpr SRate_t SAMPLE_RATE =
    22050;  // Base sample rate in Hz - 22Khz is a standard rate. Physical sample time -> 23ms
// constexpr SRate_t SAMPLE_RATE = 16000;        // 16kHz - use if FFTtask takes more than 20ms. Physical sample time ->
// 32ms constexpr SRate_t SAMPLE_RATE = 20480;        // Base sample rate in Hz - 20Khz is experimental.    Physical
// sample time -> 25ms constexpr SRate_t SAMPLE_RATE = 10240;        // Base sample rate in Hz - previous default.
...
constexpr uint16_t samplesFFT_2 =
    256;  // meaningfull part of FFT results - only the "lower half" contains useful information.
// the following are observed values, supported by a bit of "educated guessing"
// #define FFT_DOWNSCALE 0.65f                             // 20kHz - downscaling factor for FFT results - "Flat-Top"
// window @20Khz, old freq channels
#ifdef FFT_PREFER_EXACT_PEAKS
#define FFT_DOWNSCALE \
  0.40f  // downscaling factor for FFT results, RMS averaging for "Blackman-Harris" Window @22kHz (credit to MM)
#else
...
// #define sqrt_internal sqrtf          // see https://github.com/kosme/arduinoFFT/pull/83 - since v2.0.0 this must be
// done in build_flags
...
#else
  result *= 32;  // scale result to match float values. note: raw scaling value between float and int is 512, float
                 // version is scaled down by 16
#endif
...
void FFTcode(void* parameter) {
  ...
  if (valFFT == nullptr) {
    valFFT = (float*)calloc(samplesFFT, sizeof(float));
  }
  if (vImag == nullptr) {
    vImag = (float*)calloc(samplesFFT, sizeof(float));
  }
  ...
  // note: free() is never used on these pointers. If it ever is implemented, this implementation can cause memory leaks
  // (need to free raw pointers)
  if (valFFT == nullptr) {
    float* raw_buffer = (float*)heap_caps_malloc((2 * samplesFFT * sizeof(float)) + 16, MALLOC_CAP_8BIT);
    if ((raw_buffer == nullptr)) {
      return;  // something went wrong
    }
    valFFT = (float*)(((uintptr_t)raw_buffer + 15) & ~15);  // SIMD requires aligned memory to 16-byte boundary. note in
                                                            // IDF5 there is MALLOC_CAP_SIMD available
  }
  // create window
  ...
  if (dsps_fft2r_init_fc32(NULL, samplesFFT) != ESP_OK) {
    return;  // initialize FFT tables
  }
  ...
  if (valFFT == nullptr) {
    valFFT = (int16_t*)calloc(sizeof(int16_t), samplesFFT * 2);
  }
  if ((valFFT == nullptr)) {
    return;  // something went wrong
  }
  // create window
  if (windowFFT == nullptr) {
    windowFFT = (int16_t*)calloc(sizeof(int16_t), samplesFFT);
  }
  if ((windowFFT == nullptr)) {
    return;  // something went wrong
  }
  if (dsps_fft2r_init_sc16(NULL, samplesFFT) != ESP_OK) {
    return;  // initialize FFT tables
  }
  // create window function for FFT
  float* windowFloat = (float*)calloc(sizeof(float), samplesFFT);  // temporary buffer for window function
  if ((windowFloat == nullptr)) {
    return;  // something went wrong
  }
#ifdef FFT_PREFER_EXACT_PEAKS
  ...
  for (;;) {
    delay(1);  // DO NOT DELETE THIS LINE! It is needed to give the IDLE(0) task enough time and to keep the watchdog
               // happy. taskYIELD(), yield(), vTaskDelay() and esp_task_wdt_feed() didn't seem to work.
    ...
    if (audioSource) {
      audioSource->getSamples(valFFT,
                              samplesFFT);  // note: valFFT is used as a int16_t buffer on C3 and S2, could optimize RAM
                                            // use by only allocating half the size (but makes code harder to read)
    }
    ...
    // band pass filter - can reduce noise floor by a factor of 50 and avoid aliasing effects to base & high frequency
    // bands downside: frequencies below 100Hz will be ignored
    if (useMicFilter) {
    ...
    for (int i = 0; i < samplesFFT; i++) {
      ...
      if ((valFFT[i] <= (INT16_MAX - 1024)) &&
          (valFFT[i] >= (INT16_MIN + 1024))) {  // skip extreme values - normally these are artefacts
        if (FFTabs(valFFT[i]) > maxSample) {
          maxSample = FFTabs(valFFT[i]);
        }
      }
    }
    // release highest sample to volume reactive effects early - not strictly necessary here - could also be done at the
    // end of the function early release allows the filters (getSample() and agcAvg()) to work with fresh values - we
    // will have matching gain and noise gate values when we want to process the FFT results.
    ...
    if (sampleAvg >
      ...
      FFT.windowing(FFTWindow::Blackman_Harris,
                    FFTDirection::Forward);  // Weigh data using "Blackman- Harris" window - sharp peaks due to
      ...
      valFFT[0] = 0;  // The remaining DC offset on the signal produces a strong spike on position 0 that should be
                      // eliminated to avoid issues.
      FFT.majorPeak(&FFT_MajorPeak, &FFT_Magnitude);  // let the effects know which freq was most dominant
                                                      // note: scaling is done in fftAddAvg(), so we don't scale here
#else
      // run run float DSP FFT (takes ~x ms on ESP32, ~x ms on ESP32-S2, , ~x ms on ESP32-C3) TODO: test and fill in
      // these values remove DC offset
      FFTmathType sum = 0;
      for (int i = 0; i < samplesFFT; i++) {
        sum += valFFT[i];
      }
...
#else
      // run integer DSP FFT (takes ~x ms on ESP32, ~x ms on ESP32-S2, , ~1.5 ms on ESP32-C3) TODO: test and fill in
      // these values
      // apply window function to samples and fill buffer with interleaved complex values [Re,Im,Re,Im,...]
      for (int i = samplesFFT - 1; i >= 0; i--) {
        // fill the buffer back to front to avoid overwriting samples
      ...
      for (int i = 1; i < samplesFFT_2; i++) {  // skip [0], it is DC offset
        ...
        valFFT[i]         = sqrt32_bw(
            real_part * real_part +
            imag_part *
                imag_part);  // note: this should never overflow as Re and Im form a vector of maximum length 32767
    ...
    */                                    //  Range
    ...
    if ((audioSource == nullptr) ||
        (audioSource->getType() != AudioSource::Type_I2SAdc))  // the "delay trick" does not help for analog ADC
#endif
      vTaskDelayUntil(&xLastWakeTime, xFrequency);  // release CPU, and let I2S fill its buffers
...
static void runMicFilter(uint16_t numSamples, FFTsampleType* sampleBuffer)  // pre-filtering of raw samples (band-pass)
{
  ...
  // https://dsp.stackexchange.com/questions/40462/exponential-moving-average-cut-off-frequency (alpha = 2π × fc / fs)
  ...
  for (int i = 0; i < numSamples; i++) {
    ...
    if (i < (numSamples - 1)) {
      highFilteredSample_fp =
          (BETA1_FP * (int32_t)sampleBuffer[i] + BETA2_FP * last_vals[0] + BETA2_FP * (int32_t)sampleBuffer[i + 1]) >>
          16;  // smooth out spikes
    } else {
      highFilteredSample_fp =
          (BETA1_FP * (int32_t)sampleBuffer[i] + BETA2_FP * last_vals[0] + BETA2_FP * last_vals[1]) >>
          16;  // special handling for last sample in array
    }
    last_vals[1] = last_vals[0];
    last_vals[0] = (int32_t)sampleBuffer[i];
    lowfilt_fp +=
        ALPHA_FP * (highFilteredSample_fp - (lowfilt_fp >> 15));  // low pass filter in 17.15 fixed point format
...
static void detectSamplePeak(void) {
  bool havePeak = false;
  // softhack007: this code continuously triggers while amplitude in the selected bin is above a certain threshold. So
  // it does not detect peaks - it detects high activity in a frequency bin. Poor man's beat detection by seeing if
  // sample > Average + some value. This goes through ALL of the 255 bins - but ignores stupid settings Then we got a
  // peak, else we don't. The peak has to time out on its own in order to support UDP sound sync.
...
#define SR_DMTYPE 1    // default type = I2S
  ...
  struct __attribute__((packed)) audioSyncPacket {  // "packed" ensures that there are no additional gaps
    ...
    uint8_t samplePeak;  //  01 Bytes  offset 16 - 0 no peak; >=1 peak detected. In future, this will also provide peak
                         //  Magnitude
  ...
  // old "V1" audiosync struct - 83 Bytes payload, 88 bytes total (with padding added by compiler) - for backwards
  // compatibility
...
// set your config variables to their boot default value (this can also be done in readFromConfig() or a constructor if
// you prefer)
  ...
  int16_t sampleRaw =
      0;  // Current sample. Must only be updated ONCE!!! (amplified mic value by sampleGain and inputLevel)
...
#define CYCLE_SAMPLEMAX 3500            // time window for merasuring
  ...
  ////////////////////
  void logAudio() {
    if (disableSoundProcessing && (!udpSyncConnected || ((audioSyncEnabled & 0x02) == 0))) {
    ...
    //
    // Set true if wanting to see all the bands in their own vertical space on the Serial Plotter, false if wanting to
    // see values in Serial Monitor
    ...
    // if !scaleValuesFromCurrentMaxVal, we scale values from [0..defaultScalingFromHighValue] to
    // [0..scalingToHighValue], lower this if you want to see smaller values easier
    const int defaultScalingFromHighValue = 256;
    // Print values to terminal in range of [0..scalingToHighValue] if !mapValuesToPlotterSpace, or
    // [(i)*scalingToHighValue..(i+1)*scalingToHighValue] if mapValuesToPlotterSpace
    const int scalingToHighValue = 256;
    // set higher if using scaleValuesFromCurrentMaxVal and you want a small value that's also the current maxVal to
    // look small on the plotter (can't be 0 to avoid divide by zero error)
    ...
    for (int i = 0; i < NUM_GEQ_CHANNELS; i++) {
      PLOT_PRINT(i);
      PLOT_PRINT(":");
      PLOT_PRINTF("%04ld ", map(fftResult[i], 0, (scaleValuesFromCurrentMaxVal ? maxVal : defaultScalingFromHighValue),
    ...
    if (printMinVal) {
      PLOT_PRINTF("%04d:minVal ", minVal);  // printed with value first, then label, so negative values can be seen in
                                            // Serial Monitor but don't throw off y axis in Serial Plotter
    }
    if (mapValuesToPlotterSpace) {
      PLOT_PRINTF(
          "max:%04d ",
          (printMaxVal ? 17 : 16) *
              256);  // print line above the maximum value we expect to see on the plotter to avoid autoscaling y axis
    } else {
      ...
      if ((fabsf(sampleReal) < 2.0f) || (sampleMax < 1.0)) {
        ...
        if (tmpAgc <= agcTarget0Up[AGC_preset]) {
          multAgcTemp = agcTarget0[AGC_preset] /
                        sampleMax;  // Make the multiplier so that sampleMax * multiplier = first setpoint
        } else {
          multAgcTemp = agcTarget1[AGC_preset] /
                        sampleMax;  // Make the multiplier so that sampleMax * multiplier = second setpoint
        }
      }
      // limit amplification
  ...
  void getSample() {
    ...
    if (millis() - lastAnalogTime > 20) {
      micDataReal = analogRead(
          A0);  // read one sample with 10bit resolution. This is a dirty hack, supporting volumereactive effects only.
  ...
  void connectUDPSoundSync(void) {
    ...
    // necessary as we also want to transmit in "AP Mode", but the standard "connected()" callback only reacts on STA
    // connection
  ...
  void transmitAudioData() {
    ...
    memset(reinterpret_cast<void*>(&transmitData), 0,
           sizeof(transmitData));  // make sure that the packet - including "invisible" padding bytes added by the
  ...
  void decodeAudioData(int packetSize, uint8_t* fftBuff) {
    ...
    memcpy(
        ...
        min((unsigned)packetSize, (unsigned)sizeof(receivedPacket)));  // don't violate alignment - thanks @willmmiles#
    ...
    FFT_MajorPeak =
        constrain(receivedPacket.FFT_MajorPeak, 1.0f, 11025.0f);  // restrict value to range expected by effects
  }
  ...
  void decodeAudioData_v1(int packetSize, uint8_t* fftBuff) {
    ...
    FFT_Magnitude = my_magnitude;
    FFT_MajorPeak =
        constrain(receivedPacket->FFT_MajorPeak, 1.0, 11025.0);  // restrict value to range expected by effects
  }
  ...
  bool receiveAudioData()  // check & process new data. return TRUE in case that new audio data was received.
  {
    ...
    if ((packetSize > 5) && (packetSize <= UDPSOUND_MAX_PACKET)) {
      ...
      uint8_t fftBuff[UDPSOUND_MAX_PACKET + 1] = {
          0};  // fixed-size buffer for receiving (stack), to avoid heap fragmentation caused by variable sized arrays
 ...
 public:
  ...
  void setup() override {
    ...
    if (!initDone) {
      ...
      // we will assign all usermod exportable data here as pointers to original variables or arrays and allocate memory
      // for pointers
      ...
      um_data->u_data[4] = &FFT_MajorPeak;  //*used (Ripplepeak, Freqmap, Freqmatrix, Freqpixels, Freqwave, Gravfreq,
                                            // Rocktaves, Waterfall)
      um_data->u_type[4] = UMT_FLOAT;
      um_data->u_data[5] = &my_magnitude;  // used (New)
      um_data->u_type[5] = UMT_FLOAT;
      um_data->u_data[6] =
          &maxVol;  // assigned in effect function from UI element!!! (Puddlepeak, Ripplepeak, Waterfall)
      um_data->u_type[6] = UMT_BYTE;
      um_data->u_data[7] =
          &binNum;  // assigned in effect function from UI element!!! (Puddlepeak, Ripplepeak, Waterfall)
    ...
    useBandPassFilter =
        false;  // filter cuts lowest and highest frequency bands from FFT result (use on very noisy mic inputs)
    useMicFilter = true;  // filter fixes aliasing to base & highest frequency bands and reduces noise floor
                          // (recommended for all mic inputs)

#if !defined(CONFIG_IDF_TARGET_ESP32S2) && !defined(CONFIG_IDF_TARGET_ESP32C3)
    if ((i2sckPin == I2S_PIN_NO_CHANGE) && (i2ssdPin >= 0) && (i2swsPin >= 0) && ((dmType == 1) || (dmType == 4))) {
    ...
    switch (dmType) {
...
#endif
      case 1:
        DEBUGSR_PRINT("AR: Generic I2S Microphone - ");
        DEBUGSR_PRINTLN(I2S_MIC_CHANNEL_TEXT);
      ...
      case 2:
        ...
        break;
      case 3:
        DEBUGSR_PRINT("AR: SPH0645 Microphone - ");
        DEBUGSR_PRINTLN(I2S_MIC_CHANNEL_TEXT);
        ...
        break;
      case 4:
        DEBUGSR_PRINT("AR: Generic I2S Microphone with Master Clock - ");
        DEBUGSR_PRINTLN(I2S_MIC_CHANNEL_TEXT);
...
#if !defined(CONFIG_IDF_TARGET_ESP32S2) && !defined(CONFIG_IDF_TARGET_ESP32C3)
      case 5:
        DEBUGSR_PRINT("AR: Generic PDM Microphone - ");
        DEBUGSR_PRINTLN(I2S_PDM_MIC_CHANNEL_TEXT);
    ...
    if ((!audioSource) ||
        (!audioSource->isInitialized())) {  // audio source failed to initialize. Still stay "enabled", as there might
                                            // be input arriving via UDP Sound Sync
#ifdef WLED_DEBUG
      DEBUG_PRINTLN("AR: Failed to initialize sound input driver. Please check input PIN settings.");
#else
      DEBUGSR_PRINTLN("AR: Failed to initialize sound input driver. Please check input PIN settings.");
#endif
      disableSoundProcessing = true;
    }
#endif
  ...
  void loop() override {
    ...
    if ((realtimeOverride == REALTIME_OVERRIDE_NONE)  // please add other overrides here if needed
        && ((realtimeMode == REALTIME_MODE_GENERIC) || (realtimeMode == REALTIME_MODE_E131) ||
            (realtimeMode == REALTIME_MODE_UDP) || (realtimeMode == REALTIME_MODE_ADALIGHT) ||
            (realtimeMode == REALTIME_MODE_ARTNET)))  // please add other modes here if needed
    {
      ...
      if ((disableSoundProcessing == false) && (audioSyncEnabled == 0)) {  // we just switched to "disabled"
        ...
        DEBUG_PRINTF_P("               RealtimeMode = %d; RealtimeOverride = %d\n", int(realtimeMode),
                       int(realtimeOverride));
      }
#endif
      disableSoundProcessing = true;
    } else {
#if defined(ARDUINO_ARCH_ESP32) && defined(WLED_DEBUG)
      if ((disableSoundProcessing == true) && (audioSyncEnabled == 0) && audioSource &&
...
#ifdef WLED_DEBUG
      // complain when audio userloop has been delayed for long time. Currently we need userloop running between 500 and
      // 1500 times per second. softhack007 disabled temporarily - avoid serial console spam with MANY leds and low FPS
    ...
    if ((audioSyncEnabled & 0x02) && udpSyncConnected) {
      ...
      if (millis() - lastTime > delayMs) {
        ...
        if (have_new_sample) {
          ...
          fftUdp.flush();  // Flush udp input buffers if we haven't read it - avoids hickups in receive mode. Does not
                           // work on 8266.
        }
#endif
        lastTime = millis();
    ...
    if ((millis() - sampleMaxTimer) > CYCLE_SAMPLEMAX) {
      ...
      maxSample5sec  = (0.15f * maxSample5sec) +
                       0.85f * ((soundAgc) ? sampleAgc : sampleAvg);  // reset, and start with some smoothing
      if (sampleAvg < 1) {
        maxSample5sec = 0;  // noise gate
      }
    } else {
      if ((sampleAvg >= 1)) {
        maxSample5sec = fmaxf(maxSample5sec, (soundAgc) ? rawSampleAgc : sampleRaw);  // follow maximum volume
      }
    }
    ...
    if ((millis() - sampleMaxTimer) > CYCLE_SAMPLEMAX) {
      ...
      if (volumeSmth >= 1.0f) {
        maxSample5sec = fmaxf(maxSample5sec, volumeRaw);  // follow maximum volume
      }
    }
#endif
  ...
  void onUpdateBegin(bool init) {
    ...
    if (enabled) {
      disableSoundProcessing = init;  // init = true means that OTA is just starting --> don't process audio
    }
    updateIsRunning = init;
  }
#endif
  ...
  void addToJsonInfo(JsonObject& root) override {
    ...
    if (enabled) {
      ...
      if (disableSoundProcessing == false) {  // only show slider when audio processing is running
        if (soundAgc > 0) {
          infoArr = user.createNestedArray(
              "GEQ Input Level");  // if AGC is on, this slider only affects fftResult[] frequencies
        } else {
          infoArr = user.createNestedArray("Audio Input Level");
        }
        uiDomString =
            "<div class=\"slider\"><div class=\"sliderwrap il\"><input class=\"noslide\" onchange=\"requestJson({";
        uiDomString += _name;
        uiDomString += ":{";
      ...
      if (audioSyncEnabled & 0x02) {
        ...
        if (audioSource && (audioSource->isInitialized())) {
          ...
          if (audioSource->getType() == AudioSource::Type_I2SAdc) {
            ...
            if (dmType == 5) {
              infoArr.add("PDM digital");  // dmType 5 => generic PDM microphone
            } else {
      ...
      if ((soundAgc == 0) && (disableSoundProcessing == false) && !(audioSyncEnabled & 0x02)) {
        ...
        float myGain =
            ((float)sampleGain / 40.0f * (float)inputLevel / 128.0f) + 1.0f / 16.0f;  // non-AGC gain from presets
  ...
  void readFromJsonState(JsonObject& root) override {
    ...
    if (palettes > 0 && root.containsKey("rmcpal")) {
   ...
   *
   * I highly recommend checking out the basics of ArduinoJson serialization and deserialization in order to use custom
   * settings!
   */
   ...
   * assignments, buffer sizes), but also that if you want to write persistent values to a dynamic buffer, you'd need to
   * allocate it here instead of in setup. If you don't know what that is, don't fret. It most likely doesn't affect
   * your use case :)
   *
   * Return true in case the config values returned from Usermod Settings were complete, or false if you'd like WLED to
   * save your defaults to disk (so any missing values are editable in Usermod Settings)
   *
   * getJsonValue() returns false if the value is missing, or copies the value into the variable provided and returns
   * true if the value is present The configComplete variable is true only if the "exampleUsermod" object and all values
   * are present.  If any values are missing, WLED will know to call addToConfig() to save them
   *
  ...
  bool readFromConfig(JsonObject& root) override {
    ...
    if (initDone) {
      ...
      if ((addPalettes && !oldAddPalettes && enabled) || (addPalettes && !oldEnabled && enabled)) {
        createAudioPalettes();
      }
  ...
  void appendConfigData(Print& uiScript) override {
    uiScript.print("ux='AudioReactive';");  // ux = shortcut for Audioreactive - fingers crossed that "ux" isn't already
                                            // used as JS var, html post parameter or css style
#ifdef ARDUINO_ARCH_ESP32
...
#ifdef ARDUINO_ARCH_ESP32
    uiScript.print(
        "addInfo(ux+':digitalmic:type',1,'<i>requires reboot!</i>');");  // 0 is field type, 1 is actual field
...
// strings to reduce flash memory usage (used more than twice)
const char AudioReactive::_name[]      = "AudioReactive";
const char AudioReactive::_enabled[]   = "enabled";
const char AudioReactive::_config[]    = "config";
const char AudioReactive::_dynamics[]  = "dynamics";
const char AudioReactive::_frequency[] = "frequency";
const char AudioReactive::_inputLvl[]  = "inputLevel";
#if defined(ARDUINO_ARCH_ESP32) && !defined(CONFIG_IDF_TARGET_ESP32S2) && !defined(CONFIG_IDF_TARGET_ESP32C3) && \
    !defined(CONFIG_IDF_TARGET_ESP32S3)
const char AudioReactive::_analogmic[] = "analogmic";
#endif
const char AudioReactive::_digitalmic[]  = "digitalmic";
const char AudioReactive::_addPalettes[] = "add-palettes";
const char AudioReactive::UDP_SYNC_HEADER[] =
    "00002";  // new sync header version, as format no longer compatible with previous structure
const char AudioReactive::UDP_SYNC_HEADER_v1[] =
    "00001";  // old sync header version - need to add backwards-compatibility feature
