...
// see
// https://docs.espressif.com/projects/esp-idf/en/latest/esp32s3/hw-reference/chip-series-comparison.html#related-documents
// and
// https://docs.espressif.com/projects/esp-idf/en/latest/esp32s3/api-reference/peripherals/i2s.html#overview-of-all-modes
...
// * no floating point hardware (FPU) support - FFT uses float calculations. If done in software, a strong slow-down can
// be expected (between 8x and 20x)
...
// #define I2S_GRAB_ADC1_COMPLETELY // (experimental) continuously sample analog ADC microphone. WARNING will cause
// analogRead() lock-up

// data type requested from the I2S driver - currently we always use 32bit
// #define I2S_USE_16BIT_SAMPLES   // (experimental) define this to request 16bit - more efficient but possibly less
// compatible
 ...
 * I2S_CHANNEL_FMT_RIGHT_LEFT, I2S_CHANNEL_FMT_ALL_RIGHT and I2S_CHANNEL_FMT_ALL_LEFT stands for stereo mode, which
 * means two channels will transport different data. I2S_CHANNEL_FMT_ONLY_RIGHT and I2S_CHANNEL_FMT_ONLY_LEFT they are
 * mono mode, both channels will only transport same data. I2S_CHANNEL_FMT_MULTIPLE means TDM channels, up to 16 channel
 * will available, and they are stereo as default. if you want to receive two channels, one is the actual data from
 * microphone and another channel is suppose to receive 0, it's different data in two channels, you need to choose
 * I2S_CHANNEL_FMT_RIGHT_LEFT in this case.
 */
...
class AudioSource {
  ...
  */
  virtual void initialize(int8_t = I2S_PIN_NO_CHANGE, int8_t = I2S_PIN_NO_CHANGE, int8_t = I2S_PIN_NO_CHANGE,
                          int8_t = I2S_PIN_NO_CHANGE) = 0;
  ...
  */
  ...
  virtual AudioSourceType getType(void) {
    return (Type_I2SDigital);
  }  // default is "I2S digital source" - ADC type overrides this method

 protected:
  /* Post-process audio sample - currently on needed for I2SAdcSource*/
  virtual I2S_datatype postProcessSample(I2S_datatype sample_in) {
    return (sample_in);
  }  // default method can be overriden by instances (ADC) that need sample postprocessing
  ...
  AudioSource(SRate_t sampleRate, int blockSize, float sampleScale)
      : _sampleRate(sampleRate), _blockSize(blockSize), _initialized(false), _sampleScale(sampleScale) {};
...
class I2SSource : public AudioSource {
 public:
  ...
  virtual void initialize(int8_t i2swsPin = I2S_PIN_NO_CHANGE, int8_t i2ssdPin = I2S_PIN_NO_CHANGE,
                          int8_t i2sckPin = I2S_PIN_NO_CHANGE, int8_t mclkPin = I2S_PIN_NO_CHANGE) {
...
#warning this MCU does not support PDM microphones
      ...
      // https://github.com/espressif/esp-idf/blob/release/v4.4/examples/peripherals/i2s/i2s_audio_recorder_sdcard/main/i2s_recorder_main.c
      ...
      //  * C3: does not support PDM to PCM input. SoC would allow PDM RX, but there is no hardware to directly convert
      //  to PCM so it will not work. https://github.com/espressif/esp-idf/issues/8796

      _config.mode = i2s_mode_t(I2S_MODE_MASTER | I2S_MODE_RX |
                                I2S_MODE_PDM);  // Change mode to pdm if clock pin not provided. PDM is not supported on
                                                // ESP32-S2. PDM RX not supported on ESP32-C3
...
#if !defined(WLED_USE_ETHERNET)  // fix for #5391 aPLL resource conflict - aPLL is needed for ethernet boards with
                                 // internal RMII clock
      _config.use_apll =
          true;  // experimental - use aPLL clock source to improve sampling quality, and to avoid glitches.
...
#warning this MCU does not have an APLL high accuracy clock for audio
...
#endif
#if defined(ARDUINO_ARCH_ESP32) && !defined(CONFIG_IDF_TARGET_ESP32S3) && !defined(CONFIG_IDF_TARGET_ESP32S2) && \
    !defined(CONFIG_IDF_TARGET_ESP32C3)
    if (ESP.getChipRevision() == 0) {
      _config.use_apll = false;  // APLL is broken on ESP32 revision 0
    }
#endif
#endif
    ...
    _pinConfig = {
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(4, 4, 0)
        .mck_io_num = mclkPin,  // "classic" ESP32 supports setting MCK on GPIO0/GPIO1/GPIO3 only. i2s_set_pin() will
                                // fail if wrong mck_io_num is provided.
#endif
        .bck_io_num   = i2sckPin,
        .ws_io_num    = i2swsPin,
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(4, 2, 0)
    err = i2s_set_clk(I2S_NUM_0, _sampleRate, I2S_SAMPLE_RESOLUTION,
                      I2S_CHANNEL_MONO);  // set bit clocks. Also takes care of MCLK routing if needed.
    if (err != ESP_OK) {
  ...
  virtual void deinitialize() {
    ...
    if (_pinConfig.ws_io_num != I2S_PIN_NO_CHANGE) {
      PinManager::deallocatePin(_pinConfig.ws_io_num, PinOwner::UM_Audioreactive);
    }
    if (_pinConfig.data_in_num != I2S_PIN_NO_CHANGE) {
      PinManager::deallocatePin(_pinConfig.data_in_num, PinOwner::UM_Audioreactive);
    }
    if (_pinConfig.bck_io_num != I2S_PIN_NO_CHANGE) {
      PinManager::deallocatePin(_pinConfig.bck_io_num, PinOwner::UM_Audioreactive);
    }
    // Release the master clock pin
    if (_mclkPin != I2S_PIN_NO_CHANGE) {
  ...
  virtual void getSamples(FFTsampleType *buffer, uint16_t num_samples) {
    if (_initialized) {
      ...
      // constexpr int32_t FIXEDSHIFT = 8; // shift by 8 bits for fixed point math (no loss at 24bit input sample
      // resolution) int32_t intSampleScale = _sampleScale * (1<<FIXEDSHIFT); // _sampleScale <= 1.0f, shift for fixed
      // point math
#endif
      ...
      for (int i = 0; i < num_samples; i++) {
        ...
        float currSample =
            (float)newSamples[i] / 65536.0f;  // 32bit input -> 16bit; keeping lower 16bits as decimal places
#else
...
#else
#ifdef I2S_SAMPLE_DOWNSCALE_TO_16BIT
        // note on sample scaling: scaling is only used for inputs with master clock and those are better suited for
        // ESP32 or S3 execution speed is critical on single core MCUs
        ...
        int16_t currSample =
            newSamples[i] >> 16;  // no sample scaling, just shift down to 16bit (not scaling saves ~0.4ms on C3)
#else
        // int32_t currSample = (newSamples[i] * intSampleScale) >> FIXEDSHIFT;   // scale samples, shift back down to
        // 16bit
...
#define ES7243_ADDR 0x13  // default address
  ...
  void _es7243InitAdc() {
    ...
    _es7243I2cWrite(
        0x01,
        0x00);  // 0x00 for 24 bit to match INMP441 - not sure if this needs adjustment to get 16bit samples from I2S
    _es7243I2cWrite(0x08, 0x43);
    _es7243I2cWrite(0x05, 0x13);
  }

 public:
  ES7243(SRate_t sampleRate, int blockSize, float sampleScale = 1.0f) : I2SSource(sampleRate, blockSize, sampleScale) {
...
class I2SAdcSource : public I2SSource {
 public:
  I2SAdcSource(SRate_t sampleRate, int blockSize, float sampleScale = 1.0f)
      : I2SSource(sampleRate, blockSize, sampleScale) {
    _config = {.mode            = i2s_mode_t(I2S_MODE_MASTER | I2S_MODE_RX | I2S_MODE_ADC_BUILT_IN),
               .sample_rate     = _sampleRate,
               .bits_per_sample = I2S_SAMPLE_RESOLUTION,
...
#else
                                                                          // bugfix: do not disable ADC initially - its
                                                                          // already disabled after driver install.
  ...
  I2S_datatype postProcessSample(I2S_datatype sample_in) {
    ...
    I2S_unsigned_datatype rawData =
        *reinterpret_cast<I2S_unsigned_datatype *>(&sample_in);  // C++ acrobatics to get sample as "unsigned"
#ifndef I2S_USE_16BIT_SAMPLES
    rawData = (rawData >> 16) & 0xFFFF;  // scale input down from 32bit -> 16bit
    I2S_datatype lastGoodSample =
        lastADCsample / 16384;  // prepare "last good sample" accordingly (26bit-> 12bit with correct sign handling)
#else
    ...
    if ((the_channel != _myADCchannel) && (_myADCchannel != 0x0F)) {  // 0x0F means "don't know what my channel is"
      ...
      // Serial.print("\n!ADC rogue sample 0x"); Serial.print(rawData, HEX);
      // Serial.print("\tchannel:");Serial.println(the_channel);
    } else {
...
#if !defined(I2S_GRAB_ADC1_COMPLETELY)
      // old code - works for me without enable/disable, at least on ESP32.
      err = i2s_adc_disable(I2S_NUM_0);  // i2s_adc_disable() may cause crash with IDF 4.4
                                         // (https://github.com/espressif/arduino-esp32/issues/6832)
      // err = i2s_stop(I2S_NUM_0);
      if (err != ESP_OK) {
...
// https://github.com/espressif/esp-idf/issues/7192  SPH0645 i2s microphone issue when migrate from legacy esp-idf
// version (IDFGH-5453) a user recommended this: Try to set .communication_format to I2S_COMM_FORMAT_STAND_I2S and call
// i2s_set_clk() after i2s_set_pin().
class SPH0654 : public I2SSource {
 public:
  SPH0654(SRate_t sampleRate, int blockSize, float sampleScale = 1.0f) : I2SSource(sampleRate, blockSize, sampleScale) {
  }
