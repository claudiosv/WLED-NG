...
#define ENCODER_MAX_DELAY_MS \
  8  // 8 milliseconds => max 120 change impulses in 1 second, for full turn of a 30/30 encoder (4 changes per segment,
     // 30 segments for one turn)
#endif
...
void RotaryEncoderUIUsermod::sortModesAndPalettes() {
  DEBUG_PRINT("Sorting modes: ");
  DEBUG_PRINTLN(strip.getModeCount());
  // modes_qstrings = re_findModeStrings(JSON_mode_names, strip.getModeCount());
  modes_qstrings      = strip.getModeDataSrc();
  modes_alpha_indexes = re_initIndexArray(strip.getModeCount());
  re_sortModes(modes_qstrings, modes_alpha_indexes, strip.getModeCount(), MODE_SORT_SKIP_COUNT);

  DEBUG_PRINT("Sorting palettes: ");
  DEBUG_PRINT(getPaletteCount());
  DEBUG_PRINT('/');
  DEBUG_PRINTLN(customPalettes.size());
  palettes_qstrings =
      re_findModeStrings(JSON_palette_names, getPaletteCount());  // allocates memory for all palette names
  ...
  while (pgm_read_byte_near(palettes_qstrings[skipPaletteCount]) == '*') {
    skipPaletteCount++;  // legacy code
  }
  re_sortModes(palettes_qstrings, palettes_alpha_indexes, FIXED_PALETTE_COUNT,
               skipPaletteCount);  // only sort fixed palettes (skip dynamic)
}
...
void RotaryEncoderUIUsermod::loop() {
  ...
  if (strip.isUpdating() && ((currentTime - loopTime) < ENCODER_MAX_DELAY_MS)) {
  ...
  if (currentTime >= (loopTime + 2))  // 2ms since last check of encoder = 500Hz
  {
    ...
    if (buttonWaitTime && currentTime - buttonWaitTime > 350 && !buttonPressedBefore) {  // same speed as in button.cpp
      ...
      do {
        ...
        switch (newState) {
          case 0:
            strcpy(lineBuffer, "Brightness");
            changedState = true;
            break;
          case 1:
            if (!extractModeSlider(effectCurrent, 0, lineBuffer, 63)) {
              newState++;
            } else {
              changedState = true;
            }
            break;  // speed
          case 2:
            if (!extractModeSlider(effectCurrent, 1, lineBuffer, 63)) {
              newState++;
            } else {
              changedState = true;
            }
            break;  // intensity
          case 3:
            strcpy(lineBuffer, "Color Palette");
            changedState = true;
            break;
          case 4:
            strcpy(lineBuffer, "Effect");
            changedState = true;
            break;
          case 5:
            strcpy(lineBuffer, "Main Color");
            changedState = true;
            break;
          case 6:
            strcpy(lineBuffer, "Saturation");
            changedState = true;
            break;
          case 7:
            if (!(strip.getSegment(applyToAll ? strip.getFirstSelectedSegId() : strip.getMainSegmentId())
                      .getLightCapabilities() &
                  0x04)) {
              newState++;
            } else {
              strcpy(lineBuffer, "CCT");
              changedState = true;
            }
            break;
          case 8:
            if (presetHigh == 0 || presetLow == 0) {
              newState++;
            } else {
              strcpy(lineBuffer, "Preset");
              changedState = true;
            }
            break;
          case 9:
          case 10:
          case 11:
            if (!extractModeSlider(effectCurrent, newState - 7, lineBuffer, 63)) {
              newState++;
            } else {
              changedState = true;
            }
      ...
      if (display != nullptr) {
        switch (newState) {
          case 0:
            changedState = changeState(lineBuffer, 1, 0, 1);
            break;  // 1  = sun
          case 1:
            changedState = changeState(lineBuffer, 1, 4, 2);
            break;  // 2  = skip forward
          case 2:
            changedState = changeState(lineBuffer, 1, 8, 3);
            break;  // 3  = fire
          case 3:
            changedState = changeState(lineBuffer, 2, 0, 4);
            break;  // 4  = custom palette
          case 4:
            changedState = changeState(lineBuffer, 3, 0, 5);
            break;  // 5  = puzzle piece
          case 5:
            changedState = changeState(lineBuffer, 255, 255, 7);
            break;  // 7  = brush
          case 6:
            changedState = changeState(lineBuffer, 255, 255, 8);
            break;  // 8  = contrast
          case 7:
            changedState = changeState(lineBuffer, 255, 255, 10);
            break;  // 10 = star
          case 8:
            changedState = changeState(lineBuffer, 255, 255, 11);
            break;  // 11 = heart
          case 9:
            changedState = changeState(lineBuffer, 255, 255, 10);
            break;  // 10 = star
    ...
    if ((Enc_A) && (!Enc_A_prev)) {  // A has gone from high to low
      ...
      } else if (Enc_B == HIGH) {  // B is low so counter-clockwise
        switch (select_state) {
          case 0:
          ...
          case 8:
            ...
            break;
          case 9:
            changeCustom(1, false);
            break;
          case 10:
            changeCustom(2, false);
            break;
          case 11:
            changeCustom(3, false);
            break;
        }
      }
    }
...
void RotaryEncoderUIUsermod::changeBrightness(bool increase) {
  ...
  if (bri < 40) {
    bri = max(min((increase ? bri + fadeAmount / 2 : bri - fadeAmount / 2), 255),
              0);  // slower steps when brightness < 16%
  } else {
    bri = max(min((increase ? bri + fadeAmount : bri - fadeAmount), 255), 0);
  }
  lampUdated();
...
#endif
  effectCurrentIndex =
      max(min((increase ? effectCurrentIndex + 1 : effectCurrentIndex - 1), strip.getModeCount() - 1), 0);
...
void RotaryEncoderUIUsermod::changeCustom(uint8_t par, bool increase) {
  ...
  if (applyToAll) {
    ...
    switch (par) {
      case 3:
        val = sid.custom3 = max(min((increase ? sid.custom3 + fadeAmount : sid.custom3 - fadeAmount), 255), 0);
        break;
      case 2:
        val = sid.custom2 = max(min((increase ? sid.custom2 + fadeAmount : sid.custom2 - fadeAmount), 255), 0);
        break;
      default:
        val = sid.custom1 = max(min((increase ? sid.custom1 + fadeAmount : sid.custom1 - fadeAmount), 255), 0);
        break;
    }
    for (unsigned i = 0; i < strip.getSegmentsNum(); i++) {
      Segment &seg = strip.getSegment(i);
      if (!seg.isActive() || i == id) {
        continue;
      }
      switch (par) {
        case 3:
          seg.custom3 = sid.custom3;
          break;
        case 2:
          seg.custom2 = sid.custom2;
          break;
        default:
          seg.custom1 = sid.custom1;
          break;
      }
    }
  } else {
    ...
    switch (par) {
      case 3:
        val = seg.custom3 = max(min((increase ? seg.custom3 + fadeAmount : seg.custom3 - fadeAmount), 255), 0);
        break;
      case 2:
        val = seg.custom2 = max(min((increase ? seg.custom2 + fadeAmount : seg.custom2 - fadeAmount), 255), 0);
        break;
      default:
        val = seg.custom1 = max(min((increase ? seg.custom1 + fadeAmount : seg.custom1 - fadeAmount), 255), 0);
        break;
    }
  }
  lampUdated();
...
void RotaryEncoderUIUsermod::changePalette(bool increase) {
  ...
  display->updateRedrawTime();
#endif
  effectPaletteIndex = max(min((unsigned)(increase ? effectPaletteIndex + 1 : effectPaletteIndex - 1),
                               getPaletteCount() + customPalettes.size() - 1),
                           0U);
...
void RotaryEncoderUIUsermod::appendConfigData() {
  ...
  oappend(
      "d.extra.push({'Rotary-Encoder':{pin:[['P0',100],['P1',101],['P2',102],['P3',103],['P4',104],['P5',105],['P6',"
      "106],['P7',107]]}});");
}
...
// strings to reduce flash memory usage (used more than twice)
const char RotaryEncoderUIUsermod::_name[]       = "Rotary-Encoder";
const char RotaryEncoderUIUsermod::_enabled[]    = "enabled";
const char RotaryEncoderUIUsermod::_DT_pin[]     = "DT-pin";
const char RotaryEncoderUIUsermod::_CLK_pin[]    = "CLK-pin";
const char RotaryEncoderUIUsermod::_SW_pin[]     = "SW-pin";
const char RotaryEncoderUIUsermod::_presetHigh[] = "preset-high";
const char RotaryEncoderUIUsermod::_presetLow[]  = "preset-low";
const char RotaryEncoderUIUsermod::_applyToAll[] = "apply-2-all-seg";
const char RotaryEncoderUIUsermod::_pcf8574[]    = "use-PCF8574";
const char RotaryEncoderUIUsermod::_pcfAddress[] = "PCF8574-address";
const char RotaryEncoderUIUsermod::_pcfINTpin[]  = "PCF8574-INT-pin";
