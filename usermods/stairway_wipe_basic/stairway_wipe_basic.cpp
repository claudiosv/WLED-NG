 ...
 * 2. Register the usermod by adding #include "stairway-wipe-usermod-v2.h" in the top and registerUsermod(new
 * StairwayWipeUsermod()) in the bottom of usermods_list.cpp
 */
...
class StairwayWipeUsermod : public Usermod {
 ...
 public:
  ...
  void loop() {
    ...
    // can be set to 0 if no movement is detected. Otherwise LEDs will turn off after a configurable timeout (userVar1
    // seconds)
    ...
    if (userVar0 > 0) {
      ...
      if (wipeState == 0) {
        ...
        uint32_t cycleTime =
            360 +
            (255 - effectSpeed) * 75;  // this is how long one wipe takes (minus 25 ms to make sure we switch in time)
