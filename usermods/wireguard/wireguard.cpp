...
class WireguardUsermod : public Usermod {
 public:
  void setup() {
    configTzTime(posix_tz, ntpServerName);
  }
  ...
  bool readFromConfig(JsonObject& root) {
    ...
    if (top["host"].isNull() || top["port"].isNull() || top["ip"].isNull() || top["pem"].isNull() ||
        top["pub"].isNull() || top["tz"].isNull()) {
      is_enabled = false;
      return false;
    } else {
