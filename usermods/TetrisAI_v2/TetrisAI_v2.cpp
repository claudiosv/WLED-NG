    ...
    for (auto index_x = 0; index_x < tetris->grid.width; index_x++) {
      ...
      if (isRowClearing) {
        if (noFlashOnClear) {
          ...
          color = (strip.now % 200) < 150 ? CRGB::Gray : CRGB::Black;
        }
      } else if (gridPixel == 0) {
        // BG color
        color = SEGCOLOR(1);
  ...
  if (tetrisai_data->showNext) {
    ...
    if (tetrisai_data->showBorder) {
      ...
      for (auto index_y = 0; index_y < tetrisai_data->effectHeight; index_y++) {
        SEGMENT.setPixelColorXY(tetrisai_data->segOffsetX + tetrisai_data->effectWidth - 6,
                                tetrisai_data->segOffsetY + index_y, SEGCOLOR(2));
      }
    }
      ...
      for (uint8_t pieceY = 0; pieceY < piece.getRotation().height; pieceY++) {
        for (uint8_t pieceX = 0; pieceX < piece.getRotation().width; pieceX++) {
          if (piece.getPixel(pieceX, pieceY)) {
            ...
            SEGMENT.setPixelColorXY(tetrisai_data->segOffsetX + piecesOffsetX + pieceX,
                                    tetrisai_data->segOffsetY + piecesOffsetY + pieceNbrOffsetY + pieceY,
                                    ColorFromPalette(SEGPALETTE, colIdx, 255, NOBLEND));
          }
        }
      }
    }
...
void mode_2DTetrisAI() {
  ...
  if (tetrisai_data->tetris.nLookAhead != nLookAhead || tetrisai_data->segcols != cols ||
      tetrisai_data->segrows != rows || tetrisai_data->showNext != SEGMENT.check1 ||
...
static const char _data_FX_MODE_2DTETRISAI[] =
    "Tetris AI@!,Look ahead,Intelligence,Rotate color,Mistake free,Show next,Border,Mistakes;Game "
    "Over,!,Border;!;2;sx=127,ix=64,c1=255,c2=0,c3=31,o1=1,o2=1,o3=0,pal=11";
