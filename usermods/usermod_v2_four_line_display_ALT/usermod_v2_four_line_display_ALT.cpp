...
const char FourLineDisplayUsermod::_name[]            = "4LineDisplay";
const char FourLineDisplayUsermod::_enabled[]         = "enabled";
const char FourLineDisplayUsermod::_contrast[]        = "contrast";
const char FourLineDisplayUsermod::_refreshRate[]     = "refreshRate-ms";
const char FourLineDisplayUsermod::_screenTimeOut[]   = "screenTimeOutSec";
const char FourLineDisplayUsermod::_flip[]            = "flip";
const char FourLineDisplayUsermod::_sleepMode[]       = "sleepMode";
const char FourLineDisplayUsermod::_clockMode[]       = "clockMode";
const char FourLineDisplayUsermod::_showSeconds[]     = "showSeconds";
const char FourLineDisplayUsermod::_busClkFrequency[] = "i2c-freq-kHz";
const char FourLineDisplayUsermod::_contrastFix[]     = "contrastFix";
...
void FourLineDisplayUsermod::startDisplay() {
  ...
  setContrast(contrast);  // Contrast setup will help to preserve OLED lifetime. In case OLED need to be brighter
...
void FourLineDisplayUsermod::center(String &line, uint8_t width) {
  int len = line.length();
  if (len < width) {
    for (unsigned i = (width - len) / 2; i > 0; i--) {
      line = ' ' + line;
    }
  }
  for (unsigned i = line.length(); i < width; i++) {
    line += ' ';
  }
}
...
void FourLineDisplayUsermod::setup() {
  ...
  switch (type) {
    ...
    case SSD1305_64:
      ...
      break;
    case SSD1309_64:
      u8x8 = (U8X8 *)new U8X8_SSD1309_128X64_NONAME0_HW_I2C();
      break;
...
void FourLineDisplayUsermod::showCurrentEffectOrPalette(int inputEffPal, const char *qstring, uint8_t row) {
  ...
  if (overlayUntil == 0) {
    ...
    if (lineHeight == 2) {  // use this code for 8 line display
      ...
      for (unsigned i = 0; i < MAX_MODE_LINE_SPACE; i++) {
...
void FourLineDisplayUsermod::overlayLogo(long showHowLong) {
  ...
  if (lineHeight == 2) {
    ...
    switch (millis() % 3) {
      ...
      case 2:
        // Akemi
        // draw2x2Glyph( 5, 0, 12, u8x8_4LineDisplay_WLED_icons_3x3); // use this if flash runs short and comment out
        // 6x6 font
    ...
    if (Display_Task) {
      ...
      xTaskCreatePinnedToCore(
          [](void *par) {  // Function to implement the task
            ...
            for (;;) {
              delay(1);  // DO NOT DELETE THIS LINE! It is needed to give the IDLE(0) task enough time and to keep the
                         // watchdog happy. taskYIELD(), yield(), vTaskDelay() and esp_task_wdt_feed() didn't seem to
                         // work.
 ...
 * allocate it here instead of in setup. If you don't know what that is, don't fret. It most likely doesn't affect your
 * use case :)
 */
    ...
    if (pinsChanged || type != newType) {
      ...
      switch (type) {
        case SSD1306:
          ...
          break;
        case SH1106:
          u8x8_Setup(u8x8->getU8x8(), u8x8_d_sh1106_128x64_winstar, u8x8_cad_ssd13xx_fast_i2c, u8x8_byte_arduino_hw_i2c,
                     u8x8_gpio_and_delay_arduino);
          u8x8_SetPin_HW_I2C(u8x8->getU8x8(), U8X8_PIN_NONE, U8X8_PIN_NONE, U8X8_PIN_NONE);
          break;
        case SSD1306_64:
          u8x8_Setup(u8x8->getU8x8(), u8x8_d_ssd1306_128x64_noname, u8x8_cad_ssd13xx_fast_i2c, u8x8_byte_arduino_hw_i2c,
                     u8x8_gpio_and_delay_arduino);
          u8x8_SetPin_HW_I2C(u8x8->getU8x8(), U8X8_PIN_NONE, U8X8_PIN_NONE, U8X8_PIN_NONE);
          break;
        case SSD1305:
        ...
        case SSD1305_64:
          ...
          break;
        case SSD1309_64:
          u8x8_Setup(u8x8->getU8x8(), u8x8_d_ssd1309_128x64_noname0, u8x8_cad_ssd13xx_fast_i2c,
                     u8x8_byte_arduino_hw_i2c, u8x8_gpio_and_delay_arduino);
          u8x8_SetPin_HW_I2C(u8x8->getU8x8(), U8X8_PIN_NONE, U8X8_PIN_NONE, U8X8_PIN_NONE);
          break;
        case SSD1306_SPI:
