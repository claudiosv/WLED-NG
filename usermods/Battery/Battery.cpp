...
class UsermodBattery : public Usermod {
 private:
  ...
  /**
   * Indicate low power by activating a configured preset for a given time and then switching back to the preset that
   * was selected previously
   */
  void lowPowerIndicator() {
    if (!lowPowerIndicatorEnabled) {
      return;
    }
    if (batteryPin < 0) {
      return;  // no measurement
    }
    if (lowPowerIndicationDone && lowPowerIndicatorReactivationThreshold <= bat->getLevel()) {
      lowPowerIndicationDone = false;
    }
    if (lowPowerIndicatorThreshold <= bat->getLevel()) {
      return;
    }
    if (lowPowerIndicationDone) {
      return;
    }
   ...
   */
  float readVoltage() {
    // use calibrated millivolts analogread on esp32 (150 mV ~ 2450 mV default attentuation) and divide by 1000 to get
    // from milivolts to volts and multiply by voltage multiplier and apply calibration value
    return (analogReadMilliVolts(batteryPin) / 1000.0f) * bat->getVoltageMultiplier() + bat->getCalibration();
  }

#ifndef WLED_DISABLE_MQTT
  void addMqttSensor(const String& name, const String& type, const String& topic, const String& deviceClass,
                     const String& unitOfMeasurement = "", const bool& isDiagnostic = false) {
 ...
 public:
  ...
  void addBatteryToJsonObject(JsonObject& battery, bool forJsonState) {
    if (forJsonState) {
      battery["type"] = cfg.type;
    } else {
      battery["type"] = (String)cfg.type;
    }  // has to be a String otherwise it won't get converted to a Dropdown
  ...
  */
  ...
  void setAutoOffThreshold(int8_t threshold) {
    ...
    autoOffThreshold =
        lowPowerIndicatorEnabled /*&& autoOffEnabled*/ ? min(lowPowerIndicatorThreshold - 1, (int)autoOffThreshold)
                                                       : autoOffThreshold;
  }
  ...
  void setLowPowerIndicatorThreshold(int8_t threshold) {
    ...
    lowPowerIndicatorThreshold =
        autoOffEnabled /*&& lowPowerIndicatorEnabled*/ ? max(autoOffThreshold + 1, (int)lowPowerIndicatorThreshold)
                                                       : max(5, (int)lowPowerIndicatorThreshold);
  }
...
// strings to reduce flash memory usage (used more than twice)
const char UsermodBattery::_name[]         = "Battery";
const char UsermodBattery::_readInterval[] = "interval";
const char UsermodBattery::_enabled[]      = "enabled";
const char UsermodBattery::_threshold[]    = "threshold";
const char UsermodBattery::_preset[]       = "preset";
const char UsermodBattery::_duration[]     = "duration";
const char UsermodBattery::_init[]         = "init";
const char UsermodBattery::_haDiscovery[]  = "HA-discovery";
