...
class UMBattery {
 ...
 public:
  ...
  virtual void update(batteryConfig cfg) {
    if (cfg.minVoltage) {
      this->setMinVoltage(cfg.minVoltage);
    }
    if (cfg.maxVoltage) {
      this->setMaxVoltage(cfg.maxVoltage);
    }
    if (cfg.level) {
      this->setLevel(cfg.level);
    }
    if (cfg.calibration) {
      this->setCalibration(cfg.calibration);
    }
    if (cfg.voltageMultiplier) {
      this->setVoltageMultiplier(cfg.voltageMultiplier);
    }
  }
