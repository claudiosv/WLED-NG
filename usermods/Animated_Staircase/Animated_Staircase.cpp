...
#define UPPER true
  ...
  bool checkSensors() {
    ...
    if ((millis() - lastScanTime) > scanDelay) {
      ...
      bottomSensorRead =
          bottomSensorWrite ||
          (!useUSSensorBottom ? (bottomPIRorTriggerPin < 0 ? false : digitalRead(bottomPIRorTriggerPin))
                              : ultrasoundRead(bottomPIRorTriggerPin, bottomEchoPin, bottomMaxDist * 59)  // cm to us
          );
      topSensorRead = topSensorWrite ||
                      (!useUSSensorTop ? (topPIRorTriggerPin < 0 ? false : digitalRead(topPIRorTriggerPin))
  ...
  void enable(bool enable) {
    if (enable) {
      ...
      onIndex = minSegmentId = strip.getMainSegmentId();  // it may not be the best idea to start with main segment as
 ...
 public:
  ...
  void loop() {
    ...
    minSegmentId = strip.getMainSegmentId();  // it may not be the best idea to start with main segment as it may not be
                                              // the first one
  ...
  bool readFromConfig(JsonObject& root) {
    ...
    segment_delay_ms = (unsigned long)min((unsigned long)10000,
                                          max((unsigned long)10, (unsigned long)segment_delay_ms));  // max delay 10s
    ...
    if (!initDone) {
      ...
      bool changed = false;
      if ((oldUseUSSensorTop != useUSSensorTop) || (oldUseUSSensorBottom != useUSSensorBottom) ||
          (oldTopAPin != topPIRorTriggerPin) || (oldTopBPin != topEchoPin) ||
          (oldBottomAPin != bottomPIRorTriggerPin) || (oldBottomBPin != bottomEchoPin)) {
        changed = true;
...
// strings to reduce flash memory usage (used more than twice)
const char Animated_Staircase::_name[]                      = "staircase";
const char Animated_Staircase::_enabled[]                   = "enabled";
const char Animated_Staircase::_segmentDelay[]              = "segment-delay-ms";
const char Animated_Staircase::_onTime[]                    = "on-time-s";
const char Animated_Staircase::_useTopUltrasoundSensor[]    = "useTopUltrasoundSensor";
const char Animated_Staircase::_topPIRorTrigger_pin[]       = "topPIRorTrigger_pin";
const char Animated_Staircase::_topEcho_pin[]               = "topEcho_pin";
const char Animated_Staircase::_useBottomUltrasoundSensor[] = "useBottomUltrasoundSensor";
const char Animated_Staircase::_bottomPIRorTrigger_pin[]    = "bottomPIRorTrigger_pin";
const char Animated_Staircase::_bottomEcho_pin[]            = "bottomEcho_pin";
const char Animated_Staircase::_topEchoCm[]                 = "top-dist-cm";
const char Animated_Staircase::_bottomEchoCm[]              = "bottom-dist-cm";
const char Animated_Staircase::_togglePower[]               = "toggle-on-off";
