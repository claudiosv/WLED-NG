...
class Segment {
 public:
  ...
  uint8_t blendMode;  // segment blending modes: top, bottom, add, subtract, difference, average, multiply, divide,
 ...
 private:
  ...
  static CRGBPalette16
      _currentPalette;  // palette used for current effect (includes transition, used in color_from_palette())
  ...
  struct Transition {
    ...
    uint16_t
        _progress;  // transition progress (0-65535); pre-calculated from _start & _dur in updateTransitionProgress()
    uint8_t _prevPaletteBlends;  // number of previous palette blends (there are max 255 blends possible)
    uint8_t _palette, _bri,
        _cct;  // palette ID, brightness and CCT at the start of transition (brightness will be 0 if segment was off)
 ...
 protected:
  ...
  inline void setPixelColorXYRaw(unsigned x, unsigned y, uint32_t c) const {
    auto XY = [](unsigned X, unsigned Y) {
      return X + Y * Segment::vWidth();
    };
    pixels[XY(x, y)] = c;
  }
  inline uint32_t getPixelColorXYRaw(unsigned x, unsigned y) const {
    auto XY = [](unsigned X, unsigned Y) {
      return X + Y * Segment::vWidth();
    };
  ...
  inline uint16_t progress() const {
  ...
  inline Segment *getOldSegment() const {
    return isInTransition() ? _t->_oldSegment : nullptr;
  }

  inline static void modeBlend(bool blend) {
    Segment::_modeBlend = blend;
  }  // for isPreviousMode()
  inline static void setClippingRect(int startX, int stopX, int startY = 0, int stopY = 1) {
    _clipStart  = startX;
    _clipStop   = stopX;
    _clipStartY = startY;
    _clipStopY  = stopY;
  };
  inline static bool isPreviousMode() {
    return Segment::_modeBlend;
 ...
 public:
  ...
  inline bool getOption(uint8_t n) const {
  ...
  inline uint16_t groupLength() const {
    return grouping + spacing;
  }
  inline uint8_t getLightCapabilities() const {
    return _capabilities;
  }
  inline void deactivate() {
    setGeometry(0, 0);
  }
  inline Segment &clearName() {
    p_free(name);
    name = nullptr;
    return *this;
  }
  inline Segment &setName(const String &name) {
    return setName(name.c_str());
  }

  inline static unsigned vLength() {
    return Segment::_vLength;
  }
  inline static unsigned vWidth() {
    return Segment::_vWidth;
  }
  inline static unsigned vHeight() {
    return Segment::_vHeight;
  }
  inline static uint32_t getCurrentColor(unsigned i) {
    return Segment::_currentColors[i < NUM_COLORS ? i : 0];
  }
  inline static const CRGBPalette16 &getCurrentPalette() {
    return Segment::_currentPalette;
  }

  inline void setDrawDimensions() const {
    Segment::_vWidth  = virtualWidth();
    Segment::_vHeight = virtualHeight();
    Segment::_vLength = virtualLength();
  }
  ...
  inline static unsigned getUsedSegmentData() {
    return Segment::_usedSegmentData;
  }
  /**
  ...
  inline void setPixelColor(int n, CRGB c) const {
  ...
  inline void blendPixelColor(int n, uint32_t color, uint8_t blend) const {
    setPixelColor(n, color_blend(getPixelColor(n), color, blend));
  }
  inline void blendPixelColor(int n, CRGB c, uint8_t blend) const {
    blendPixelColor(n, RGBW32(c.r, c.g, c.b, 0), blend);
  }
  inline void addPixelColor(int n, uint32_t color, bool preserveCR = true) const {
  ...
  inline void addPixelColor(int n, CRGB c, bool preserveCR = true) const {
    addPixelColor(n, RGBW32(c.r, c.g, c.b, 0), preserveCR);
  }
  inline void fadePixelColor(uint16_t n, uint8_t fade) const {
  ...
  inline void blendPixelColorXY(uint16_t x, uint16_t y, uint32_t color, uint8_t blend) const {
    setPixelColorXY(x, y, color_blend(getPixelColorXY(x, y), color, blend));
  }
  inline void blendPixelColorXY(uint16_t x, uint16_t y, CRGB c, uint8_t blend) const {
    blendPixelColorXY(x, y, RGBW32(c.r, c.g, c.b, 0), blend);
  }
  inline void addPixelColorXY(int x, int y, uint32_t color, bool preserveCR = true) const {
  ...
  inline void addPixelColorXY(int x, int y, CRGB c, bool preserveCR = true) const {
    addPixelColorXY(x, y, RGBW32(c.r, c.g, c.b, 0), preserveCR);
  }
  inline void fadePixelColorXY(uint16_t x, uint16_t y, uint8_t fade) const {
    setPixelColorXY(x, y, color_fade(getPixelColorXY(x, y), fade, true));
  }
  inline void blurCols(uint8_t blur_amount, bool smear = false) const {
    blur2D(0, blur_amount, smear);
  }  // blur all columns (50% faster than full 2D blur)
  inline void blurRows(uint8_t blur_amount, bool smear = false) const {
  ...
  inline void drawCircle(uint16_t cx, uint16_t cy, uint8_t radius, CRGB c, bool soft = false) const {
    drawCircle(cx, cy, radius, RGBW32(c.r, c.g, c.b, 0), soft);
  }
  inline void fillCircle(uint16_t cx, uint16_t cy, uint8_t radius, CRGB c, bool soft = false) const {
    fillCircle(cx, cy, radius, RGBW32(c.r, c.g, c.b, 0), soft);
  }
  inline void drawLine(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, CRGB c, bool soft = false) const {
    drawLine(x0, y0, x1, y1, RGBW32(c.r, c.g, c.b, 0), soft);
  }  // automatic inline
  inline void fill_solid(CRGB c) const {
...
class WS2812FX {
 ...
 public:
  ...
  inline void setPixelColor(unsigned n, CRGB c) const {
    setPixelColor(n, c.red, c.green, c.blue);
  }
  inline void fill(uint32_t c) const {
  ...
  inline void setShowCallback(show_callback cb) {
    _callback = cb;
  }
  inline void setTransition(uint16_t t) {
    _transitionDur = t;
  ...
  inline bool isServicing() const {
    return _isServicing;
  }  // returns true if strip.service() is executing
  inline bool hasWhiteChannel() const {
  ...
  inline uint16_t getFrameTime() const {
    return _frametime;
  }  // returns amount of time a frame should take (in ms)
  inline uint16_t getMinShowDelay() const {
  ...
  inline uint16_t getTransition() const {
  ...
  inline uint32_t getLastShow() const {
    return _lastShow;
  ...
  struct Panel {
    ...
    Panel() : xOffset(0), yOffset(0), width(8), height(8), options(0) {
    }
  };
  ...
  inline void setPixelColorXY(unsigned x, unsigned y, byte r, byte g, byte b, byte w = 0) const {
    setPixelColorXY(x, y, RGBW32(r, g, b, w));
  }
  inline void setPixelColorXY(unsigned x, unsigned y, CRGB c) const {
    setPixelColorXY(x, y, RGBW32(c.r, c.g, c.b, 0));
  ...
  inline uint32_t getPixelColorXY(unsigned x, unsigned y) const {
    return getPixelColor(y * Segment::maxWidth + x);
  }
