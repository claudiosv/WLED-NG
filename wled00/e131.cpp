...
static void handleDDPPacket(e131_packet_t* p) {
  ...
  if (maskedType != DDP_TYPE_RGB24 && maskedType != DDP_TYPE_RGBW32) {
    return;
  }
  ...
  unsigned ddpChannelsPerLed = ((p->dataType & 0b00111000) >> 3 == 0b011)
                                   ? 4
                                   : 3;  // data type 0x1B (formerly 0x1A) is RGBW (type 3, 8 bit/channel)
  ...
  if (p->flags & DDP_FLAGS_TIME) {
    c = 4;  // packet has timecode flag, we do not support it, but data starts 4 bytes
            // later
  }
...
void handleE131Packet(e131_packet_t* p, const IPAddress& clientIP, byte protocol) {
  ...
  if (protocol == P_ARTNET) {
    ...
    if (e131Priority != 0) {
      if (p->priority < e131Priority) {
        return;
      }
      // track highest priority & skip all lower priorities
      if (p->priority >= highPriority.get()) {
        highPriority.set(p->priority);
      }
      if (p->priority < highPriority.get()) {
        return;
      }
  ...
  if (e131SkipOutOfSequence) {
    if (seq < e131LastSequenceNumber[previousUniverses] && seq > 20 &&
        e131LastSequenceNumber[previousUniverses] < 250) {
...
void handleDMXData(uint16_t uni, uint16_t dmxChannels, uint8_t* e131_data, uint8_t mde, uint8_t previousUniverses) {
  ...
  switch (DMXMode) {
    ...
    {
      if (uni != e131Universe || availDMXLen < 2) {
        return;
      }
    ...
    case DMX_MODE_EFFECT:  // 15 channels
    ...
    case DMX_MODE_EFFECT_SEGMENT_W:  // 18 Channels per segment;
    {
      ...
      for (unsigned id = 0; id < strip.getSegmentsNum(); id++) {
        ...
        if (e131_data[dataOffset + 1] < strip.getModeCount()) {
          if (e131_data[dataOffset + 1] != seg.mode) {
            seg.setMode(e131_data[dataOffset + 1]);
          }
        }
        if (e131_data[dataOffset + 2] != seg.speed) {
          seg.speed = e131_data[dataOffset + 2];
        }
        if (e131_data[dataOffset + 3] != seg.intensity) {
          seg.intensity = e131_data[dataOffset + 3];
        }
        if (e131_data[dataOffset + 4] != seg.palette) {
        ...
        colors[1] =
            RGBW32(e131_data[dataOffset + 9], e131_data[dataOffset + 10], e131_data[dataOffset + 11], whites[1]);
        colors[2] =
            RGBW32(e131_data[dataOffset + 12], e131_data[dataOffset + 13], e131_data[dataOffset + 14], whites[2]);
        ...
        if (isSegmentMode) {
          if (e131_data[dataOffset] != seg.opacity) {
            seg.setOpacity(e131_data[dataOffset]);
          }
    ...
    case DMX_MODE_MULTIPLE_RGBW: {
      ...
      for (unsigned i = previousLeds; i < ledsTotal; i++) {
        setRealtimePixel(i, e131_data[dmxOffset], e131_data[dmxOffset + 1], e131_data[dmxOffset + 2],
                         is4Chan ? e131_data[dmxOffset + 3] : 0);
...
static void handleArtnetPollReply(const IPAddress& ipAddress) {
  ...
  switch (DMXMode) {
    ...
    case DMX_MODE_MULTIPLE_RGBW: {
      ...
      const unsigned ledsInFirstUniverse =
          (((MAX_CHANNELS_PER_UNIVERSE - DMXAddress) + dmxLenOffset) - dimmerOffset) / dmxChannelsPerLed;
  ...
  if (e131ProxyUniverse > 0 &&
      (DMXMode == DMX_MODE_DISABLED || (e131ProxyUniverse < startUniverse || e131ProxyUniverse > endUniverse))) {
