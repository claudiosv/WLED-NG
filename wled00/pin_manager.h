...
enum struct PinOwner : uint8_t {
  ...
  UM_PIXELS_DICE_TRAY = USERMOD_ID_PIXELS_DICE_TRAY  // 0x35 // Usermod "pixels_dice_tray.h" -- Needs compile time
...
namespace PinManager {
  ...
  inline bool allocatePin(byte gpio, bool output = true) {
    return allocatePin(gpio, output, PinOwner::None);
  }
  ...
  inline void deallocatePin(byte gpio) {
    deallocatePin(gpio, PinOwner::None);
  }
