...
static void doSaveState() {
  ...
  while (strip.isUpdating() && millis() < maxWait) {
    delay(1);  // wait for strip to finish updating, accessing FS during sendout causes glitches
  }

  if (!requestJSONBufferLock(JSON_LOCK_PRESET_SAVE)) {
    return;
  }
  ...
  if (saveName) {
    ...
    sObj["n"] = "Unkonwn preset";  // should not happen, but just in case...
  }
  if (quickLoad && quickLoad[0]) {
    sObj["ql"] = quickLoad;
  }
  if (saveLedmap >= 0) {
    sObj["ledmap"] = saveLedmap;
  }
  /*
    #ifdef WLED_DEBUG
...
void initPresetsFile() {
  char fileName[33];
  strncpy(fileName, getPresetsFileName(), 32);
  fileName[32] = 0;  // use safe copy as FS.open() does not
...
void handlePresets() {
  ...
  {
    presetErrFlag =
        readObjectFromFileUsingId(getPresetsFileName(tmpPreset < 255), tmpPreset, pDoc) ? ERR_NONE : ERR_FS_PLOAD;
  }
  fdo = pDoc->as<JsonObject>();

  // only reset errorflag if previous error was preset-related
  if ((errorFlag == ERR_NONE) || (errorFlag == ERR_FS_PLOAD)) {
    errorFlag = presetErrFlag;
  }
  ...
  if (httpwin) {
    ...
    if (!fdo["seg"].isNull() || !fdo["on"].isNull() || !fdo["bri"].isNull() || !fdo["nl"].isNull() ||
        !fdo["ps"].isNull() || !fdo["playlist"].isNull()) {
      changePreset = true;
    }
    if (!(tmpMode == CALL_MODE_BUTTON_PRESET && fdo["ps"].is<const char*>() &&
          strchr(fdo["ps"].as<const char*>(), '~') != strrchr(fdo["ps"].as<const char*>(), '~'))) {
      fdo.remove("ps");  // remove load request for presets to prevent recursive crash (if not called by button and
                         // contains preset cycling string "1~5~")
    }
  ...
  releaseJSONBufferLock();
  if (changePreset) {
    notify(tmpMode);  // force UDP notification
  }
  stateUpdated(tmpMode);  // was colorUpdated() if anything breaks
  updateInterfaces(tmpMode);
}

// called from handleSet(PS=) [network callback (sObj is empty), IR (irrational), deserializeState, UDP] and
// deserializeState() [network callback (filedoc!=nullptr)]
void savePreset(byte index, const char* pname, JsonObject sObj) {
  if (!saveName) {
    saveName = static_cast<char*>(p_malloc(33));
  }
  if (!quickLoad) {
    quickLoad = static_cast<char*>(p_malloc(9));
  }
  ...
  if (pname) {
    ...
    if (sObj["n"].is<const char*>()) {
      strlcpy(saveName, sObj["n"].as<const char*>(), 33);
    } else {
      sprintf(saveName, "Preset %d", index);
    }
  }
  ...
  presetToSave = index;
  playlistSave = false;
  if (sObj["ql"].is<const char*>()) {
    strlcpy(quickLoad, sObj["ql"].as<const char*>(),
  ...
  if (sObj.size() == 0 ||
      sObj["o"]
          .isNull()) {  // no "o" means not a playlist or custom API call, saving of state is async (not immediately)
    ...
    if (sObj["playlist"].isNull()) {
      ...
      // store playlist
      // WARNING: playlist will be loaded in json.cpp after this call and will have repeat counter increased by 1 it
      // will also be randomised if selected
