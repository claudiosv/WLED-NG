...
Segment::Segment(const Segment &orig) {
  ...
  if (orig.pixels) {
    ...
    pixels = static_cast<uint32_t *>(
        allocate_buffer(orig.length() * sizeof(uint32_t), BFRALLOC_PREFER_PSRAM | BFRALLOC_NOBYTEACCESS));
    if (pixels) {
      memcpy(pixels, orig.pixels, sizeof(uint32_t) * orig.length());
      if (orig.name) {
        name = static_cast<char *>(allocate_buffer(strlen(orig.name) + 1, BFRALLOC_PREFER_PSRAM));
        if (name) {
...
//  limit to MAX_SEGMENT_DATA if there is no PSRAM, otherwise prefer functionality over speed
  ...
  data = static_cast<byte *>(
      allocate_buffer(len, BFRALLOC_PREFER_DRAM | BFRALLOC_CLEAR));  // prefer DRAM over PSRAM for speed
...
void Segment::deallocateData() {
  ...
  if ((Segment::getUsedSegmentData() > 0) &&
      (_dataLen > 0)) {  // check that we don't have a dangling / inconsistent data pointer
...
void Segment::resetIfRequired() {
  ...
  if (data && _dataLen > 0) {
    if (_dataLen > FAIR_DATA_PER_SEG) {
      deallocateData();  // do not keep large allocations
    } else {
      memset(data, 0, _dataLen);  // can prevent heap fragmentation
    }
    DEBUG_PRINTF_P("-- Segment %p reset, data cleared\n", this);
  }
  if (pixels) {
    for (size_t i = 0; i < length(); i++) {
      pixels[i] = BLACK;  // clear pixel buffer
    }
  }
  step  = 0;
  call  = 0;
...
void Segment::loadPalette(CRGBPalette16 &targetPalette, uint8_t pal) {
  ...
  if (pal >= FIXED_PALETTE_COUNT && pal <= 255 - customPalettes.size()) {
    pal = 0;  // out of bounds palette
  }
  // default palette. Differs depending on effect
  if (pal == 0) {
    pal = _default_palette;  // _default_palette is set in setMode()
  }
  switch (pal) {
    ...
    case 5: {  // primary + secondary (+tertiary if not off), more distinct
      ...
      if (colors[2]) {
        CRGB ter = colors[2];
        targetPalette =
            CRGBPalette16(prim, prim, prim, prim, prim, sec, sec, sec, sec, sec, ter, ter, ter, ter, ter, prim);
      } else {
        targetPalette =
            CRGBPalette16(prim, prim, prim, prim, prim, prim, prim, prim, sec, sec, sec, sec, sec, sec, sec, sec);
      }
      break;
    }
    default:  // progmem palettes
      if (pal > 255 - customPalettes.size()) {
        ...
        byte tcp[72];
        memcpy_P(tcp,
                 (byte *)pgm_read_dword(&(gGradientPalettes[pal - (DYNAMIC_PALETTE_COUNT + FASTLED_PALETTE_COUNT)])),
                 sizeof(tcp));
...
void Segment::updateTransitionProgress() const {
  if (isInTransition()) {
    ...
    if (_t->_dur > 0 && diff < _t->_dur) {
      _t->_progress = diff * 0xFFFFU / _t->_dur;
    }
  }
}
...
// prog is the progress of the transition (0-65535) and is passed to the function as it may be called in the context of
// old segment which does not have transition structure
void Segment::beginDraw(uint16_t prog) {
  setDrawDimensions();
  // load colors into _currentColors
  for (unsigned i = 0; i < NUM_COLORS; i++) {
    _currentColors[i] = colors[i];
  }
...
// minimum blend time is 100ms maximum is 65535ms
    ...
    for (unsigned i = 0; i < noOfBlends; i++) {
      nblendPaletteTowardPalette(tmpPalette, Segment::_currentPalette, 48);
    }
...
void Segment::handleRandomPalette() {
  ...
  if (now_s > Segment::_lastPaletteChange + randomPaletteChangeTime) {
    Segment::_newRandomPalette =
        useHarmonicRandomPalette ? generateHarmonicRandomPalette(Segment::_randomPalette) : generateRandomPalette();
  ...
  unsigned transitionFrames =
      frameTime > transitionTime
          ? 1
          : transitionTime / frameTime;  // i.e. 700ms/23ms = 30 or 20000ms/8ms = 2500 or 100ms/1000ms = 0 -> 1
  unsigned noOfBlends =
      transitionFrames > 255 ? 1 : (255 + (transitionFrames >> 1)) / transitionFrames;  // we do some rounding here
  for (unsigned i = 0; i < noOfBlends; i++) {
    nblendPaletteTowardPalette(Segment::_randomPalette, Segment::_newRandomPalette, 48);
  }
...
// this function may call fill() to clear pixels if spacing or mapping changed (which requires setting _vWidth,
// _vHeight, _vLength or beginDraw())
void Segment::setGeometry(uint16_t i1, uint16_t i2, uint8_t grp, uint8_t spc, uint16_t ofs, uint16_t i1Y, uint16_t i2Y,
  ...
  boundsUnchanged &=
      (grouping == grp &&
       spacing == spc);  // changing grouping and/or spacing changes virtual segment length (painting dimensions)

  if (stop && (spc > 0 || m12 != map1D2D)) {
    clear();
  }
  ...
  stop   = i2 > Segment::maxWidth * Segment::maxHeight && i1 >= Segment::maxWidth * Segment::maxHeight
               ? MIN(i2, strip.getLengthTotal())
               : constrain(i2, 1, Segment::maxWidth);  // check for 2D trailing strip
  startY = 0;
  stopY  = 1;
#ifndef WLED_DISABLE_2D
  ...
  if (length() != oldLength) {
    // allocate render buffer (always entire segment), prefer IRAM/PSRAM. Note: impact on FPS with PSRAM buffer is low
    // (<2% with QSPI PSRAM) on S2/S3
    p_free(pixels);
    pixels = static_cast<uint32_t *>(
        allocate_buffer(length() * sizeof(uint32_t), BFRALLOC_PREFER_PSRAM | BFRALLOC_NOBYTEACCESS));
    if (!pixels) {
      DEBUGFX_PRINTLN("!!! Not enough RAM for pixel buffer !!!");
#ifdef WLED_ENABLE_GIF
      endImagePlayback(this);
...
Segment &Segment::setColor(uint8_t slot, uint32_t c) {
  ...
  if (!_isRGB && !_hasW) {
    if (slot == 0 && c == BLACK) {
      return *this;  // on/off segment cannot have primary color black
    }
    if (slot == 1 && c != BLACK) {
      return *this;  // on/off segment cannot have secondary color non black
    }
  }
...
Segment &Segment::setMode(uint8_t fx, bool loadDefaults) {
  // skip reserved
  while (fx < strip.getModeCount() && strncmp("RSVD", strip.getModeData(fx), 4) == 0) {
    fx++;
  }
  ...
  if (fx != mode) {
    ...
    if (loadDefaults) {
      ...
      if (sOpt >= 0) {
        ...
        map1D2D = M12_Pixels;  // reset mapping if not defined (2D FX may not work)
      }
      sOpt = extractModeDefaults(fx, "si");
      if (sOpt >= 0) {
        soundSim = constrain(sOpt, 0, 3);
      }
    ...
    if (sOpt >= 0 && loadDefaults) {
      setPalette(sOpt);
    }
    if (sOpt <= 0) {
      sOpt = 6;  // partycolors if zero or not set
    }
...
Segment &Segment::setPalette(uint8_t pal) {
  if (pal <= 255 - customPalettes.size() && pal > FIXED_PALETTE_COUNT) {
    pal = 0;  // not built in palette or custom palette
  }
  if (pal != palette) {
    // DEBUG_PRINTF_P("- Starting palette transition: %d\n", pal);
    startTransition(strip.getTransition(),
                    blendingStyle != TRANSITION_FADE);  // start transition prior to change (no need to copy segment)
    palette      = pal;
    stateChanged = true;  // send UDP/WS broadcast
  }
  return *this;
  ...
  if (newName) {
    ...
    if (newLen) {
      ...
      if (mode == FX_MODE_2DSCROLLTEXT) {
        startTransition(strip.getTransition(),
                        true);  // if the name changes in scrolling text mode, we need to copy the segment for blending
      }
      if (name) {
        strlcpy(name, newName, newLen + 1);
      }
      return *this;
    }
  }
...
unsigned Segment::virtualWidth() const {
  ...
  if (mirror) {
    vWidth = (vWidth + 1) / 2;  // divide by 2 if mirror, leave at least a single LED
  }
  return vWidth;
}
...
unsigned Segment::virtualHeight() const {
  ...
  if (mirror_y) {
    vHeight = (vHeight + 1) / 2;  // divide by 2 if mirror, leave at least a single LED
  }
  return vHeight;
}
...
void WLED_O2_ATTR Segment::setPixelColor(int i, uint32_t col) const {
  if (!isActive() || i < 0) {
    return;  // not active or invalid index
  }
#ifndef WLED_DISABLE_2D
  int vStrip = 0;
#endif
  ...
  if (is2D()) {
    ...
    switch (map1D2D) {
      ...
      case M12_pBar:
        ...
        if (vStrip > 0) {
          setPixelColorRaw(XY(vStrip - 1, vH - i - 1), col);
        } else {
          for (int x = 0; x < vW; x++) {
            setPixelColorRaw(XY(x, vH - i - 1), col);
          }
        }
        break;
      case M12_pArc:
      ...
      case M12_pCorner:
        for (int x = 0; x <= i; x++) {
          setPixelColorXY(x, i, col);  // note: <= to include i=0. Relies on overflow check in sPC()
        }
        for (int y = 0; y < i; y++) {
      ...
      case M12_sPinwheel: {
        ...
        unsigned maxLineLength =
            max(vW, vH) + 2;  // pixels drawn is always smaller than dx or dy, +1 pair for rounding errors
...
uint32_t WLED_O2_ATTR Segment::getPixelColor(int i) const {
  if (!isActive() || i < 0) {
    return 0;  // not active or invalid index
  }
    ...
    for (unsigned x = start; x < stop; x++) {
      ...
      for (unsigned b = 0; b < BusManager::getNumBusses(); b++) {
        ...
        if (bus->containsPixel(index)) {
          if (bus->hasRGB() || (strip.cctFromRgb && bus->hasCCT())) {
            capabilities |= SEG_CAPABILITY_RGB;
          }
          if (!strip.cctFromRgb && bus->hasCCT()) {
            capabilities |= SEG_CAPABILITY_CCT;
          }
          if (strip.correctWB && (bus->hasRGB() || bus->hasCCT())) {
            capabilities |= SEG_CAPABILITY_CCT;  // white balance correction (CCT slider)
          }
          if (bus->hasWhite()) {
            unsigned aWM =
                Bus::getGlobalAWMode() == AW_GLOBAL_DISABLED ? bus->getAutoWhiteMode() : Bus::getGlobalAWMode();
            bool whiteSlider = (aWM == RGBW_MODE_DUAL || aWM == RGBW_MODE_MANUAL_ONLY);  // white slider allowed
            // if auto white calculation from RGB is active (Accurate/Brighter), force RGB controls even if there are no
            // RGB busses
            if (!whiteSlider) {
              capabilities |= SEG_CAPABILITY_RGB;
            }
...
void Segment::fill(uint32_t c) const {
  ...
  for (unsigned i = 0; i < length(); i++) {
    setPixelColorRaw(i, c);  // always fill all pixels (blending will take care of grouping, spacing and clipping)
  }
}
...
void Segment::fadeToSecondaryBy(uint8_t fadeBy) const {
  ...
  for (unsigned i = 0; i < rlength; i++) {
    setPixelColorRaw(i, color_blend(getPixelColorRaw(i), colors[1], fadeBy));
  }
}
...
uint32_t Segment::color_wheel(uint8_t pos) const {
  if (palette) {
    return color_from_palette(pos, false, false, 0);  // only wrap if "always wrap" is set
  }
 ...
 * @param i Palette Index (if mapping is true, the full palette will be _virtualSegmentLength long, if false, 255). Will
 * wrap around automatically.
 * @param mapping if true, LED position in segment is considered for color
 * @param moving FastLED palettes will usually wrap back to the start smoothly. Set to true if effect has moving palette
 * and you want wrap.
 * @param mcol If the default palette 0 is selected, return the standard color 0, 1 or 2 instead. If >2, Party palette
 * is used instead
...
uint32_t Segment::color_from_palette(uint16_t i, bool mapping, bool moving, uint8_t mcol, uint8_t pbri) const {
  ...
  if (mapping) {
    paletteIndex = min((i * 255) / vLength(), 255U);
  }
  // paletteBlend: 0 - wrap when moving, 1 - always wrap, 2 - never wrap, 3 - none (undefined/no interpolation of
  // palette entries) ColorFromPalette interpolations are: NOBLEND, LINEARBLEND, LINEARBLEND_NOWRAP
...
// estimate maximum I2S memory usage (only relevant for digital non-2pin busses when I2S is enabled)
...
#endif
      unsigned i2sCommonMem =
          (stepFactor * bus.count * (3 * Bus::hasRGB(bus.type) + Bus::hasWhite(bus.type) + Bus::hasCCT(bus.type)) *
           (Bus::is16bit(bus.type) + 1));
      if (useParallelI2S) {
        i2sCommonMem *= 8;  // parallel I2S uses 8 channels, requiring 8x the DMA buffer size (common buffer shared
                            // between all parallel busses)
      }
      if (i2sCommonMem > I2SdmaMem) {
        I2SdmaMem = i2sCommonMem;
      }
    }
#endif
    if (mem + I2SdmaMem > MAX_LED_MEMORY + 1024) {  // +1k to allow some margin to not drop buses that are allowed in UI
                                                    // (calculation here includes bus overhead)
    ...
    if (BusManager::add(bus, use_placeholder) != -1) {
      ...
      if (Bus::isDigital(bus.type) && !Bus::is2Pin(bus.type) && BusManager::busses.back()->isPlaceholder()) {
        digitalCount--;  // remove placeholder from digital count
      }
    }
  }
  ...
  for (size_t i = 0; i < BusManager::getNumBusses(); i++) {
    ...
    if (!bus || !bus->isOk() || bus->getStart() + bus->getLength() > MAX_LEDS) {
      break;
    }
  ...
  // use PSRAM if available: there is no measurable perfomance impact between PSRAM and DRAM on S2/S3 with QSPI PSRAM
  // for this buffer
...
void WS2812FX::service() {
  ...
  if (_triggered || _targetFps == FPS_UNLIMITED) {
    timeToShow = true;  // unlimited mode = no frametime; strip.trigger() can overrule timing
  }

  now = nowUp + timebase;  // common time base for all effects
  if (!timeToShow) {
    return;  // too early for service
  }
  if (_suspend || elapsed <= MIN_FRAME_DELAY) {
    return;  // keep wifi alive - no matter if triggered or unlimited
  }

  _isServicing = true;
  bool doShow  = _triggered;  // true if ≥1 active segment was processed (and strip was not suspended mid-loop), or
                              // trigger received → triggers show()
  for (size_t i = 0; i < _segments.size(); i++) {
    Segment &seg   = _segments[i];
    _segment_index = i;
    if (_suspend) {
      break;  // abort processing segments if suspend requested during service()
    }
    ...
    if (seg.isActive()) {
      ...
      if (!seg.freeze) {  // only run effect function if not frozen
        ...
        seg.beginDraw(prog);  // set up parameters for get/setPixelColor() (will also blend colors and palette if blend
                              // style is FADE)
        ...
        if (segO && segO->isActive() &&
            (seg.mode != segO->mode || blendingStyle != TRANSITION_FADE ||
             (segO->name != seg.name && segO->name && seg.name &&
              strncmp(segO->name, seg.name, WLED_MAX_SEGNAME_LEN) != 0))) {
          Segment::modeBlend(true);  // set flag for beginDraw() to blend colors and palette
          segO->beginDraw(
              prog);  // set up palette & colors (also sets draw dimensions), parent segment has transition progress
  ...
  _currentSegment = &_segments[0];  // safe fallback to prevent stale pointer - SEGMENT/SEGENV should not be used
                                    // outside of the service loop

#ifdef WLED_DEBUG
  if ((_targetFps != FPS_UNLIMITED) && (millis() - nowUp > _frametime)) {
    DEBUG_PRINTF_P("Slow effects %u/%d.\n", (unsigned)(millis() - nowUp), (int)_frametime);
  }
#endif
  if (doShow && !_suspend) {
    yield();
    Segment::handleRandomPalette();  // slowly transition random palette; move it into for loop when each segment has
...
#ifdef WLED_DEBUG
  if ((_targetFps != FPS_UNLIMITED) && (millis() - nowUp > _frametime)) {
    DEBUG_PRINTF_P("Slow strip %u/%d.\n", (unsigned)(millis() - nowUp), (int)_frametime);
  }
#endif

  if (!_suspend) {
    _triggered = false;  // avoid losing "trigger" events if suspend requested during effect service()
  }
  _isServicing = false;
}
...
static uint8_t _subtract(uint8_t a, uint8_t b) {
  return b > a ? (b - a) : 0;
}
static uint8_t _difference(uint8_t a, uint8_t b) {
  return b > a ? (b - a) : (a - b);
}
...
static uint8_t _multiply(uint8_t a, uint8_t b) {
  return (a * b) / 255;
}  // origianl uses a & b in range [0,1]
#endif
static uint8_t _divide(uint8_t a, uint8_t b) {
  return a > b ? (b * 255) / a : 255;
}
static uint8_t _lighten(uint8_t a, uint8_t b) {
  return a > b ? a : b;
}
static uint8_t _darken(uint8_t a, uint8_t b) {
  return a < b ? a : b;
}
static uint8_t _screen(uint8_t a, uint8_t b) {
...
static uint8_t _burn(uint8_t a, uint8_t b) {
  return ~_divide(a, ~b);
}
...
void WS2812FX::blendSegment(const Segment &topSegment) const {
  ...
  FuncType funcs[] = {_dummy,  _dummy,  _dummy,   _subtract,  _difference, _average, _dummy, _divide, _lighten,
                      _darken, _screen, _overlay, _hardlight, _softlight,  _dodge,   _burn,  _dummy};

  const size_t blendMode =
      topSegment.blendMode < BLENDMODES ? topSegment.blendMode : 0;  // default to top if unsupported mode
  const auto segblend = [&](uint32_t t, uint32_t b) {
    ...
    switch (blendMode) {
      ...
      case 1:
        return b;  // bottom
      case 2:
        return color_add(t, b, true);  // add with preserve color ratio to avoid color clipping
      case 6:
        return RGBW32(_multiply(R(t), R(b)), _multiply(G(t), G(b)), _multiply(B(t), B(b)),
                      _multiply(W(t), W(b)));  // multiply (7% faster than lambda at 100bytes flash cost)
      case 16:
        return t ? t : b;  // stencil (use top layer if not black, else bottom)
    }
  ...
  if (gammaCorrectCol) {
    opacity = gamma8inv(opacity);  // use inverse gamma on brightness for correct color scaling after gamma correction
  ...
  if (!segO && blendingStyle == TRANSITION_FADE && !hasGrouping && !topSegment.mirror && !topSegment.mirror_y) {
    if (isMatrix && stopIndx <= matrixSize && !_pixelCCT) {
      ...
      if (!topSegment.transpose) {
        if (topSegment.reverse) {
          start_offset += (width - 1);
          x_inc = -1;
        }
        if (topSegment.reverse_y) {
          start_offset += (height - 1) * Segment::maxWidth;
          y_inc = -Segment::maxWidth;
        }
  ...
  if (isMatrix && stopIndx <= matrixSize) {
    ...
    const auto setMirroredPixel = [&](int x, int y, uint32_t c, uint8_t o) {
      ...
      if (topSegment.mirror || topSegment.mirror_y) {
        ...
        if (topSegment.mirror) {
          _pixels[idxMX] = color_blend(_pixels[idxMX], segblend(c, _pixels[idxMX]), o);
        }
        if (topSegment.mirror_y) {
          _pixels[idxMY] = color_blend(_pixels[idxMY], segblend(c, _pixels[idxMY]), o);
        }
        if (topSegment.mirror && topSegment.mirror_y) {
          _pixels[idxMM] = color_blend(_pixels[idxMM], segblend(c, _pixels[idxMM]), o);
        }
        if (_pixelCCT) {
          if (topSegment.mirror) {
            _pixelCCT[idxMX] = cct;
    ...
    unsigned offsetX =
        (blendingStyle == TRANSITION_PUSH_UP || blendingStyle == TRANSITION_PUSH_DOWN) ? 0 : progInv * nCols / 0xFFFFU;
    unsigned       offsetY      = (blendingStyle == TRANSITION_PUSH_LEFT || blendingStyle == TRANSITION_PUSH_RIGHT)
                                      ? 0
    ...
    switch (blendingStyle) {
      ...
      case TRANSITION_PUSH_LEFT:
        pushOffsetX = -offsetX + nCols;
        break;
      case TRANSITION_PUSH_DOWN:
        pushOffsetY = offsetY;
        break;
      case TRANSITION_PUSH_UP:
        pushOffsetY = -offsetY + nRows;
        break;
      case TRANSITION_PUSH_TL:
        pushOffsetX = offsetX;
        pushOffsetY = offsetY;
        break;  // unused
      case TRANSITION_PUSH_TR:
        pushOffsetX = -offsetX + nCols;
        pushOffsetY = offsetY;
        break;  // unused
      case TRANSITION_PUSH_BR:
        pushOffsetX = -offsetX + nCols;
        pushOffsetY = -offsetY + nRows;
        break;  // unused
      case TRANSITION_PUSH_BL:
        pushOffsetX = offsetX;
        pushOffsetY = -offsetY + nRows;
        break;  // unused
    }
    ...
    for (int r = 0; r < nRows; r++) {
      for (int c = 0; c < nCols; c++) {
        ...
        if (segO && blendingStyle == TRANSITION_FADE &&
          ...
          // briT) &&  bri => from Off to On
          if ((briOld == 0 || bri == 0) && ((!clipped && (bri != briT) && !bri) || (clipped && (bri != briT) && bri))) {
            c_a = BLACK;
          }
        }
        ...
        if (applyReverse) {
          ...
          if (topSegment.transpose) {
            std::swap(x, y);  // swap X & Y if segment transposed
          }
        }
        // expand pixel
        if (groupLen == 1) {
          ...
          while (y < maxY) {
            int _x = x;
            while (_x < maxX) {
              setMirroredPixel(_x++, y, c_a, opacity);
            }
            y++;
          }
        }
    ...
    for (int k = 0; k < nLen; k++) {
      ...
      if (topSegment.reverse) {
        i = nLen - i - 1;  // is segment reversed?
      }
      // expand pixel
      ...
      while (i < maxI) {
        setMirroredPixel(i++, c_a, opacity);
      }
    }
  }
...
void WS2812FX::show() {
  ...
  if ((hasCCTBus() || correctWB) && !cctFromRgb) {
    _pixelCCT = static_cast<uint8_t *>(allocate_buffer(
        totalLen * sizeof(uint8_t), BFRALLOC_PREFER_PSRAM));  // allocate CCT buffer if necessary, prefer PSRAM
  }
  if (_pixelCCT) {
    memset(_pixelCCT, 127, totalLen);  // set neutral (50:50) CCT
  }
  ...
  if (callback) {
    callback();  // will call setPixelColor or setRealtimePixelColor
  }
  ...
  for (size_t i = 0; i < totalLen; i++) {
    ...
    if (c > 0 && !(realtimeMode && arlsDisableGammaCorrection)) {
      c = gamma32(
          c);  // apply gamma correction if enabled note: applying gamma after brightness has too much color loss
    }
...
void WS2812FX::setRealtimePixelColor(unsigned i, uint32_t c) {
  if (useMainSegmentOnly) {
    ...
    if (seg.isActive() && i < seg.length()) {
      seg.setPixelColorRaw(i, c);
    }
  } else {
    setPixelColor(i, c);
  }
...
void WS2812FX::waitForIt() {
  ...
  if (millis() - waitStart >= maxWait) {
    DEBUG_PRINTLN("Waited for strip to finish servicing.");
  }
#endif
};
...
void WS2812FX::setTargetFps(unsigned fps) {
  ...
  if (_targetFps > 0) {
    ...
    _frametime = MIN_FRAME_DELAY;  // unlimited mode
  }
}
...
// direct=true either expects the caller to call show() themselves (realtime modes) or be ok waiting for the next frame
// for the change to apply direct=false immediately triggers an effect redraw
...
uint16_t WS2812FX::getLengthTotal() const {
  unsigned len =
      Segment::maxWidth *
      Segment::maxHeight;  // will be _length for 1D (see finalizeInit()) but should cover whole matrix for 2D
    ...
    for (size_t i = s; i < BusManager::getNumBusses(); i++) {
      ...
      for (size_t j = 0; j < s; j++) {
        if (segStops[j] > segStarts[s] && segStarts[j] < segStops[s]) {
          ...
          segStops[j]  = max(segStops[s], segStops[j]);
          segStops[s]  = 0;
          s--;
        }
      }
  ...
  for (size_t i = getSegmentsNum() - 1; i > 0; i--) {
    if (isMatrix) {
      ...
      if (_segments[i].start >= Segment::maxWidth * Segment::maxHeight) {
        ...
        if (_segments[i].start >= _length || _segments[i].startY > 0 || _segments[i].stopY > 1) {
          _segments.erase(_segments.begin() + i);
          continue;
        }
      ...
      if (_segments[i].start >= Segment::maxWidth || _segments[i].startY >= Segment::maxHeight) {
        _segments.erase(_segments.begin() + i);
        continue;
      }
...
bool WS2812FX::checkSegmentAlignment() const {
  ...
  for (const Segment &seg : _segments) {
    for (unsigned b = 0; b < BusManager::getNumBusses(); b++) {
      ...
      if (seg.start == bus->getStart() && seg.stop == bus->getStart() + bus->getLength()) {
        aligned = true;
      }
    }
...
void WS2812FX::printSize() {
  ...
  for (const Segment &seg : _segments) {
    DEBUG_PRINTF_P("  Seg: %d,%d [A=%d, 2D=%d, RGB=%d, W=%d, CCT=%d]\n", seg.width(), seg.height(), seg.isActive(),
                   seg.is2D(), seg.hasRGB(), seg.hasWhite(), seg.isCCT());
  }
...
// if this is a matrix set-up and default ledmap.json file does not exist, create mapping table using setUpMatrix() from
// panel information WARNING: effect drawing has to be suspended (strip.suspend()) or must be called from loop() context
bool WS2812FX::deserializeMap(unsigned n) {
  char fileName[32];
  strcpy(fileName, "/ledmap");
  if (n) {
    sprintf(fileName + 7, "%d", n);
  }
  ...
  if (!isFile || !requestJSONBufferLock(JSON_LOCK_LEDMAP)) {
    return false;
  }
  ...
  if (customMappingTable) {
    ...
    while (f.available()) {  // f.position() < f.size() - 1
      char   number[32];
      size_t numRead = f.readBytesUntil(
          ',', number,
          sizeof(number) - 1);  // read a single number (may include array terminating "]" but not number separator ',')
      number[numRead] = 0;
      if (numRead > 0) {
        ...
        if (index < 0 || index > 65535) {
          index = 0xFFFF;  // prevent integer wrap around
        }
        customMappingTable[customMappingSize++] = index;
        if (customMappingSize >= getLengthTotal()) {
          break;
        }
      } else {
        break;  // there was nothing to read, stop
      }
    }
    currentLedmap = n;
    f.close();
...
const char JSON_mode_names[]    = R"=====(["FX names moved"])=====";
