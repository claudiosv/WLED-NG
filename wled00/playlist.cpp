...
void shufflePlaylist() {
  ...
  DEBUG_PRINTLN("Playlist shuffle.");
}
...
void unloadPlaylist() {
  ...
  playlistOptions                 = 0;
  playlistEntryDur                = 0;
  DEBUG_PRINTLN("Playlist unloaded.");
}

int16_t loadPlaylist(JsonObject playlistObj, byte presetId) {
  if (currentPlaylist > 0 && parentPlaylistPresetId > 0) {
    return -1;  // we are already in nested playlist, do nothing
  }
  ...
  if (tr.isNull()) {
    playlistEntries[0].tr = playlistObj["transition"] | (transitionDelay / 100);
    it                    = 1;
  } else {
  ...
  playlistRepeat = rep;
  if (playlistRepeat > 0) {
    playlistRepeat++;  // add one extra repetition immediately since it will be deducted on first start
  }
  ...
  DEBUG_PRINTLN("Playlist loaded.");
  return currentPlaylist;
}

void handlePlaylist() {
  static unsigned long presetCycledTime = 0;
  if (currentPlaylist < 0 || playlistEntries == nullptr) {
    return;
  }
  ...
  if ((playlistEntryDur < UINT32_MAX && millis() - presetCycledTime > playlistEntryDur) || doAdvancePlaylist) {
    ...
    if (!playlistIndex) {
      if (playlistRepeat == 1) {  // stop if all repetitions are done
        ...
        if (parentPlaylistPresetId > 0) {
          ...
          parentPlaylistPresetId = 0;  // reset previous playlist but do not reset Index or Repeat (they will be loaded
                                       // & reset in loadPlaylist())
        } else if (playlistEndPreset) {
          applyPresetFromPlaylist(playlistEndPreset);
        }
        return;
      }
      if (playlistRepeat > 1) {
        playlistRepeat--;  // decrease repeat count on each index reset if not an endless playlist
      }
    ...
    playlistEntryDur = playlistEntries[playlistIndex].dur > 0 ? playlistEntries[playlistIndex].dur
                                                              : UINT32_MAX;  // UINT32_MAX means infinite
...
void serializePlaylist(JsonObject sObj) {
  ...
  playlist["repeat"]    = (playlistIndex < 0 && playlistRepeat > 0)
                              ? playlistRepeat - 1
                              : playlistRepeat;  // remove added repetition count (if not yet running)
