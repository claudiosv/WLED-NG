...
// See
// https://docs.espressif.com/projects/esp-idf/en/latest/esp32/hw-reference/esp32/get-started-ethernet-kit-v1.1.html#ip101gri-phy-interface
...
bool initEthernet() {
  ...
  if (successfullyConfiguredEthernet) {
    // DEBUG_PRINTLN("initE: ETH already successfully configured, ignoring");
    return false;
  }
  ...
  if (!PinManager::allocateMultiplePins(pinsToAllocate, 10, PinOwner::Ethernet)) {
    DEBUG_PRINTLN("initE: Failed to allocate ethernet pins");
    return false;
  }
...
*/
  ...
  if (!ETH.begin((uint8_t)es.eth_address, (int)es.eth_power, (int)es.eth_mdc, (int)es.eth_mdio,
                 (eth_phy_type_t)es.eth_type, (eth_clock_mode_t)es.eth_clk_mode)) {
  ...
  DEBUG_PRINTLN("initE: *** Ethernet successfully configured! ***");
  return true;
}
#endif
...
void fillMAC2Str(char* str, const uint8_t* mac) {
  sprintf(str, "%02x%02x%02x%02x%02x%02x", MAC2STR(mac));
  byte nul = 0;
  for (int i = 0; i < 6; i++) {
    nul |= *mac++;  // do we have 0
  }
  if (!nul) {
...
int findWiFi(bool doScan) {
  ...
  int status =
      WiFi.scanComplete();  // complete scan may take as much as several seconds (usually <6s with not very crowded air)
  ...
  if (doScan || status == WIFI_SCAN_FAILED) {
    ...
    WiFi.scanNetworks(true);  // start scanning in asynchronous mode (will delete old scan)
             ...
             0) {  // status contains number of found networks (including duplicate SSIDs with different BSSID)
...
bool isWiFiConfigured() {
  return multiWiFi.size() > 1 ||
         (strlen(multiWiFi[0].clientSSID) >= 1 && strcmp(multiWiFi[0].clientSSID, DEFAULT_CLIENT_SSID) != 0);
}
...
void WiFiEvent(WiFiEvent_t event) {
  switch (event) {
    case ARDUINO_EVENT_WIFI_AP_STADISCONNECTED:
      ...
      if (--apClients == 0 && isWiFiConfigured()) {
        forceReconnect = true;  // no clients reconnect WiFi if awailable
      }
    ...
    case ARDUINO_EVENT_WIFI_SCAN_DONE:
      ...
      DEBUG_PRINTLN("WiFi-E: SSID scan completed.");
      break;
    case ARDUINO_EVENT_WIFI_AP_START:
      DEBUG_PRINTLN("WiFi-E: AP Started");
      break;
    ...
    case ARDUINO_EVENT_ETH_CONNECTED: {
      DEBUG_PRINTLN("ETH-E: Connected");
      if (!apActive) {
        WiFi.disconnect(true);  // disable WiFi entirely
      }
      char hostname[64] = {
          '\0'};  // any "hostname" within a Fully Qualified Domain Name (FQDN) must not exceed 63 characters
    ...
    case ARDUINO_EVENT_ETH_DISCONNECTED:
      DEBUG_PRINTLN("ETH-E: Disconnected");
