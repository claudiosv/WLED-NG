...
static bool     checkBoundsAndWrap(int32_t &position, int32_t max, int32_t particleradius,
                                   bool wrap);  // returns false if out of bounds by more than particleradius
static uint32_t fast_color_scaleAdd(
    uint32_t c1, uint32_t c2,
    uint8_t scale = 255);  // fast and accurate color adding with scaling (scales c2 before adding)
#endif

#ifndef WLED_DISABLE_PARTICLESYSTEM2D
ParticleSystem2D::ParticleSystem2D(uint32_t width, uint32_t height, uint32_t numberofparticles,
                                   uint32_t numberofsources, bool isadvanced, bool sizecontrol) {
  ...
  updatePSpointers(
      isadvanced,
      sizecontrol);       // set the particle and sources pointer (call this before accessing sprays or particles)
  ...
  perParticleSize =
      isadvanced;  // enable per particle size by default if using advanced properties (FX can disable if needed)
}
...
void ParticleSystem2D::update() {
  ...
  if (advPartSize != nullptr) {
    for (uint32_t i = 0; i < usedParticles; i++) {
      if (!updateSize(&advPartProps[i], &advPartSize[i])) {  // if particle shrinks to 0 size
  ...
  // causing a crash if using local buffer for speed)
  ...
  // move all particles
  for (uint32_t i = 0; i < usedParticles; i++) {
    particleMoveUpdate(particles[i], particleFlags[i], nullptr,
                       advPartProps ? &advPartProps[i]
                                    : nullptr);  // note: splitting this into two loops is slower and uses more flash
  }
...
void ParticleSystem2D::setUsedParticles(uint8_t percentage) {
  usedParticles = max(static_cast<uint32_t>(1), (numParticles * (static_cast<int>(percentage) + 1)) >>
                                       8);  // number of particles to use (percentage is 0-255, 255 = 100%)
...
void ParticleSystem2D::setParticleSize(uint8_t size) {
  ...
  if (particlesize > 1) {
    particleHardRadius =
        PS_P_MINHARDRADIUS + ((particlesize * 52) >> 6);  // use 1 pixel + 80% of size for hard radius (slight overlap
                                                          // with boarders so they do not "float" and nicer stacking)
...
// force is in 3.4 fixed point notation so force=16 means apply v+1 each frame default of 8 is every other frame (gives
// good results)
...
void ParticleSystem2D::enableParticleCollisions(
    bool enable, uint8_t hardness) {  // enable/disable gravity, optionally, set the force (force=8 is default) can be
                                      // 1-255, 0 is also disable
...
int32_t ParticleSystem2D::angleEmit(PSsource &emitter, const uint16_t angle, const int32_t speed) {
  emitter.vx =
      (static_cast<int32_t>(cos16_t(angle)) * speed) / static_cast<int32_t>(32600);  // cos16_t() and sin16_t() return signed 16bit, division
                                                           // should be 32767 but 32600 gives slightly better rounding
  emitter.vy = (static_cast<int32_t>(sin16_t(angle)) * speed) /
               static_cast<int32_t>(32600);  // note: cannot use bit shifts as bit shifting is asymmetrical (1>>1=0 / -1>>1=-1) and
...
// uses passed settings to set bounce or wrap, if useGravity is enabled, it will never bounce at the top and
// killoutofbounds is not applied over the top
void ParticleSystem2D::particleMoveUpdate(PSparticle &part, PSparticleFlags &partFlags, PSsettings2D *options,
  ...
  if (part.ttl > 0) {
    ...
    if (options->colorByAge) {
      part.hue = min(part.ttl, static_cast<uint16_t>(255));  // set color to ttl
    }

    int32_t renderradius = PS_P_HALFRADIUS - 1 +
                           particlesize;  // used to check out of bounds, if its more than half a radius out of bounds,
                                          // it will render to x = -2/-1 or x=max/max+1 in standard 2x2 rendering
    int32_t newX          = part.x + static_cast<int32_t>(part.vx);
    int32_t newY          = part.y + static_cast<int32_t>(part.vy);
    partFlags.outofbounds = false;  // reset out of bounds (in case particle was created outside the matrix and is now
                                    // moving into view) note: moving this to checks below adds code and is not faster

    if (perParticleSize && advancedproperties != nullptr) {  // using individual particle size
      renderradius =
          PS_P_HALFRADIUS - 1 +
          advancedproperties
              ->size;  // note: single pixel particles should be zero but OOB checks in rendering function handle this
      if (advancedproperties->size > 0) {
        particleHardRadius =
            PS_P_MINHARDRADIUS +
            ((advancedproperties->size * 52) >>
             6);  // use 1 pixel + 80% of size for hard radius (slight overlap with boarders so they do not "float")
      } else {    // single pixel particles use half the collision distance for walls
        particleHardRadius = PS_P_MINHARDRADIUS >> 1;
      }
    }
    // note: if wall collisions are enabled, bounce them before they reach the edge, it looks much nicer if the particle
    // does not go half out of view
    if (options->bounceY) {
      if ((newY < static_cast<int32_t>(particleHardRadius)) ||
          ((newY > static_cast<int32_t>(maxY - particleHardRadius)) && !options->useGravity)) {  // reached floor / ceiling
        bounce(part.vy, part.vx, newY, maxY);
      }
    }

    if (!checkBoundsAndWrap(newY, maxY, renderradius,
                            options->wrapY)) {  // check out of bounds  note: this must not be skipped. if gravity is
                                                // enabled, particles will never bounce at the top
    ...
    if (part.ttl) {  // check x direction only if still alive
      if (options->bounceX) {
        if ((newX < static_cast<int32_t>(particleHardRadius)) || (newX > static_cast<int32_t>(maxX - particleHardRadius))) {  // reached a wall
...
void ParticleSystem2D::fireParticleupdate() const {
  for (uint32_t i = 0; i < usedParticles; i++) {
    if (particles[i].ttl > 0) {
      particles[i].ttl--;  // age
      int32_t newY = particles[i].y + static_cast<int32_t>(particles[i].vy) +
                     (particles[i].ttl >> 2);  // younger particles move faster upward as they are hotter
      int32_t newX = particles[i].x + static_cast<int32_t>(particles[i].vx);
      particleFlags[i].outofbounds =
          0;  // reset out of bounds flag  note: moving this to checks below is not faster but adds code
      // check if particle is out of bounds, wrap x around to other side if wrapping is enabled
      // as fire particles start below the frame, lots of particles are out of bounds in y direction. to improve speed,
      // only check x direction if y is not out of bounds
      if (newY < -PS_P_HALFRADIUS) {
        ...
        particles[i].ttl = 0;
      } else  // particle is in frame in y direction, also check x direction now Note: using checkBoundsAndWrap() is
              // slower, only saves a few bytes
      {
        if ((newX < 0) || (newX > maxX)) {  // handle out of bounds & wrap
          if (particlesettings.wrapX) {
            newX = newX % (maxX + 1);
            if (newX < 0) {  // handle negative modulo
              newX += maxX + 1;
            }
          } else if ((newX < -PS_P_HALFRADIUS) || (newX > static_cast<int32_t>(maxX + PS_P_HALFRADIUS))) {  // if fully out of view
            particles[i].ttl = 0;
          }
        }
        particles[i].x = newX;
...
bool ParticleSystem2D::updateSize(PSadvancedParticle *advprops, PSsizeControl *advsize) {
  ...
  if (advsize->wobble) {
    advsize->asymdir +=
        advsize->wobblespeed;  // note: if need better wobblespeed control a counter is already in the struct
  }
  return true;
}
...
void ParticleSystem2D::getParticleXYsize(PSadvancedParticle *advprops, PSsizeControl *advsize, uint32_t &xsize,
                                         uint32_t &ysize) {
  if (advsize ==
      nullptr) {  // if advsize is valid, also advanced properties pointer is valid (handled by updatePSpointers())
    return;
  }
  int32_t size      = advprops->size;
  int32_t asymdir   = advsize->asymdir;
  int32_t deviation = ((static_cast<uint32_t>(size) * static_cast<uint32_t>(advsize->asymmetry)) + 255) >> 8;  // deviation from symmetrical size
  ...
  xsize = min((size - deviation), static_cast<int32_t>(255));
  ysize = min((size + deviation), static_cast<int32_t>(255));
  ;
}
...
void ParticleSystem2D::bounce(int8_t &incomingspeed, int8_t &parallelspeed, int32_t &position,
                              const uint32_t maxposition) const {
  ...
  if (position < static_cast<int32_t>(particleHardRadius)) {
    position =
        particleHardRadius;  // fast particles will never reach the edge if position is inverted, this looks better
  } else {
    position = maxposition - particleHardRadius;
  }
  if (wallRoughness) {
    int32_t incomingspeed_abs = abs(static_cast<int32_t>(incomingspeed));
    int32_t totalspeed        = incomingspeed_abs + abs(static_cast<int32_t>(parallelspeed));
    // transfer an amount of incomingspeed speed to parallel speed
    int32_t donatespeed = ((hw_random16(incomingspeed_abs << 1) - incomingspeed_abs) * static_cast<int32_t>(wallRoughness)) /
                          static_cast<int32_t>(255);  // take random portion of + or - perpendicular speed, scaled by roughness
...
void ParticleSystem2D::applyForce(const int8_t xforce, const int8_t yforce) {
  // for small forces, need to use a delay counter
  uint8_t tempcounter;
  // note: this is not the most computationally efficient way to do this, but it saves on duplicate code and is fast
  // enough
  for (uint32_t i = 0; i < usedParticles; i++) {
...
// caller needs to provide a 8bit counter that holds its value between calls (if using single particles, a counter for
// each particle is needed) angle is from 0-65535 (=0-360deg) angle = 0 means in positive x-direction (i.e. to the
// right) force is in 3.4 fixed point notation so force=16 means apply v+1 each frame (useful force range is +/- 127)
void ParticleSystem2D::applyAngleForce(PSparticle &part, const int8_t force, const uint16_t angle, uint8_t &counter) {
  ...
  int8_t yforce =
      (static_cast<int32_t>(force) * sin16_t(angle)) / 32767;  // note: cannot use bit shifts as bit shifting is asymmetrical (1>>1=0
                                                  // / -1>>1=-1) and this needs to be accurate!
...
void ParticleSystem2D::applyGravity() {
  ...
  for (uint32_t i = 0; i < usedParticles; i++) {
    // Note: not checking if particle is dead is faster as most are usually alive and if few are alive, rendering is
    // fast anyways
...
// slow down particle by friction, the higher the speed, the higher the friction. a high friction coefficient slows them
// more (255 means instant stop) note: a coefficient smaller than 0 will speed them up (this is a feature, not a bug),
// coefficient larger than 255 inverts the speed, so don't do that
...
#if defined(CONFIG_IDF_TARGET_ESP32C3)  // use bitshifts with rounding instead of division (2x faster)
  int32_t friction = 256 - coefficient;
  part.vx = ((int32_t)part.vx * friction + (((int32_t)part.vx >> 31) & 0xFF)) >>
            8;  // note: (v>>31) & 0xFF)) extracts the sign and adds 255 if negative for correct rounding using shifts
...
// note: not checking if particle is dead is faster as most are usually alive and if few are alive, rendering is fast
// anyways
void ParticleSystem2D::applyFriction(const int32_t coefficient) const {
#if defined(CONFIG_IDF_TARGET_ESP32C3)  // use bitshifts with rounding instead of division (2x faster)
  int32_t friction = 256 - coefficient;
  for (uint32_t i = 0; i < usedParticles; i++) {
    particles[i].vx =
        ((int32_t)particles[i].vx * friction + (((int32_t)particles[i].vx >> 31) & 0xFF)) >>
        8;  // note: (v>>31) & 0xFF)) extracts the sign and adds 255 if negative for correct rounding using shifts
...
void ParticleSystem2D::pointAttractor(const uint32_t particleindex, PSparticle &attractor, const uint8_t strength,
  ...
  int8_t  yforce = (force * dy) / 1024;  // note: cannot use bit shifts as bit shifting is asymmetrical (1>>1=0 /
                                         // -1>>1=-1) and this needs to be accurate!
...
// warning: do not render out of bounds particles or system will crash! rendering does not check if particle is out of
// bounds firemode is only used for PS Fire FX
void ParticleSystem2D::render() {
  ...
  if (motionBlur) {  // motion-blurring active
    for (int32_t y = 0; y <= maxYpixel; y++) {
      ...
      for (int32_t x = 0; x <= maxXpixel; x++) {
        framebuffer[index] = fast_color_scale(framebuffer[index],
                                              motionBlur);  // note: could skip if only globalsmear is active but
                                                            // usually they are both active and scaling is fast enough
        index++;
      }
    }
  ...
  for (uint32_t i = 0; i < usedParticles; i++) {
    ...
    if (fireIntesity) {  // fire mode
      ...
      brightness = min(brightness, static_cast<uint32_t>(255));
...
void WLED_O2_ATTR ParticleSystem2D::renderParticle(const uint32_t particleindex, const uint8_t brightness,
  ...
  if (perParticleSize && advPartProps != nullptr) {  // use advanced size properties
    size =
        1 +
        advPartProps[particleindex].size;  // add 1 to avoid single pixel size particles (collisions do not support it)
  }
  ...
  if (size == 0) {  // single pixel rendering
    ...
    if (x <= static_cast<uint32_t>(maxXpixel) && y <= static_cast<uint32_t>(maxYpixel)) {
      uint32_t index =
          x + ((maxYpixel - y) *
                  (maxXpixel + 1));  // flip y coordinate (0,0 is bottom left in PS but top left in framebuffer)
  ...
  struct {
    int32_t x, y;
  } pixco[4];  // particle pixel coordinates, the order is bottom left [0], bottom right[1], top right [2], top left [3]
               // (thx @blazoncek for improved readability struct)
  bool pixelvalid[4] = {true, true, true, true};  // is set to false if pixel is out of bounds

  // add half a radius as the rendering algorithm always starts at the bottom left, this leaves things positive, so
  // shifts can be used, then shift coordinate by a full pixel (x--/y-- below) if sub-pixel position is
  // 0-PS_P_HALFRADIUS it will render to x>>PS_P_RADIUS_SHIFT as the right pixel
  ...
  int32_t x =
      (xoffset >>
       PS_P_RADIUS_SHIFT);  // divide by PS_P_RADIUS which is 64, so can bitshift (compiler can not optimize integer)
  ...
  // precalculate values for speed optimization. Note: rounding is not perfect but close enough, some inaccuracy is
  // traded for speed
  ...
  pxlbrightness[0] =
      (precal1 * precal2) >>
      PS_P_SURFACE;  // bottom left value equal to ((PS_P_RADIUS - dx) * (PS_P_RADIUS-dy) * brightness) >> PS_P_SURFACE
  pxlbrightness[1] = (dx * precal2) >>
                     PS_P_SURFACE;  // bottom right value equal to (dx * (PS_P_RADIUS-dy) * brightness) >> PS_P_SURFACE
  pxlbrightness[2] = (dx * precal3) >> PS_P_SURFACE;  // top right value equal to (dx * dy * brightness) >> PS_P_SURFACE
  pxlbrightness[3] = (precal1 * precal3) >>
                     PS_P_SURFACE;  // top left value equal to ((PS_P_RADIUS-dx) * dy * brightness) >> PS_P_SURFACE
  ...
  if (gammaCorrectCol) {
    for (unsigned char & pxlbrightnes : pxlbrightness) {
      pxlbrightnes = gamma8inv(pxlbrightnes);  // use look-up-table for invers gamma
    }
  }
  ...
  } else if (pixco[1].x > maxXpixel) {  // right pixels, only has to be checked if left pixel is in frame
    if (wrapX) {                                 // wrap y to the other side if required
      pixco[1].x = pixco[2].x = 0;
    } else {
      pixelvalid[1] = pixelvalid[2] = false;  // out of bounds
      if (pixco[0].x > maxXpixel) {
  ...
  for (uint32_t i = 0; i < 4; i++) {
    if (pixelvalid[i]) {
      uint32_t idx =
          pixco[i].x + ((maxYpixel - pixco[i].y) *
                           (maxXpixel + 1));  // flip y coordinate (0,0 is bottom left in PS but top left in framebuffer)
      framebuffer[idx] = fast_color_scaleAdd(
          framebuffer[idx], color, pxlbrightness[i]);  // order is: bottom left, bottom right, top right, top left
    }
  }
}

// render particle as ellipse/circle with linear brightness falloff and sub-pixel precision
void WLED_O2_ATTR ParticleSystem2D::renderLargeParticle(const uint32_t size, const uint32_t particleindex,
  ...
  // when calculating the dx, we need to take this into account: at x = 128 the x offset is 1, the pixel center is at
  // pixel 2: for pixel 1, dx = 1 * PS_P_RADIUS - 128 = -64 but the center of the pixel is actually only -32 from the
  // particle center so need to add half a radius: dx = pixel_x * PS_P_RADIUS - x_subcenter + PS_P_HALFRADIUS
  ...
  int32_t rx_subpixel =
      xsize + PS_P_RADIUS +
      1;  // size = 1 means radius of just over 1 pixel, + PS_P_RADIUS (+1 to accoutn for bit-shift loss)
  ...
  int32_t x_min =
      x_center - rx_pixels;  // note: the "+1" extension needed for 1D is not required for 2D, it is smooth as-is
  ...
  for (int32_t py = y_min; py <= y_max; py++) {
    for (int32_t px = x_min; px <= x_max; px++) {
      ...
      // apply inverse gamma correction if needed, if this is skipped, particles flicker due to changing total
      // brightness
      if (gammaCorrectCol) {
        pixel_brightness = gamma8inv(
            pixel_brightness);  // invert brigthess so brightness distribution is linear after gamma correction
      }
      // Render pixel
      uint32_t idx =
          render_x +
          ((maxYpixel - render_y) * matrixX);  // flip y coordinate (0,0 is bottom left in PS but top left in framebuffer)
...
// uses binning by dividing the frame into slices in x direction which is efficient if using gravity in y direction (but
// less efficient for FX that use forces in x direction) for code simplicity, no y slicing is done, making very tall
// matrix configurations less efficient note: also tested adding y slicing, it gives diminishing returns, some FX even
// get slower. FX not using gravity would benefit with a 10% FPS improvement
void ParticleSystem2D::handleCollisions() {
  uint32_t collDistSq =
      particleHardRadius
      << 1;  // distance is double the radius note: particleHardRadius is updated when setting global particle size
  ...
  // if they are, collisionStartIdx is increased so each particle collides at least every second frame (which still
  // gives decent collisions)
  ...
  if (perParticleSize && advPartProps != nullptr) {
    overlap = 512;  // max overlap for collision detection if using per-particle size, enough to catch all particles
                    // even at max speed
  }

  uint32_t maxBinParticles = max(
      static_cast<uint32_t>(50),
      (usedParticles + 1) /
          2);  // assume no more than half of the particles are in the same bin, do not bin small amounts of particles
  ...
  uint16_t binIndices[maxBinParticles];  // creat array on stack for indices, 2kB max for 1024 particles
                                         // (ESP32_MAXPARTICLES/2)
  uint32_t binParticleCount;             // number of particles in the current bin
  uint32_t nextFrameStartIdx =
      hw_random16(usedParticles);  // index of the first particle in the next frame (set to fixed value if bin overflow)
  ...
  for (uint32_t bin = 0; bin < numBins; bin++) {
    ...
    int32_t binStart =
        (bin * binWidth) -
        overlap;  // note: first bin will extend to negative, but that is ok as out of bounds particles are ignored
    int32_t binEnd =
        binStart + binWidth +
        (overlap
         << 1);  // add twice the overlap as start is start-overlap, note: last bin can be out of bounds, see above;

    // fill the binIndices array for this bin
    for (uint32_t i = 0; i < usedParticles; i++) {
      if (particles[pidx].ttl > 0) {  // is alive
        if (particles[pidx].x >= binStart &&
            particles[pidx].x <= binEnd) {  // >= and <= to include particles on the edge of the bin (overlap to ensure
                                            // boarder particles collide with adjacent bins)
          if (particleFlags[pidx].outofbounds == 0 &&
              particleFlags[pidx].collide) {  // particle is in frame and does collide note: checking flags is quite
                                              // slow and usually these are set, so faster to check here
            if (binParticleCount >=
                maxBinParticles) {  // bin is full, more particles in this bin so do the rest next frame
              nextFrameStartIdx =
                  pidx;  // bin overflow can only happen once as bin size is at least half of the particles (or half +1)
              break;
            }
    ...
    int32_t massratio2 = 0;  // TODO: claudio - if implementing "fixed" particles, set to 1 (fixed) and 255 (movable)
    for (uint32_t i = 0; i < binParticleCount;
         i++) {  // go though all 'higher number' particles in this bin and see if any of those are in close proximity
                 // and if they are, make them collide
      ...
      for (uint32_t j = i + 1; j < binParticleCount; j++) {  // check against higher number particles
        ...
        if (perParticleSize && advPartProps != nullptr) {  // using individual particle size
          collDistSq = (PS_P_MINHARDRADIUS << 1) +
                       (((static_cast<uint32_t>(advPartProps[idx_i].size) + static_cast<uint32_t>(advPartProps[idx_j].size)) * 52) >>
                        6);  // collision distance, use 80% of size for tighter stacking (slight overlap)
          ...
          massratio1 = (mass2 << 8) / totalmass;  // massratio 1 depends on mass of particle 2, i.e. if 2 is heavier ->
                                                  // higher velocity impact on 1
          massratio2 = (mass1 << 8) / totalmass;
        }
        // note: using the same logic as in 1D is much slower though it would be more accurate but it is not really
        // needed in 2D: particles slipping through each other is much less visible
        int32_t dx = (particles[idx_j].x + particles[idx_j].vx) -
                     (particles[idx_i].x + particles[idx_i].vx);  // distance with lookahead
        if (dx * dx < collDistSq) {  // check x direction, if close, check y direction (squaring is faster than abs() or
                                     // dual compare)
          int32_t dy = (particles[idx_j].y + particles[idx_j].vy) -
                       (particles[idx_i].y + particles[idx_i].vy);  // distance with lookahead
...
// 255 means full hard)
void WLED_O2_ATTR ParticleSystem2D::collideParticles(PSparticle &particle1, PSparticle &particle2, int32_t dx,
                                                     int32_t dy, const uint32_t collDistSq, int32_t massratio1,
                                                     int32_t massratio2) {
  int32_t distanceSquared = (dx * dx) + (dy * dy);
  // Calculate relative velocity note: could zero check but that does not improve overall speed but deminish it as that
  // is rarely the case and pushing is still required
  ...
  // if dx and dy are zero (i.e. same position) give them an offset, if speeds are also zero, also offset them (pushes
  // particles apart if they are clumped before enabling collisions)
  ...
  if (dotProduct < 0) {  // particles are moving towards each other
    ...
    // overflow check: dx/dy are 7bit, relativV are 8bit -> dotproduct is 15bit, dotproduct/distsquared ist 8b,
    // multiplied by collisionhardness of 8bit. so a 16bit shift is ok, make it 15 to be sure no overflows happen note:
    // cannot use right shifts as bit shifting in right direction is asymmetrical (1>>1=0 / -1>>1=-1) and this needs to
    // be accurate! the trick is: only shift positive numers Calculate new velocities after collision
    int32_t surfacehardness =
        max(collisionHardness, static_cast<int32_t>(PS_P_MINSURFACEHARDNESS) >>
                                   1);  // if particles are soft, the impulse must stay above a limit or collisions slip
                                        // through at higher speeds, 170 seems to be a good value
    int32_t impulse =
        (((((-dotProduct) << 15) / distanceSquared) * surfacehardness) >>
         8);  // note: inverting before bitshift corrects for asymmetry in right-shifts (is slightly faster)

#if defined(CONFIG_IDF_TARGET_ESP32C3)  // use bitshifts with rounding instead of division (2x faster)
    int32_t ximpulse =
        (impulse * dx + ((dx >> 31) & 0x7FFF)) >>
        15;  // note: extracting sign bit and adding rounding value to correct for asymmetry in right shifts
    ...
    if (massratio1) {
      int32_t vx1 = static_cast<int32_t>(particle1.vx) -
                    ((ximpulse * massratio1) >> 7);  // mass ratio is in fixed point 8bit, multiply by two to account
                                                     // for the fact that we distribute the impulse to both particles
    ...
    if (collisionHardness < PS_P_MINSURFACEHARDNESS &&
        (SEGMENT.call & 0x07) == 0) {  // if particles are soft, they become 'sticky' i.e. apply some friction (they do
                                       // pile more nicely and stop sloshing around)
...
// Note: could call applyFriction, but this is faster and speed is key here
#if defined(CONFIG_IDF_TARGET_ESP32C3)  // use bitshifts with rounding instead of division (2x faster)
      particle1.vx =
          ((int32_t)particle1.vx * coeff + (((int32_t)particle1.vx >> 31) & 0xFF)) >>
          8;  // note: (v>>31) & 0xFF)) extracts the sign and adds 255 if negative for correct rounding using shifts
  ...
  // tried lots of configurations, what works best is to give one particle a little velocity. When adding hard pushing
  // things tend to oscillate. when hard pushing by offsetting position without velocity, they tend to sink into each
  // other under gravity. when using hard-pushing and velocity, there are some oscillations and softer particles do not
  // pile nicely. oscillation get worse if pushing both particles so one is chosen somewhat randomly. softer collisions
  // are not perfect on purpose: soft particles should pile up and overlap slightly, if separation is made perfect, it
  // does not have the intended look

  if (distanceSquared < collDistSq &&
      ((relativeVx * relativeVx) + (relativeVy * relativeVy) < 50)) {  // too close and also slow, push them apart
    bool fairlyrandom =
        (dotProduct & 0x01) != 0;  // dotprouct LSB should be somewhat random, so no need to calculate a random number
    int32_t pushamount = 1 + ((collDistSq - distanceSquared) >>
                              13);  // found this by experimentation: it means push by 1, push more if overlapping more
                                    // than 1.4 physical pixels (i.e. larger particles only)
...
void ParticleSystem2D::updateSystem() {
  ...
  updatePSpointers(advPartProps != nullptr,
                   advPartSize != nullptr);  // update pointers to PS data, also updates availableParticles
  // PSPRINTLN("\n END update System2D, running FX...");
}

// set the pointers for the class (this only has to be done once and not on every FX call, only the class pointer needs
// to be reassigned to SEGENV.data every time) function returns the pointer to the next byte available for the FX (if it
// assigned more memory for other stuff using the above allocate function) FX handles the PSsources, need to tell this
// function how many there are
void ParticleSystem2D::updatePSpointers(bool isadvanced, bool sizecontrol) {
  ...
  //  a pointer MUST be 4 byte aligned. sizeof() in a struct/class is always aligned to the largest element. if it
  //  contains a 32bit, it will be padded to 4 bytes, 16bit is padded to 2byte alignment. The PS is aligned to 4 bytes,
  //  a PSparticle is aligned to 2 and a struct containing only byte sized variables is not aligned at all and may need
  //  to be padded when dividing the memoryblock. by making sure that the number of sources and particles is a multiple
  //  of 4, padding can be skipped here as alignent is ensured, independent of struct sizes.
  ...
  sources       = reinterpret_cast<PSsource *>(particleFlags +
                                               numParticles);  // pointer to source(s) at data+sizeof(ParticleSystem2D)
  framebuffer   = SEGMENT.getPixels();                         // pointer to framebuffer
  PSdataEnd =
      reinterpret_cast<uint8_t *>(sources + numSources);  // pointer to first available byte after the PS for FX
                                                          // additional data (already aligned to 4 byte boundary)
  if (isadvanced) {
...
uint32_t calculateNumberOfParticles2D(uint32_t const pixels, const bool isadvanced, const bool sizecontrol) {
  ...
  numberofParticles          = max(static_cast<uint32_t>(4), min(numberofParticles, particlelimit));  // limit to 4 - particlelimit
  ...
  if (sizecontrol) {         // advanced property array needs ram, reduce number of particles
    numberofParticles /= 8;  // if advanced size control is used, much fewer particles are needed note: if changing this
                             // number, adjust FX using this accordingly
  }
...
uint32_t calculateNumberOfSources2D(uint32_t pixels, uint32_t requestedsources) {
  int numberofSources = min(pixels / SOURCEREDUCTIONFACTOR, requestedsources);
...
// allocate memory for particle system class, particles, sprays plus additional memory requested by FX //TODO: add
// percentofparticles like in 1D to reduce memory footprint of some FX?
bool allocateParticleSystemMemory2D(uint32_t numparticles, uint32_t numsources, bool isadvanced, bool sizecontrol,
                                    uint32_t additionalbytes) {
  PSPRINTLN("PS 2D alloc");
  PSPRINTLN("numparticles:" + String(numparticles) + " numsources:" + String(numsources) +
            " additionalbytes:" + String(additionalbytes));
...
// initialize Particle System, allocate additional bytes if needed (pointer to those bytes can be read from particle
// system class: PSdataEnd)
bool initParticleSystem2D(ParticleSystem2D *&PartSys, uint32_t requestedsources, uint32_t additionalbytes,
                          bool advanced, bool sizecontrol) {
  PSPRINT("PS 2D init ");
  if (!strip.isMatrix) {
    errorFlag = ERR_NOT_IMPL;  // TODO: claudio - need a better error code if more codes are added
    SEGMENT.deallocateData();  // deallocate any data to make sure data is null (there is no valid PS in data and data
                               // can only be checked for null)
  ...
  while (numparticles >= 5) {  // make sure we have at least 5 particles or quit
    ...
    numparticles =
        ((numparticles / 2) + 3) & ~0x03;  // cut number of particles in half and try again, must be 4 byte aligned
  ...
  PartSys = new (SEGENV.data)
      ParticleSystem2D(cols, rows, numparticles, numsources, advanced, sizecontrol);  // particle system constructor
...
ParticleSystem1D::ParticleSystem1D(uint32_t length, uint32_t numberofparticles, uint32_t numberofsources,
                                   bool isadvanced) {
  ...
  setSize(length);
  updatePSpointers(
      isadvanced);        // set the particle and sources pointer (call this before accessing sprays or particles)
  setWallHardness(255);   // set default wall hardness to max
  setGravity(0);          // gravity disabled by default
  ...
  perParticleSize = isadvanced;  // enable per particle size by default so FX do not need to set this explicitly. FX can
                                 // disable by setting global size.
  if (isadvanced) {
    for (uint32_t i = 0; i < numParticles; i++) {
...
void ParticleSystem1D::update() {
  // apply gravity globally if enabled
  if (particlesettings
          .useGravity) {  // note: in 1D system, applying gravity after collisions also works but may be worse
    applyGravity();
  }
  ...
  if (particlesettings.useCollisions) {
    handleCollisions();
    if (perParticleSize) {
      handleCollisions();  // second pass for per particle size (as impulse transfer can recoil at high speed, this
                           // improves "slip through" issues for small particles but is expensive)
    }
  }
...
void ParticleSystem1D::setParticleSize(const uint8_t size) {
  ...
  if (particlesize > 1) {
    particleHardRadius = PS_P_MINHARDRADIUS_1D +
                         ((particlesize * 52) >> 6);  // use 1 pixel + 80% of size for hard radius (slight overlap with
                                                      // boarders so they do not "float" and nicer stacking)
  } else if (particlesize == 0) {
...
int32_t ParticleSystem1D::sprayEmit(const PSsource1D &emitter) {
  for (uint32_t i = 0; i < usedParticles; i++) {
    ...
    if (particles[emitIndex].ttl == 0) {                                                   // find a dead particle
      ...
      particles[emitIndex].hue = emitter.source.hue;
      particles[emitIndex].ttl = hw_random16(emitter.minLife, emitter.maxLife);
      particleFlags[emitIndex].collide =
          emitter.sourceFlags
              .collide;  // TODO: claudio - could just set all flags (asByte) but need to check if that breaks any of the FX
...
// particle moves, decays and dies, if killoutofbounds is set, out of bounds particles are set to ttl=0
// uses passed settings to set bounce or wrap, if useGravity is set, it will never bounce at the top and killoutofbounds
// is not applied over the top
void ParticleSystem1D::particleMoveUpdate(PSparticle1D &part, PSparticleFlags1D &partFlags, PSsettings1D *options,
  ...
  if (part.ttl > 0) {
    ...
    int32_t renderradius =
        PS_P_HALFRADIUS_1D - 1 + particlesize;  // used to check out of bounds, default for 2 pixel rendering
    int32_t newX = part.x + static_cast<int32_t>(part.vx);
    partFlags.outofbounds =
        false;  // reset out of bounds (in case particle was created outside the matrix and is now moving into view)

    if (perParticleSize && advancedproperties != nullptr) {  // using individual particle size?
      renderradius = PS_P_HALFRADIUS_1D - 1 +
                     advancedproperties->size;  // note: for single pixel particles, it should be zero, but it does not
                                                // matter as out of bounds checking is done in rendering function
      if (advancedproperties->size > 1) {
        particleHardRadius = PS_P_MINHARDRADIUS_1D + ((advancedproperties->size * 52) >>
                                                      6);  // use 1 pixel + 80% of size for hard radius (slight overlap
                                                           // with boarders so they do not "float" and nicer stacking)
      } else {  // single pixel particles use half the collision distance for walls
    ...
    // if wall collisions are enabled, bounce them before they reach the edge, it looks much nicer if the particle is
    // not half out of view
    if (options->bounce) {
      if ((newX < static_cast<int32_t>(particleHardRadius)) || (newX > static_cast<int32_t>(maxX - particleHardRadius))) {  // reached a wall
        ...
        if (bouncethis) {
          ...
          part.vx =
              (static_cast<int32_t>(part.vx) * static_cast<int32_t>(wallHardness)) / 255;  // reduce speed as energy is lost on non-hard surface
          if (newX < static_cast<int32_t>(particleHardRadius)) {
            newX = particleHardRadius;  // fast particles will never reach the edge if position is inverted, this looks
                                        // better
          } else {
    ...
    if (!checkBoundsAndWrap(
            newX, maxX, renderradius,
            options->wrap)) {  // check out of bounds note: this must not be skipped or it can lead to crashes
      partFlags.outofbounds = true;
      if (options->killoutofbounds) {
        bool killthis = true;
    ...
    if (!partFlags.fixed) {
      ...
      part.vx =
          0;  // set speed to zero. note: particle can get speed in collisions, if unfixed, it should not speed away
    }
  }
}
...
void ParticleSystem1D::applyForce(PSparticle1D &part, const int8_t xforce, uint8_t &counter) {
  ...
  part.vx    = limitSpeed(static_cast<int32_t>(part.vx) + dv);  // apply the force to particle
}
...
void ParticleSystem1D::applyGravity() {
  ...
  for (uint32_t i = 0; i < usedParticles; i++) {
    ...
    // note: not checking if particle is dead is omitted as most are usually alive and if few are alive, rendering is
    // fast anyways
    particles[i].vx = limitSpeed(static_cast<int32_t>(particles[i].vx) - dv);
  }
}
...
// more (255 means instant stop) note: a coefficient smaller than 0 will speed them up (this is a feature, not a bug),
// coefficient larger than 255 inverts the speed, so don't do that
void ParticleSystem1D::applyFriction(int32_t coefficient) const {
#if defined(CONFIG_IDF_TARGET_ESP32C3)  // use bitshifts with rounding instead of division (2x faster)
  int32_t friction = 256 - coefficient;
  for (uint32_t i = 0; i < usedParticles; i++) {
    if (particles[i].ttl) {
...
// calculate pixel positions and brightness distribution and render the particle to local buffer or global buffer
void WLED_O2_ATTR ParticleSystem1D::renderParticle(const uint32_t particleindex, const uint8_t brightness,
  ...
  if (size == 0) {  // single pixel particle, can be out of bounds as oob checking is made for 2-pixel particles (and
                    // updating it uses more code)
    uint32_t x = particles[particleindex].x >> PS_P_RADIUS_SHIFT_1D;
    if (x <= static_cast<uint32_t>(maxXpixel)) {  // by making x unsigned there is no need to check < 0 as it will overflow
  ...
  // add half a radius as the rendering algorithm always starts at the bottom left, this leaves things positive, so
  // shifts can be used, then shift coordinate by a full pixel (x-- below)
  int32_t xoffset = particles[particleindex].x + PS_P_HALFRADIUS_1D;
  int32_t dx =
      xoffset & (PS_P_RADIUS_1D - 1);  // relativ particle position in subpixel space,  modulo replaced with bitwise AND
  int32_t x = xoffset >> PS_P_RADIUS_SHIFT_1D;  // divide by PS_P_RADIUS, bitshift of negative number stays negative ->
                                                // checking below for x < 0 works (but does not when using division)
  ...
  // calculate the brightness values for both pixels using linear interpolation (note: in standard rendering out of
  // frame pixels could be skipped but if checks add more clock cycles over all)
...
void WLED_O2_ATTR ParticleSystem1D::renderLargeParticle(const uint32_t size, const uint32_t particleindex,
  ...
  int32_t r_subpixel = size + PS_P_RADIUS_1D + 1;  // size = 255 is radius of 9, so add 33 -> 33+255=288, 288>>5=9
                                                   // pixels (i.e. the +1 is needed to correct for bitshift losses)
  // rendering bounding box in pixels
  ...
  for (int32_t px = x_min; px <= x_max; px++) {
    ...
    // if (pixel_brightness == 0) continue; // skip black pixels note: very few pixels will be black, skipping this is
    // usually faster
...
void ParticleSystem1D::handleCollisions() {
  ...
  uint32_t checkDistSq       = max(2 * PS_P_MAXSPEED, static_cast<int>(collisiondistance));
  if (perParticleSize && advPartProps != nullptr) {  // using individual particle size
    checkDistSq =
        max(2 * PS_P_MAXSPEED, (512 * 52) >> 6);  // max possible collision distance that catches all collisons
  }
  ...
  int32_t overlap =
      collisiondistance +
      (2 * PS_P_MAXSPEED);  // overlap bins to include edge particles to neighbouring bins (+ look-ahead of speed)
  ...
  uint32_t maxBinParticles =
      max(static_cast<uint32_t>(50), (usedParticles + 1) / 4);         // do not bin small amounts, limit max to 1/4 of particles
  ...
  for (uint32_t bin = 0; bin < numBins; bin++) {
    ...
    for (uint32_t i = 0; i < binParticleCount;
         i++) {  // go though all 'higher number' particles and see if any of those are in close proximity and if they
                 // are, make them collide
      uint32_t idx_i = binIndices[i];
      for (uint32_t j = i + 1; j < binParticleCount; j++) {  // check against higher number particles
        uint32_t idx_j = binIndices[j];
        ...
        if (dx_sq <= checkDistSq) {  // possible collision imminent, check properly note: this is slower than using
                                     // direct speed look-ahead (like in 2D) but more accurate and fast enough for 1D
...
void WLED_O2_ATTR ParticleSystem1D::collideParticles(uint32_t partIdx1, uint32_t partIdx2, int32_t dx,
                                                     uint32_t collisiondistance) {
  int32_t massratio1 = 0;  // 0 means dont use mass ratio (equal mass)
  int32_t massratio2 = 0;
  if (perParticleSize &&
      advPartProps != nullptr) {  // use advanced size properties, calculate collision distance and mass ratio
    collisiondistance = (PS_P_MINHARDRADIUS_1D * 2) +
                        (((static_cast<uint32_t>(advPartProps[partIdx1].size) + static_cast<uint32_t>(advPartProps[partIdx2].size)) * 52) >>
                         6);  // collision distance, use 80% of size for tighter stacking (slight overlap)
    // calculate mass ratio for collision response
    ...
    massratio1 =
        (mass2 << 8) /
        totalmass;  // massratio 1 depends on mass of particle 2, i.e. if 2 is heavier -> higher velocity impact on 1
    massratio2 = (mass1 << 8) / totalmass;
  }
  ...
  if (dotProduct < 0) {  // particles are moving towards each other
    uint32_t lookaheadDistance =
        collisiondistance + absdv;  // add look-ahead: if reaching collisiondistance in this frame, collide
    if (dx_abs <= lookaheadDistance) {
      // if one of the particles is fixed, invert the other particle's velocity and multiply by hardness, also set its
      // position to the edge of the fixed particle
      if (particleFlags[partIdx1].fixed) {
        particles[partIdx2].vx = -(particles[partIdx2].vx * collisionHardness) / 255;
        particles[partIdx2].x =
            particles[partIdx1].x +
            (dx < 0 ? -collisiondistance : collisiondistance);  // dv < 0 means particle2.x < particle1.x
        return;
      } if (particleFlags[partIdx2].fixed) {
      ...
      int32_t surfacehardness =
          max(collisionHardness, static_cast<int32_t>(PS_P_MINSURFACEHARDNESS_1D));  // if particles are soft, the impulse must stay
                                                                        // above a limit or collisions slip through
// Calculate new velocities after collision  note: not using dot product like in 2D as impulse is purely speed depnedent
#if defined(CONFIG_IDF_TARGET_ESP32C3)  // use bitshifts with rounding instead of division (2x faster)
      int32_t impulse =
          (dv * surfacehardness + ((dv >> 31) & 0xFF)) >>
          8;  // note: (v>>31) & 0xFF)) extracts the sign and adds 255 if negative for correct rounding using shifts
#else         // division is faster on ESP32, S2 and S3
      ...
      // if particles are not the same size, use a mass ratio. mass ratio is set to 0 if particles are the same size
      if (massratio1) {
        int vx1 = static_cast<int>(particles[partIdx1].vx) + ((impulse * massratio1) >> 7);  // mass ratio is in fixed point 8bit
      ...
      if (collisionHardness < PS_P_MINSURFACEHARDNESS_1D &&
          (SEGMENT.call & 0x07) == 0) {  // if particles are soft, they become 'sticky' i.e. apply some friction
        const uint32_t coeff = collisionHardness + (250 - PS_P_MINSURFACEHARDNESS_1D);
#if defined(CONFIG_IDF_TARGET_ESP32C3)  // use bitshifts with rounding instead of division (2x faster)
        particles[partIdx1].vx =
            ((int32_t)particles[partIdx1].vx * coeff + (((int32_t)particles[partIdx1].vx >> 31) & 0xFF)) >>
            8;  // note: (v>>31) & 0xFF)) extracts the sign and adds 255 if negative for correct rounding using shifts
        particles[partIdx2].vx =
            ((int32_t)particles[partIdx2].vx * coeff + (((int32_t)particles[partIdx2].vx >> 31) & 0xFF)) >> 8;
#else  // division is faster on ESP32, S2 and S3
  ...
  // particles have volume, push particles apart if they are too close
  // note: like in 2D, pushing by a distance makes softer piles collapse, giving particles speed prevents that and looks
  // nicer

  if (dx_abs < collisiondistance) {  // too close, force push particles so they dont collapse
    int32_t pushamount =
        1 + ((collisiondistance - dx_abs) >> 3);  // push by eighth of deviation (plus 1 to push at least a little),
                                                  // note: pushing too much leads to pass-throughs and more flickering
    int32_t addspeed = 1;
    ...
    bool fairlyrandom = (dotProduct & 0x01) != 0;
    if (fairlyrandom) {
...
void ParticleSystem1D::updatePSpointers(bool isadvanced) {
  // Note on memory alignment:
  // a pointer MUST be 4 byte aligned. sizeof() in a struct/class is always aligned to the largest element. if it
  // contains a 32bit, it will be padded to 4 bytes, 16bit is padded to 2byte alignment. The PS is aligned to 4 bytes, a
  // PSparticle is aligned to 2 and a struct containing only byte sized variables is not aligned at all and may need to
  // be padded when dividing the memoryblock. by making sure that the number of sources and particles is a multiple of
  ...
  if (SEGMENT.is2D() && SEGMENT.map1D2D) {
    ...
    PSdataEnd   = reinterpret_cast<uint8_t *>(
        framebuffer + SEGMENT.maxMappingLength());  // pointer to first available byte after the PS for FX additional
                                                    // data (still aligned to 4 byte boundary)
  } else {
#endif
  ...
  if (isadvanced) {
    ...
    PSdataEnd    = reinterpret_cast<uint8_t *>(advPartProps +
                                               numParticles);  // since numParticles is a multiple of 4, this is always
                                                               // aligned to 4 bytes. No need to add padding bytes here
  }
#ifdef WLED_DEBUG_PS
...
uint32_t calculateNumberOfParticles1D(const uint32_t fraction, const bool isadvanced) {
  ...
  // make sure it is a multiple of 4 for proper memory alignment (easier than using padding bytes)
  numberofParticles =
      (numberofParticles + 3) & ~0x03;  // note: with a separate particle buffer, this is probably unnecessary
  PSPRINTLN(" calc numparticles:" + String(numberofParticles));
  return numberofParticles;
}

uint32_t calculateNumberOfSources1D(const uint32_t requestedsources) {
  int numberofSources = max(1, min(static_cast<int>(requestedsources), MAXSOURCES_1D));  // limit
...
bool allocateParticleSystemMemory1D(const uint32_t numparticles, const uint32_t numsources, const bool isadvanced,
...
// initialize Particle System, allocate additional bytes if needed (pointer to those bytes can be read from particle
// system class: PSdataEnd) note: percentofparticles is in uint8_t, for example 191 means 75%, (deafaults to 255 or 100%
// meaning one particle per pixel), can be more than 100% (but not recommended, can cause out of memory)
bool initParticleSystem1D(ParticleSystem1D *&PartSys, const uint32_t requestedsources,
  ...
  PartSys = new (SEGENV.data)
      ParticleSystem1D(SEGMENT.virtualLength(), numparticles, numsources, advanced);  // particle system constructor
  return true;
}
...
// calculate the delta speed (dV) value and update the counter for force calculation (is used several times, function
// saves on codesize) force is in 3.4 fixedpoint notation, +/-127
...
static bool checkBoundsAndWrap(int32_t &position, const int32_t max, const int32_t particleradius, const bool wrap) {
  if (static_cast<uint32_t>(position) > static_cast<uint32_t>(max)) {  // check if particle reached an edge, cast to uint32_t to save negative
                                             // checking (max is always positive)
    if (wrap) {
      position = position %
                 (max + 1);  // note: cannot optimize modulo, particles can be far out of bounds when wrap is enabled
      if (position < 0) {
        position += max + 1;
      }
    } else if (((position < -particleradius) ||
                (position >
                 max + particleradius))) {  // particle is leaving boundaries, out of bounds if it has fully left
...
// this is a fast version for RGB color adding ignoring white channel (PS does not handle white) including scaling of
// second color note: function is mainly used to add scaled colors, so checking if one color is black is slower
static uint32_t fast_color_scaleAdd(const uint32_t c1, const uint32_t c2, const uint8_t scale) {
  constexpr uint32_t kMaskRb = 0x00FF00FF;  // red and blue mask
  constexpr uint32_t kMaskG  = 0x0000FF00;  // green mask

  uint32_t rb = c2 & kMaskRb;  // 0x00RR00BB
  uint32_t g  = c2 & kMaskG;   // 0x0000GG00
  // scale second color
