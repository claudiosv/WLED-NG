...
void longPressAction(uint8_t b) {
  if (!buttons[b].macroLongPress) {
    switch (b) {
      case 0:
        setRandomColor(colPri);
        colorUpdated(CALL_MODE_BUTTON);
        break;
      case 1:
        if (buttonBriDirection) {
          if (bri == 255) {
            break;  // avoid unnecessary updates to brightness
          }
          if (bri >= 255 - WLED_LONG_BRI_STEPS) {
            bri = 255;
          } else {
            bri += WLED_LONG_BRI_STEPS;
          }
        } else {
          if (bri == 1) {
            break;  // avoid unnecessary updates to brightness
          }
          if (bri <= WLED_LONG_BRI_STEPS) {
            bri = 1;
          } else {
            bri -= WLED_LONG_BRI_STEPS;
          }
...
#else
      if (digitalPinToTouchChannel(pin) >= 0 && touchRead(pin) <= touchThreshold) {
        return true;
      }
#endif
#endif
...
static void handleSwitch(uint8_t b) {
  ...
  if (millis() - buttons[b].pressedTime >
...
static void handleAnalog(uint8_t b) {
  ...
  if ((buttons[b].pin < 0) /*|| (digitalPinToAnalogChannel(buttons[b].pin) < 0)*/) {
    return;  // pin must support analog ADC - newer esp32 frameworks throw lots of warnings otherwise
  }
  ...
  if (buttons[b].type == BTN_TYPE_ANALOG_INVERTED) {
    aRead = 255 - aRead;
  }
  ...
  // This waits until strip finishes updating (why: strip was not updating at the start of handleButton() but may have
  // started during analogRead()?)
  ...
  if (!buttons[b].macroButton && !buttons[b].macroLongPress) {
    ...
    if (buttons[b].macroDoublePress >= 250) {
      ...
      effectPalette = constrain(
          effectPalette, 0, getPaletteCount() - 1);  // map is allowed to "overshoot", so we need to contrain the result
...
void handleButton() {
  ...
  if (strip.isUpdating() && (now - lastRun < ANALOG_BTN_READ_CYCLE + 1)) {
    return;  // don't interfere with strip update (unless strip is updating continuously, e.g. very long strips)
  }
  lastRun = now;

  for (unsigned b = 0; b < buttons.size(); b++) {
    if (buttons[b].pin < 0 || buttons[b].type == BTN_TYPE_NONE) {
      continue;
    }

    if (UsermodManager::handleButton(b)) {
      continue;  // did usermod handle buttons
    }

    if (buttons[b].type == BTN_TYPE_ANALOG ||
        buttons[b].type == BTN_TYPE_ANALOG_INVERTED) {  // button is not a button but a potentiometer
    ...
    // button is not momentary, but switch. This is only suitable on pins whose on-boot state does not matter (NOT
    // gpio0)
    if (buttons[b].type == BTN_TYPE_SWITCH || buttons[b].type == BTN_TYPE_TOUCH_SWITCH ||
    ...
    if (isButtonPressed(b)) {  // pressed
      ...
      if (buttons[b].macroButton && buttons[b].macroButton == buttons[b].macroLongPress &&
          buttons[b].macroButton == buttons[b].macroDoublePress) {
      ...
      if (buttons[b].macroButton && buttons[b].macroButton == buttons[b].macroLongPress &&
        ...
        if (dur > WLED_DEBOUNCE_THRESHOLD) {
          buttons[b].pressedBefore = false;  // debounce, blocks button for 50 ms once it has been released
        }
        continue;
      }
      ...
      if (b == 0 && dur > WLED_LONG_AP) {     // long press on button 0 (when released)
        ...
        // NOTE: this interferes with double click handling in usermods so usermod needs to implement full button
        // handling
        if (b != 1 && !buttons[b].macroDoublePress) {  // don't wait for double press on buttons without a default
                                                       // action if no double press macro set
          shortPressAction(b);
        } else {  // double press if less than 350 ms between current press and previous short press release
                  // (buttonWaitTime!=0)
          if (doublePress) {
            doublePressAction(b);
          } else {
