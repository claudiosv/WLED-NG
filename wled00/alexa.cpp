...
void alexaInit() {
  ...
  // up to 9 devices (added second, third, ... i.e. index 1 to 9) serve for switching on up to nine presets (preset IDs
  // 1 to 9 in WLED), names are identical as the preset names, switching off can be done by switching off any of them
  if (alexaNumPresets) {
    String name = "";
  ...
  if (m == EspalexaDeviceProperty::on) {
    if (dev->getId() == 0)  // Device 0 is for on/off or macros
    {
      if (!macroAlexaOn) {
        ...
        if (bri == 0) {
          dev->setValue(briLast);  // stop Alexa from complaining if macroAlexaOn does not actually turn on
        }
      }
