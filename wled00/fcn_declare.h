...
using wifi_config = struct WiFiConfig {
  ...
  WiFiConfig(const char* ssid = "", const char* pass = "", uint32_t ip = 0, uint32_t gw = 0,
             uint32_t subnet = 0x00FFFFFF  // little endian
             ,
...
// function, only used in e131.cpp void sendArtnetPollReply(ArtPollReply* reply, IPAddress ipAddress, uint16_t
// portAddress); // local function, only used in e131.cpp
...
void updateFSInfo();
void closeFile();
inline bool writeObjectToFileUsingId(const String& file, uint16_t id, const JsonDocument* content) {
  return writeObjectToFileUsingId(file.c_str(), id, content);
};
inline bool writeObjectToFile(const String& file, const char* key, const JsonDocument* content) {
...
void serializeSegment(const JsonObject& root, const Segment& seg, byte id, bool forPreset = false,
...
void   addTimer(uint8_t preset, uint8_t hour, int8_t minute, uint8_t weekdays, uint8_t monthStart = 1,
                uint8_t monthEnd = 12, uint8_t dayStart = 1, uint8_t dayEnd = 31);
...
inline bool applyTemporaryPreset() {
  return applyPreset(255);
};
void        savePreset(byte index, const char* pname = nullptr, JsonObject saveobj = JsonObject());
inline void saveTemporaryPreset() {
  savePreset(255);
};
...
uint8_t realtimeBroadcast(uint8_t type, IPAddress client, uint16_t length, const uint8_t* buffer, uint8_t bri = 255,
                          bool isRGBW = false);
...
class Usermod {
 ...
 public:
  ...
  virtual void appendConfigData(
      Print& settingsScript);  // helper function called from usermod settings page to add metadata for entry fields
  virtual void addToJsonState(JsonObject& obj) {
  }  // add JSON objects for WLED state
  virtual void addToJsonInfo(JsonObject& obj) {
  }  // add JSON objects for UI Info page
  ...
  }  // fired prior to and after unsuccessful firmware update
...
namespace usermod_manager {
  ...
  bool getUMData(um_data_t** um_data,
                 uint8_t     mod_id = USERMOD_ID_RESERVED);  // USERMOD_ID_RESERVED will poll all usermods
  void setup();
  void connected();
...
bool               getVal(JsonVariant elem, byte& val, byte vmin = 0,
                          byte vmax = 255);  // getVal supports inc/decrementing and random ("X~Y(r|[w]~[-][Z])" form)
...
inline uint8_t hw_random8(uint32_t upperlimit) {
  return (hw_random8() * upperlimit) >> 8;
};  // input range 0-255
...
}  // returns free heap (ESP.getFreeHeap() can include other memory types)
