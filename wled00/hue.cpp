...
void handleHue() {
  ...
  if (!WLED_CONNECTED || hueClient == nullptr || millis() - hueLastRequestSent < huePollIntervalMs) {
    return;
  }
  ...
  if (huePollingEnabled) {
    ...
    hueClient->close();
    if (hueError == HUE_ERROR_ACTIVE) {
      hueError = HUE_ERROR_INACTIVE;
    }
  }
}

void reconnectHue() {
  if (!WLED_CONNECTED || !huePollingEnabled) {
    return;
  }
...
void onHueData(void*  /*arg*/, AsyncClient*  /*client*/, void* data, size_t len) {
  ...
  if (str[0] == '[')  // is JSON array
  {
    ...
    if (error) {
      hueError = HUE_ERROR_JSON_PARSING;
      return;
    }
    ...
    if (hueErrorCode)  // hue bridge returned error
    {
      ...
      switch (hueErrorCode) {
        case 1:
          ...
          break;  // Unauthorized user
        case 3:
          huePollingEnabled = false;
          break;  // Invalid light ID
        case 101:
          hueAuthRequired = true;
          break;  // link button not presset
      }
      return;
    }
    ...
    switch (hueColormode) {
      ...
      case 2:
        if (hueHue != hueHueLast || hueSat != hueSatLast) {
          colorHStoRGB(hueHue, hueSat, colPri);
        }
        hueHueLast = hueHue;
        hueSatLast = hueSat;
        break;
