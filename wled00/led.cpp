...
void applyValuesToSelectedSegs() {
  for (unsigned i = 0; i < strip.getSegmentsNum(); i++) {
    ...
    if (effectSpeed != seg.speed) {
      seg.speed    = effectSpeed;
      stateChanged = true;
    }
    if (effectIntensity != seg.intensity) {
      seg.intensity = effectIntensity;
      stateChanged  = true;
    }
    if (effectPalette != seg.palette) {
      seg.setPalette(effectPalette);
    }
    if (effectCurrent != seg.mode) {
      seg.setMode(effectCurrent);
    }
    uint32_t col0 = RGBW32(colPri[0], colPri[1], colPri[2], colPri[3]);
    uint32_t col1 = RGBW32(colSec[0], colSec[1], colSec[2], colSec[3]);
    if (col0 != seg.colors[0]) {
      seg.setColor(0, col0);
    }
    if (col1 != seg.colors[1]) {
      seg.setColor(1, col1);
    }
  }
}
...
// called after every state changes, schedules interface updates, handles brightness transition and nightlight
// activation unlike colorUpdated(), does NOT apply any colors or FX to segments
void stateUpdated(byte callMode) {
  ...
  if (bri != briOld || stateChanged) {
    ...
    if (nightlightActive && !nightlightActiveOld && callMode != CALL_MODE_NOTIFICATION &&
  ...
  if (callMode != CALL_MODE_NO_NOTIFY && nightlightActive &&
      (nightlightMode == NL_MODE_FADE || nightlightMode == NL_MODE_COLORFADE)) {
    briNlT = bri;
  ...
  if (briT == 0) {
    if (callMode != CALL_MODE_NOTIFICATION) {
      strip.resetTimebase();  // effect start from beginning
    }
  }
...
void handleTransitions() {
  ...
  if (transitionActive && strip.getTransition() > 0) {
    ...
    if (ti / tr) {
      ...
      // restore (global) transition time if not called from UDP notifier or single/temporary transition from JSON (also
      // playlist)
      if (jsonTransitionOnce) {
  ...
  if (nightlightActive) {
    if (!nightlightActiveOld)  // init
    {
      ...
      for (unsigned i = 0; i < 4; i++) {
        colNlT[i] = colPri[i];  // remember starting color
      }
      if (nightlightMode == NL_MODE_SUN) {
        ...
        strip.getFirstSelectedSeg().setMode(
            FX_MODE_STATIC);              // make sure seg runtime is reset if it was in sunrise mode
    ...
    if (nightlightMode == NL_MODE_FADE || nightlightMode == NL_MODE_COLORFADE) {
      ...
      {
        for (unsigned i = 0; i < 4; i++) {
          colPri[i] = colNlT[i] + ((colSec[i] - colNlT[i]) * nper);  // fading from actual color to secondary color
        }
      }
      colorUpdated(CALL_MODE_NO_NOTIFY);
    }
