 ...
 * 3. There must not be any additional characters between a root-level key and its value object (e.g. space, tab,
 * newline)
 ...
 * 7. If it is desired to delete the first usable object (e.g. preset file), a dummy object '"0":{}' is inserted at the
 * beginning. It shall be disregarded by receiving software. The reason for it is that deleting the first preset would
 * require special code to handle commas between it and the 2nd preset
 */
...
void closeFile() {
  ...
  uint32_t s = millis();
#endif
  f.close();  // "if (f)" check is aleady done inside f.close(), and f cannot be nullptr -> no need for double checking
...
static bool appendObjectToFile(const char* key, const JsonDocument* content, uint32_t s, uint32_t contentLen = 0) {
  ...
  if (f.size() + 9000 >
      (fsBytesTotal - fsBytesUsed)) {  // make sure there is enough space to at least copy the file once
...
bool writeObjectToFile(const char* file, const char* key, const JsonDocument* content) {
  ...
  if (doCloseFile) {
    closeFile();  // This prevents the loss of file data that is still cached in the File object.
  }

  size_t pos = 0;
  char   fileName[129];
  strncpy(fileName, file, 128);
  fileName[128] = 0;  // use safe copy as FS.open() does not
  ...
  if (contentLen && contentLen <= oldLen) {  // replace and fill diff with spaces
    ...
    DEBUGFS_PRINTLN("replace (trailing)");
    f.seek(pos);
    serializeJson(*content, f);
  } else {
...
static const uint8_t* getPresetCache(size_t& size) {
  ...
  // if (presetsModifiedTime != presetsCachedTime) DEBUG_PRINTLN("getPresetCache(): presetsModifiedTime changed.");
  // if (presetsCachedValidate != cacheInvalidate) DEBUG_PRINTLN("getPresetCache(): cacheInvalidate changed.");
  ...
  if (!presetsCached) {
    File file = WLED_FS.open(getPresetsFileName(), "r");
    if (file) {
      presetsCachedTime     = presetsModifiedTime;
      presetsCachedValidate = cacheInvalidate;
      presetsCachedSize     = 0;
      presetsCached         = static_cast<uint8_t*>(p_malloc(file.size() + 1));
      if (presetsCached) {
...
bool handleFileRead(AsyncWebServerRequest* request, String path) {
  DEBUGFS_PRINT("WS FileRead: ");
  DEBUGFS_PRINTLN(path);
  if (path.endsWith("/")) {
    path += "index.htm";
  }
  ...
  if (path.endsWith(getPresetsFileName())) {
    size_t         psize;
    const uint8_t* presets = getPresetCache(psize);
    if (presets) {
      AsyncWebServerResponse* response = request->beginResponse_P(200, CONTENT_TYPE_JSON, presets, psize);
  ...
  if (WLED_FS.exists(path) || WLED_FS.exists(path + ".gz")) {
    request->send(request->beginResponse(WLED_FS, path, {}, request->hasArg("download"), {}));
    return true;
  }
  return false;
}
...
bool restoreFile(const char* filename) {
  ...
  if (!validateJsonFile(backupname)) {
    DEBUG_PRINTLN("broken backup");
    return false;
  }

  if (copyFile(backupname, filename)) {
    DEBUG_PRINTLN("restore ok");
    return true;
  }
  DEBUG_PRINTLN("restore failed");
  return false;
}
