...
static const std::pair<TimeChangeRule, TimeChangeRule> TZ_TABLE[] = {
 ...
 {Last, Sun, Oct, 3, 60}    // Central European Standard Time
    ...
    /* TZ_NORTH_KOREA */
    {                 {Last, Sun, Mar, 1,
                 510},                                        //                                                                       Pyongyang Time = UTC + 8.5 hours
                                        {Last, Sun, Mar, 1, 510}   },
    /* TZ_INDIA */
    {                 {Last, Sun, Mar, 1,
                 330},                                        //                                                                       India Standard Time = UTC + 5.5
                                        //                                                                       hours
                                        {Last, Sun, Mar, 1, 330}   },
    /* TZ_SASKACHEWAN */
    {                {First, Sun, Nov, 2,
    ...
    /* TZ_PAKISTAN */
    {                 {Last, Sun, Mar, 1,
                 300},                                        //                                                                       Pakistan Standard Time = UTC + 5
                                        //                                                                       hours
                                        {Last, Sun, Mar, 1, 300}   },
    /* TZ_BRASILIA */
    {                {Last, Sun, Mar, 1,
                -180},                                       //                                                                      Brasília Standard Time = UTC - 3
                                       //                                                                      hours
                                       {Last, Sun, Mar, 1, -180}   },
  ...
  if (ntpEnabled && ntpConnected && millis() - ntpLastSyncTime > (1000 * NTP_SYNC_INTERVAL) && WLED_CONNECTED) {
    if (millis() - ntpPacketSentTime > 10000) {
      ...
      while (ntpUdp.parsePacket() > 0) {
        ntpUdp.flush();  // flush any existing packets
      }
#endif
        ...
        switch (res) {
          ...
          case AsyncDNS::result::Error:
            ...
            if (ntpDNSlookup->getErrorCount() > 6) {
              // after 6 failed attempts (30min), reset network connection as dns is probably stuck (TODO: IDF bug,
              // should be fixed in V5)
              if (offMode) {
                forceReconnect = true;  // do not disturb while LEDs are running
              }
              ntpDNSlookup.reset();
            } else {
              // Retry
...
#ifdef ARDUINO_ARCH_ESP32  // I had problems using udp.flush() on 8266
    if (cb > 0) {
      ntpUdp.flush();  // this avoids memory leaks on esp32
    }
#endif
    return false;
  }
...
void getTimeString(char* out) {
  ...
  sprintf(out, "%i-%i-%i, %02d:%02d:%02d", year(localTime), month(localTime), day(localTime), hr, minute(localTime),
          second(localTime));
  if (useAMPM) {
    strcat(out, " ");
...
void setCountdown() {
  ...
  countdownTime =
      tz->toUTC(getUnixTime(countdownHour, countdownMin, countdownSec, countdownDay, countdownMonth, countdownYear));
  if (countdownTime - toki.second() > 0) {
    countdownOverTriggered = false;
  }
}
...
bool checkCountdown() {
  ...
  if (countdownMode) {
    localTime = countdownTime - n + utcOffsetSecs;
  }
  if (n > countdownTime) {
    if (countdownMode) {
      localTime = n - countdownTime + utcOffsetSecs;
    }
    if (!countdownOverTriggered) {
      if (macroCountdown != 0) {
        applyPreset(macroCountdown);
      }
...
bool isTodayInDateRange(byte monthStart, byte dayStart, byte monthEnd, byte dayEnd) {
  ...
  if (dayEnd < dayStart) {
    return (m != monthStart || (d <= dayEnd || d >= dayStart));  // all year, except the designated days in this month
  }
...
void checkTimers() {
  if (lastTimerMinute != minute(localTime)) {
    ...
    for (size_t i = 0; i < timers.size(); i++) {
      ...
      if ((hour(tt) == hour(localTime) && minute(tt) == minute(localTime)) ||
          (t.hour == 24 && t.minute == minute(localTime))) {
        if (!((t.weekdays >> weekdayMondayFirst()) & 0x01)) {
          continue;
        }
...
#ifdef WLED_DEBUG
        if (t.isSunrise()) {
          DEBUG_PRINTF_P("Sunrise timer %d offset %d\n", t.preset, t.minute);
        } else if (t.isSunset()) {
          DEBUG_PRINTF_P("Sunset timer %d offset %d\n", t.preset, t.minute);
        } else {
          DEBUG_PRINTF_P("Timer %d: preset %d\n", i, t.preset);
        }
#endif
      }
    }
...
void calculateSunriseAndSunset() {
  if (static_cast<int>(longitude * 10.) || static_cast<int>(latitude * 10.)) {
    ...
    // So in case of invalid result, we try to use the sunset/sunrise of previous day. Max 3 days back, this worked well
    // in all cases I tried. When latitude = 66,6 (N or S), the functions sometimes returns 2147483647, so this
    // "unexpected large" is another condition for retry
    int minUTC     = 0;
    int retryCount = 0;
    do {
...
void setTimeFromAPI(uint32_t timein) {
  ...
  if (toki.getTimeSource() > TOKI_TS_JSON && diff < 60U) {
    return;
  }
...
void addTimer(uint8_t preset, uint8_t hour, int8_t minute, uint8_t weekdays, uint8_t monthStart, uint8_t monthEnd,
  ...
  if ((monthStart != 0 && monthStart > 12) || (monthEnd != 0 && monthEnd > 12)) {
