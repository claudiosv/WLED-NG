...
static GifDecoder<320, 320, 12, true> decoder;  // this creates the basic object; parameter lzwMaxBits is not used;
                                                // decoder.alloc() always allocated "everything else" = 24Kb
...
void updateScreenCallback(void) {
  ...
  if (activeSeg->intensity > 1) {
    ...
    if ((blurAmount < 24) && (activeSeg->is2D())) {
      activeSeg->blurRows(activeSeg->intensity);  // some blur - fast
    } else {
      activeSeg->blur(blurAmount);  // more blur - slower
    }
  }
...
void drawPixelCallback2D(int16_t x, int16_t y, uint8_t red, uint8_t green, uint8_t blue) {
  ...
  lastCoordinate =
      (outY << 16) | outX;  // since input is a "scanline" this is sufficient to identify a "unique" coordinate
...
byte renderImageToSegment(Segment &seg) {
  if (!seg.name) {
    return IMAGE_ERROR_NO_NAME;
  }
  // disable during effect transition, causes flickering, multiple allocations and depending on image, part of old FX
  // remaining
  // if (seg.mode != seg.currentMode()) return IMAGE_ERROR_WAITING;
  if (activeSeg && activeSeg != &seg) {  // only one segment at a time
    if (!seg.isActive()) {
      return IMAGE_ERROR_SEG_LIMIT;  // sanity check: calling segment must be active
    }
  ...
  if (strncmp(lastFilename + 1, seg.name, WLED_MAX_SEGNAME_LEN) != 0) {  // segment name changed, load new image
    ...
    if ((fnameLen < 4) || strcmp(lastFilename + fnameLen - 4, ".gif") !=
                              0) {  // empty segment name, name too short, or name not ending in .gif
    ...
    try {
      ...
      // decoder cleanup (hi @coderabbitai): No additonal cleanup necessary - decoder.alloc() ultimately uses "new
      // AnimatedGIF". If new throws, no pointer is assigned, previous decoder state (if any) has already been deleted
      // inside alloc(), so calling decoder.dealloc() here is unnecessary.
    }
#endif
    ...
    DEBUG_PRINTLN("Decoding started");
  ...
  unsigned long tooSlowBy =
      (millis() - lastFrameDisplayTime) - wait;  // if last frame was longer than intended, compensate
...
void endImagePlayback(Segment *seg) {
  DEBUG_PRINTLN("Image playback end called");
  if (!activeSeg || activeSeg != seg) {
    return;
  }
  if (file) {
    file.close();
  ...
  DEBUG_PRINTLN("Image playback ended");
}
