...
#define DECLARE_DYNARRAY(type, array_name)                                              \
  static type const DYNARRAY_BEGIN(array_name)[0]                                       \
      __attribute__((__section__(DYNARRAY_SECTION "." #array_name ".0"), unused)) = {}; \
  static type const DYNARRAY_END(array_name)[0]                                         \
      __attribute__((__section__(DYNARRAY_SECTION "." #array_name ".99999"), unused)) = {};

// Declare an object that is a member of a dynamic array.  "member name" must be unique; "array_section" is an integer
// for ordering items. It is legal to define multiple items with the same section name; the order of those items will be
// up to the linker.
