...
// convert HSV (16bit hue) to RGB (24bit), optimized for speed (integer types and function arguments were very carefully
// selected for best performance) this does the same as the FastLED hsv2rgb_rainbow function but with 16bit hue and
// optimizations for use with CRGB as well as CRGBW note: this function is used when converting CHSV->CRGB or
// CHSV32->CRGBW by assignment or constructor, there is no need to call it explicitly
__attribute__((optimize("O2"))) void hsv2rgb_rainbow(uint16_t h, uint8_t s, uint8_t v, uint8_t* rgbdata, bool isRGBW) {
  ...
  if (sat != 255) {
    if (sat == 0) {
      ...
      desat = (desat * desat);  // scale8_video(desat, desat) but more accurate, dropped the "+1" for speed: visual
...
void nblendPaletteTowardPalette(CRGBPalette16& current, CRGBPalette16& target, uint8_t maxChanges) {
  ...
  for (uint32_t i = 0; i < totalChannels; ++i) {
    if (p1[i] == p2[i]) {
      continue;  // if the values are equal, no changes are needed
    }
    if (p1[i] < p2[i]) {
      ++p1[i];
      ++changes;
    ...
    if (p1[i] >
        p2[i]) {  // if the current value is greater than the target, increase it by one (or two if it's still greater).
      --p1[i];
      ++changes;
      if (p1[i] > p2[i]) {
        --p1[i];
