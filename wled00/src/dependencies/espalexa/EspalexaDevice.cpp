...
EspalexaDevice::EspalexaDevice(String deviceName, BrightnessCallbackFunction gnCallback,
                               uint8_t initialValue) {  // constructor for dimmable device
...
EspalexaDevice::EspalexaDevice(String deviceName, ColorCallbackFunction gnCallback,
                               uint8_t initialValue) {  // constructor for color device
...
EspalexaDevice::EspalexaDevice(String deviceName, DeviceCallbackFunction gnCallback, EspalexaDeviceType t,
                               uint8_t initialValue) {  // constructor for general device

  _deviceName  = deviceName;
  _callbackDev = gnCallback;
  _type        = t;
  if (t == EspalexaDeviceType::onoff) {
    _type = EspalexaDeviceType::dimmable;  // on/off is broken, so make dimmable device instead
  }
  if (t == EspalexaDeviceType::whitespectrum) {
...
uint32_t EspalexaDevice::getRGB() {
  if (_rgb != 0) {
    return _rgb;  // color has not changed
  }
  byte rgb[4]{0, 0, 0, 0};

  if (_mode == EspalexaColorMode::none) {
    return 0;
  }
  ...
  if (_mode == EspalexaColorMode::ct) {
    ...
    float temp = (_ct != 0) ? (10000 / _ct) : 2;  // kelvins = 1,000,000/mired (and that /100)   softhack007: avoid
                                                  // division by zero - using "2" as substitute
    float r, g, b;

#ifdef ESPALEXA_DEBUG
    if (_ct == 0) {
      EA_DEBUGLN("EspalexaDevice::getRGB() Warning: ct = 0!");
    }
#endif
    if (temp <= 66) {
      r = 255;
...
void EspalexaDevice::setColor(uint8_t r, uint8_t g, uint8_t b) {
  ...
  if ((r + g + b) > 0) {  //   softhack007: avoid division by zero
    ...
    _x = _y = 0.5f;
  }  // softhack007: use default values in case of "black"
  _rgb  = ((r << 16) | (g << 8) | b);
  _mode = EspalexaColorMode::xy;
}
...
void EspalexaDevice::doCallback() {
  ...
  if (_callbackDev != nullptr) {
    _callbackDev(this);
    return;
  }
  if (_callbackCol != nullptr) {
    _callbackCol(_val, getRGB());
  }
}