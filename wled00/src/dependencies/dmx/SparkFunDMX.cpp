...
static const int rxPin =
    -1;  // disable the receiving pin because it is not needed - softhack007: Pin=-1 means "use default" not "disable"
...
#if !defined(DMX_SEND_ONLY)
  else if (_READWRITE == _READ)  // In a perfect world, this function ends serial communication upon packet completion
                                 // and attaches RX to a CHANGE interrupt so the start code can be read again
  {
