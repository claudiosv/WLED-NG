...
void notify(byte callMode, bool followUp) {
...
#endif
  if (!syncGroups || !sendNotificationsRT) {
    return;
  }
  switch (callMode) {
    case CALL_MODE_INIT:
      return;
    case CALL_MODE_DIRECT_CHANGE:
      if (!notifyDirect) {
        return;
      }
      break;
    case CALL_MODE_BUTTON:
      if (!notifyButton) {
        return;
      }
      break;
    case CALL_MODE_BUTTON_PRESET:
      if (!notifyButton) {
        return;
      }
      break;
    case CALL_MODE_NIGHTLIGHT:
      if (!notifyDirect) {
        return;
      }
      break;
    case CALL_MODE_HUE:
      ...
      break;
    case CALL_MODE_PRESET_CYCLE:
      if (!notifyDirect) {
        return;
      }
      break;
    case CALL_MODE_ALEXA:
      if (!notifyAlexa) {
        return;
      }
      break;
  ...
  // 9: supports sync groups, 37 byte packet 10: supports CCT, 39 byte packet 11: per segment options, variable packet
  // length (40+WS2812FX::getMaxSegments()*3) 12: enhanced effect sliders, 2D & mapping options
  udpOut[11] = 12;
  ...
  for (size_t i = 0; i < nsegs; i++) {
    ...
    udpOut[9 + ofs] = selseg.options & 0x8F;  // only take into account selected, mirrored, on, reversed, reverse_y (for
                                              // 2D); ignore freeze, reset, transitional
  ...
  if (enableESPNow && useESPNowSync && statusESPNow == ESP_NOW_STATE_ON) {
    ...
    if (!err && s0 < s) {
      ...
      buffer.packet++;
      packetSize = 0;
      // WARNING: this will only work for up to 3 messages (~17 segments) as QuickESPNOW only has a ring buffer capable
...
static void parseNotifyPacket(const uint8_t* udpIn) {
  ...
  if (millis() - notificationSentTime < 1000) {
    return;
  }
  if (udpIn[1] > 199) {
    return;  // do not receive custom versions
  }
  ...
  if (version < 9) {
    ...
    if (!(receiveGroups & 0x01)) {
      return;
    }
  } else if (!(receiveGroups & udpIn[36])) {
    return;
  }

  bool someSel = (receiveNotificationBrightness || receiveNotificationColor || receiveNotificationEffects ||
                  receiveNotificationPalette);
  ...
  if (applyEffects && currentPlaylist >= 0) {
    unloadPlaylist();
  }
  if (version > 10 && (receiveSegmentOptions || receiveSegmentBounds)) {
    ...
    if (receiveSegmentBounds && numSrcSegs < strip.getActiveSegmentsNum()) {
      ...
      strip.suspend();  // should not be needed as UDP handling is not done in ISR callbacks but still added "just in
                        // case"
      for (size_t i = strip.getSegmentsNum(); i > numSrcSegs && i > 0; i--) {
        Segment& seg = strip.getSegment(i - 1);
        if (seg.isActive()) {
          seg.deactivate();  // delete segment
        }
      }
      strip.resume();
    }
    ...
    for (size_t i = 0; i < numSrcSegs && i < WS2812FX::getMaxSegments(); i++) {
      ...
      if (selseg.isActive() && !(selseg.isSelected() || receiveSegmentBounds)) {
        continue;
      }
      ...
      selseg.options =
          (selseg.options & 0x0071U) | (udpIn[9 + ofs] & 0x0E);  // ignore selected, freeze, reset & transitional
      ...
      if (receiveSegmentBounds) {
        ...
        strip.suspend();  // should not be needed as UDP handling is not done in ISR callbacks but still added "just in
                          // case"
        selseg.setGeometry(selseg.start, selseg.stop, udpIn[5 + ofs], udpIn[6 + ofs], selseg.offset, selseg.startY,
                           selseg.stopY, selseg.map1D2D);
        strip.resume();
      }
    }
    stateChanged = true;
  ...
  if ((applyEffects || receiveNotificationPalette) && (version < 11 || !receiveSegmentOptions)) {
    for (size_t i = 0; i < strip.getSegmentsNum(); i++) {
      ...
      if (applyEffects) {
        ...
        seg.speed = udpIn[9];
        if (version > 2) {
          seg.intensity = udpIn[16];
        }
      }
      if (version > 4 && receiveNotificationPalette) {
        seg.setPalette(udpIn[19]);
      }
    }
    stateChanged = true;
  }
  ...
  if (version > 7) {
    ...
    if (udpIn[29] > toki.getTimeSource()) {     // if sender's time source is more accurate
      ...
      if (udpIn[29] > 99) {
        ts = TOKI_TS_UDP_NTP;
      } else if (udpIn[29] >= TOKI_TS_SEC) {
        ts = TOKI_TS_UDP_SEC;
      }
      toki.setTime(tm, ts);
    } else if (timebaseUpdated &&
               toki.getTimeSource() > 99) {  // if we both have good times, get a more accurate timebase
      Toki::Time myTime = toki.getTime();
      uint32_t   diff   = toki.msDifference(tm, myTime);
      strip.timebase -=
          PRESUMED_NETWORK_DELAY;  // no need to presume, use difference between NTP times at send and receive points
...
void realtimeLock(uint32_t timeoutMs, byte md) {
  ...
  if (briT > 0 && md == REALTIME_MODE_GENERIC) {
    strip.show();
  }
}
...
void handleNotifications() {
  ...
  if (!packetSize || packetSize > UDP_IN_MAXSIZE) {
    return;
  }
  if (!isSupp && notifierUdp.remoteIP() == localIP) {
    return;  // don't process broadcasts we send ourselves
  }
  ...
  if (isSupp && udpIn[0] == 255 && udpIn[1] == 1 && len >= 40) {
    if (!nodeListEnabled || notifier2Udp.remoteIP() == localIP) {
      return;
    }
  ...
  if (udpIn[0] == 0 && !realtimeMode && receiveGroups) {
    DEBUG_PRINTF_P("UDP notification from: %d.%d.%d.%d\n", notifierUdp.remoteIP()[0], notifierUdp.remoteIP()[1],
                   notifierUdp.remoteIP()[2], notifierUdp.remoteIP()[3]);
  ...
  if (receiveDirect) {
    // TPM2.NET
    if (udpIn[0] == 0x9c) {
      // WARNING: this code assumes that the final TMP2.NET payload is evenly distributed if using multiple packets (ie.
      // frame size is constant) if the number of LEDs in your installation doesn't allow that, please include padding
      // bytes at the end of the last packet
      ...
      if (tpmPacketCount == 1) {
        tpmPayloadFrameSize =
            (udpIn[2] << 8) + udpIn[3];  // save frame size for the whole payload if this is the first packet
      }
...
#endif
  if (bri) {
    data[38] |= 0x80U;  // add on/off state
  }
  ...
  switch (type) {
    case 0:  // DDP
    {
      ...
      for (size_t currentPacket = 0; currentPacket < packetCount; currentPacket++) {
        ...
        /*1*/ ddpUdp.write(
            sequenceNumber++ &
            0x0F);  // sequence may be unnecessary unless we are sending twice (as requested in Sync settings)
        ...
        for (size_t i = 0; i < packetSize; i += (isRGBW ? 4 : 3)) {
          ...
          if (isRGBW) {
            ddpUdp.write(scale8(buffer[bufferOffset++], bri));  // W
          }
        }
...
void espNowReceiveCB(uint8_t* address, uint8_t* data, uint8_t len, signed int rssi, bool broadcast) {
  ...
  DEBUG_PRINTLN(len);
  for (int i = 0; i < len; i++) {
    DEBUG_PRINTF_P("%02x ", data[i]);
  }
  DEBUG_PRINTLN();
#endif

  // usermods hook can override processing
  if (UsermodManager::onEspNowMessage(address, data, len)) {
    return;
  }
  ...
  if (buffer->packet == 0) {
    ...
    if (udpIn == nullptr) {
      ...
      if (!udpIn) {
        return;  // memory alocation failed
      }
  ...
  packetsReceived++;
  DEBUG_PRINTF_P("ESP-NOW packet received: %d (%d/%d s:[%d/%d]\n", (int)buffer->packet, (int)packetsReceived,
                 (int)buffer->noOfPackets, (int)segsReceived, MAX_NUM_SEGMENTS);
