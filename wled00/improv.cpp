        ...
        if (packetByte >= packetLen) {  // end of packet, check checksum match
          ...
          switch (rpcCommandType) {
            ...
            case ImprovRPCType::Request_State: {
              ...
              if (improvState == 0x04) {
                sendImprovIPRPCResult(ImprovRPCType::Request_State);
              }
              break;
            }
...
void sendImprovRPCResult(ImprovRPCType type, uint8_t n_strings, const char** strings) {
  ...
  for (unsigned s = 0; s < n_strings; s++) {
    ...
    if (pos + len > 254) {
      continue;  // simple buffer overflow guard
    }
    out[pos++] = len;
  ...
  unsigned checksum = 0;
  for (unsigned i = 0; i < packetLen - 1; i++) {
    checksum += out[i];
  }
...
void sendImprovInfoResponse() {
  ...
  char vString[32];
  sprintf(vString, "%.19s/%i", versionString, VERSION);
