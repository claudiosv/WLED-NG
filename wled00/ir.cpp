...
const uint8_t brightnessSteps[]  = {5, 7, 9, 12, 16, 20, 26, 34, 43, 56, 72, 93, 119, 154, 198, 255};
...
static void changeColor(uint32_t c, int16_t cct = -1) {
  if (irApplyToAllSelected) {
    ...
    for (unsigned i = 0; i < strip.getSegmentsNum(); i++) {
      ...
      if (hasW) {
        mask |= 0xFF000000;  // white
      }
      if (hasW && !wSlider &&
          (c & 0xFF000000)) {  // segment has white channel & white channel is auto calculated & white specified
        seg.setColor(0, c | 0xFFFFFF);  // for accurate/brighter mode we fake white (since button may not set white
                                        // color to 0xFFFFFF)
      } else if (c & mask) {
        seg.setColor(0, c & mask);  // only apply if not black
      }
      if (isCCT && cct >= 0) {
        seg.setCCT(cct);
      }
    ...
    if (hasW && !wSlider &&
      ...
      seg.setColor(
          0,
          c | 0xFFFFFF);  // for accurate/brighter mode we fake white (since button may not set white color to 0xFFFFFF)
    } else if (c & mask) {
      seg.setColor(0, c & mask);  // only apply if not black
    }
    if (isCCT && cct >= 0) {
...
static void decodeIR24(uint32_t code) {
  switch (code) {
    ...
    case IR24_WHITE:
      ...
      break;
    case IR24_FLASH:
      presetFallback(1, FX_MODE_COLORTWINKLE, effectPalette);
      break;
    case IR24_STROBE:
      presetFallback(2, FX_MODE_RAINBOW_CYCLE, effectPalette);
      break;
    case IR24_FADE:
      presetFallback(3, FX_MODE_BREATH, effectPalette);
      break;
    case IR24_SMOOTH:
      presetFallback(4, FX_MODE_RAINBOW, effectPalette);
      break;
    default:
      return;
  }
...
static void decodeIR24OLD(uint32_t code) {
  switch (code) {
    case IR24_OLD_BRIGHTER:
      incBrightness();
      break;
    case IR24_OLD_DARKER:
      decBrightness();
      break;
    case IR24_OLD_OFF:
      if (bri > 0) {
    ...
    case IR24_OLD_RED:
      changeColor(COLOR_RED);
      break;
    case IR24_OLD_REDDISH:
      changeColor(COLOR_REDDISH);
      break;
    case IR24_OLD_ORANGE:
      changeColor(COLOR_ORANGE);
      break;
    case IR24_OLD_YELLOWISH:
      changeColor(COLOR_YELLOWISH);
      break;
    case IR24_OLD_YELLOW:
      changeColor(COLOR_YELLOW);
      break;
    case IR24_OLD_GREEN:
      changeColor(COLOR_GREEN);
      break;
    case IR24_OLD_GREENISH:
      changeColor(COLOR_GREENISH);
      break;
    case IR24_OLD_TURQUOISE:
      changeColor(COLOR_TURQUOISE);
      break;
    case IR24_OLD_CYAN:
      changeColor(COLOR_CYAN);
    ...
    case IR24_OLD_BLUE:
      changeColor(COLOR_BLUE);
      break;
    case IR24_OLD_DEEPBLUE:
      changeColor(COLOR_DEEPBLUE);
      break;
    case IR24_OLD_PURPLE:
      changeColor(COLOR_PURPLE);
      break;
    case IR24_OLD_MAGENTA:
      changeColor(COLOR_MAGENTA);
      break;
    case IR24_OLD_PINK:
      changeColor(COLOR_PINK);
    ...
    case IR24_OLD_WHITE:
      ...
      break;
    case IR24_OLD_FLASH:
      presetFallback(1, FX_MODE_COLORTWINKLE, 0);
      break;
    case IR24_OLD_STROBE:
      presetFallback(2, FX_MODE_RAINBOW_CYCLE, 0);
      break;
    case IR24_OLD_FADE:
      presetFallback(3, FX_MODE_BREATH, 0);
      break;
    case IR24_OLD_SMOOTH:
      presetFallback(4, FX_MODE_RAINBOW, 0);
      break;
    default:
      return;
  }
...
static void decodeIR24CT(uint32_t code) {
  switch (code) {
    ...
    case IR24_CT_RED:
      changeColor(COLOR_RED);
      break;
    case IR24_CT_REDDISH:
      changeColor(COLOR_REDDISH);
      break;
    case IR24_CT_ORANGE:
      changeColor(COLOR_ORANGE);
      break;
    case IR24_CT_YELLOWISH:
      changeColor(COLOR_YELLOWISH);
      break;
    case IR24_CT_YELLOW:
      changeColor(COLOR_YELLOW);
    ...
    case IR24_CT_GREEN:
      changeColor(COLOR_GREEN);
      break;
    case IR24_CT_GREENISH:
      changeColor(COLOR_GREENISH);
      break;
    case IR24_CT_TURQUOISE:
      changeColor(COLOR_TURQUOISE);
      break;
    case IR24_CT_CYAN:
      changeColor(COLOR_CYAN);
    ...
    case IR24_CT_BLUE:
      changeColor(COLOR_BLUE);
      break;
    case IR24_CT_DEEPBLUE:
      changeColor(COLOR_DEEPBLUE);
      break;
    case IR24_CT_PURPLE:
      changeColor(COLOR_PURPLE);
      break;
    case IR24_CT_MAGENTA:
      changeColor(COLOR_MAGENTA);
      break;
    case IR24_CT_PINK:
      changeColor(COLOR_PINK);
      break;
    case IR24_CT_COLDWHITE:
      changeColor(COLOR_COLDWHITE2, 255);
      changeEffect(FX_MODE_STATIC);
      break;
    case IR24_CT_WARMWHITE:
      changeColor(COLOR_WARMWHITE2, 0);
      changeEffect(FX_MODE_STATIC);
      break;
    case IR24_CT_CTPLUS:
      changeColor(COLOR_COLDWHITE, strip.getSegment(strip.getMainSegmentId()).cct + 1);
      changeEffect(FX_MODE_STATIC);
      break;
    case IR24_CT_CTMINUS:
      changeColor(COLOR_WARMWHITE, strip.getSegment(strip.getMainSegmentId()).cct - 1);
      changeEffect(FX_MODE_STATIC);
      break;
    case IR24_CT_MEMORY:
      changeColor(COLOR_NEUTRALWHITE, 127);
      changeEffect(FX_MODE_STATIC);
      break;
    default:
      return;
...
static void decodeIR40(uint32_t code) {
  ...
  switch (code) {
    ...
    case IR40_WARMWHITE2:
      ...
      break;
    case IR40_WARMWHITE:
      changeColor(COLOR_WARMWHITE, 63);
      changeEffect(FX_MODE_STATIC);
      break;
    case IR40_WHITE:
      changeColor(COLOR_NEUTRALWHITE, 127);
      ...
      break;
    case IR40_COLDWHITE:
      changeColor(COLOR_COLDWHITE, 191);
      changeEffect(FX_MODE_STATIC);
      break;
    ...
    case IR40_WOFF:
      ...
      changeColor(RGBW32(r, g, b, 0));
      break;
    case IR40_WON:
      changeColor(RGBW32(r, g, b, whiteLast));
      break;
    case IR40_W25:
      bri = 63;
      break;
    ...
    case IR40_AUTO:
      ...
      break;
    case IR40_JUMP3:
      presetFallback(1, FX_MODE_STATIC, 0);
      break;
    case IR40_FADE3:
      presetFallback(2, FX_MODE_BREATH, 0);
      break;
    case IR40_FADE7:
...
static void decodeIR44(uint32_t code) {
  switch (code) {
    ...
    case IR44_REDMINUS:
      ...
      break;
    case IR44_GREENPLUS:
      changePalette(relativeChange(effectPalette, 1, 0, getPaletteCount() - 1));
      break;
    ...
    case IR44_DIY4:
      ...
      break;
    case IR44_DIY5:
      presetFallback(5, FX_MODE_METEOR, 0);
      break;
    case IR44_DIY6:
      presetFallback(6, FX_MODE_RAIN, 0);
      break;
    case IR44_AUTO:
      changeEffect(FX_MODE_STATIC);
      break;
    case IR44_FLASH:
      changeEffect(FX_MODE_PALETTE);
      break;
    case IR44_JUMP3:
      bri = 63;
      break;
...
static void decodeIRJson(uint32_t code) {
  ...
  if (!requestJSONBufferLock(JSON_LOCK_IR)) {
    return;
  }
  ...
  if (jsonCmdObj.isNull())  // we could also use: fdo["cmd"].is<String>()
  {
    if (cmdStr.startsWith("!")) {
      ...
      if (cmdStr.indexOf("~") > 0 || fdo["rpt"]) {
        lastValidCode = code;  // repeatable action
      }
    ...
    if (jsonCmdObj["psave"].isNull()) {
      if (irApplyToAllSelected && jsonCmdObj["seg"].is<JsonArray>()) {
        JsonObject seg =
            jsonCmdObj["seg"][0];  // take 1st segment from array and use it to apply to all selected segments
  ...
  if (irEnabled == 8) {
    ...
    switch (lastRepeatableAction) {
      case ACTION_BRIGHT_UP:
        ...
        return;
      case ACTION_BRIGHT_DOWN:
        decBrightness();
        stateUpdated(CALL_MODE_BUTTON);
        return;
      case ACTION_SPEED_UP:
        changeEffectSpeed(lastRepeatableValue);
        stateUpdated(CALL_MODE_BUTTON);
        return;
      case ACTION_SPEED_DOWN:
        ...
        stateUpdated(CALL_MODE_BUTTON);
        return;
      case ACTION_INTENSITY_UP:
        changeEffectIntensity(lastRepeatableValue);
        stateUpdated(CALL_MODE_BUTTON);
        return;
      case ACTION_INTENSITY_DOWN:
        changeEffectIntensity(lastRepeatableValue);
        stateUpdated(CALL_MODE_BUTTON);
        return;
      default:
...
static void decodeIR(uint32_t code) {
  ...
  switch (irEnabled) {
    ...
    case 2:
      decodeIR24CT(code);
      break;  // white 24-key remote with CW, WW, CT+ and CT- keys
    case 3:
      decodeIR40(code);
      break;  // blue  40-key remote with 25%, 50%, 75% and 100% keys
    case 4:
      decodeIR44(code);
      break;  // white 44-key remote with color-up/down keys and DIY1 to 6 keys
    case 5:
      decodeIR21(code);
      break;  // white 21-key remote
    case 6:
      decodeIR6(code);
      break;  // black 6-key learning remote defaults: "CH" controls brightness,
...
void handleIR() {
  ...
  if (timeDiff > 120 && irEnabled > 0 && irrecv) {
    if (strip.isUpdating() && timeDiff < 240) {
