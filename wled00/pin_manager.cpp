...
static bool PinManager::deallocatePin(byte gpio, PinOwner tag) {
  ...
  if (!isPinOk(gpio, false)) {
    return false;  // but return false for any other invalid pin
  }
  ...
  if ((ownerTag[gpio] != PinOwner::None) && (ownerTag[gpio] != tag)) {
    DEBUG_PRINTF_P("PIN DEALLOC: FAIL GPIO %d allocated by 0x%02X, but attempted de-allocation by 0x%02X.\n", gpio,
                   static_cast<int>(ownerTag[gpio]), static_cast<int>(tag));
    return false;
  }
  ...
  if (!isPinOk(gpio, output) || (gpio >= WLED_NUM_PINS) || tag == PinOwner::HW_I2C || tag == PinOwner::HW_SPI ||
    ...
    if (gpio < 255) {  // 255 (-1) is the "not defined GPIO"
      if (!isPinOk(gpio, output)) {
        ...
        if (output) {
          DEBUG_PRINTLN(" cannot be used for i/o on this MCU.");
        } else {
          DEBUG_PRINTLN(" cannot be used as input on this MCU.");
        }
      } else {
 ...
 * general-purpose I/O Strapping pins: GPIO0, GPIO3, GPIO45 and GPIO46 are strapping pins. For more infomation, please
 * refer to ESP32-S3 datasheet. Serial TX = GPIO43, RX = GPIO44; LED BUILTIN is usually GPIO39 USB-JTAG: GPIO 19 and 20
 * are used by USB-JTAG by default. In order to use them as GPIOs, USB-JTAG will be disabled by the drivers. SPI0/1:
 * GPIO26-32 are usually used for SPI flash and PSRAM and not recommended for other uses. When using Octal Flash or
 * Octal PSRAM or both, GPIO33~37 are connected to SPIIO4 ~ SPIIO7 and SPIDQS. Therefore, on boards embedded with
 * ESP32-S3R8 / ESP32-S3R8V chip, GPIO33~37 are also not recommended for other uses.
 *
 ...
 * adc_power_acquire(): Please do not use the interrupt of GPIO36 and GPIO39 when using ADC or Wi-Fi and Bluetooth with
 * sleep mode enabled. As a workaround, call adc_power_acquire() in the APP. Since the ADC2 module is also used by the
 * Wi-Fi, reading operation of adc2_get_raw() may fail between esp_wifi_start() and esp_wifi_stop(). Use the return code
...
static bool PinManager::isPinOk(byte gpio, bool output) {
  ...
  if (digitalPinIsValid(gpio)) {
    ...
    if (gpio > 17 && gpio < 20) {
      return false;  // 18-19 USB-JTAG
    }
#endif
#elif defined(CONFIG_IDF_TARGET_ESP32S3)
// 00 to 18 are for general use. Be careful about straping pins GPIO0 and GPIO3 - these may be pulled-up or pulled-down
// on your board.
#if ARDUINO_USB_CDC_ON_BOOT == 1 || ARDUINO_USB_DFU_ON_BOOT == 1
    if (gpio > 18 && gpio < 21) {
      return false;  // 19 + 20 = USB-JTAG. Not recommended for other uses.
    }
#endif
    if (gpio > 21 && gpio < 33) {
      return false;  // 22 to 32: not connected + SPI FLASH
    }
...
#endif
#if CONFIG_SPIRAM_MODE_OCT  // 33-37: not available if using _octal_ PSRAM (qio_opi), but free to use on _quad_ PSRAM
                            // (qio_qspi)
    if (gpio > 32 && gpio < 38) {
      return !psramFound();
    }
#endif
    // 38 to 48 are for general use. Be careful about straping pins GPIO45 and GPIO46 - these may be pull-up or
    // pulled-down on your board.
    ...
    if ((strncmp("ESP32-U4WDH", ESP.getChipModel(), 11) == 0) ||  // this is the correct identifier, but....
        (strncmp("ESP32-PICO-D", ESP.getChipModel(), 12) ==
         0)) {  // https://github.com/espressif/arduino-esp32/issues/10683
      // this chip has 4 MB of internal Flash and different packaging, so available pins are different!
      if ((gpio > 5 && gpio < 9) || gpio == 11) {
        return false;  // U4WDH/PICO-D2 & PICO-D4: GPIO 6, 7, 8, 11 are used for SPI flash; 9 & 10 are free
      }
      if (gpio == 16 || gpio == 17) {
        return false;  // U4WDH/PICO-D?: GPIO 16 and 17 are used for PSRAM
      }
    } else if (strncmp("ESP32-PICO-V3", ESP.getChipModel(), 13) == 0) {
      if (gpio == 6 || gpio == 11) {
        return false;  // PICO-V3: uses GPIO 6 and 11 for flash
      }
      if (strstr_P(ESP.getChipModel(), "V3-02") != nullptr && (gpio == 9 || gpio == 10)) {
        return false;  // PICO-V3-02: uses GPIO 9 and 10 for PSRAM; 7, 8 are free
      }
    } else {
      // for classic ESP32 (non-mini) modules, these are the SPI flash pins
      if (gpio > 5 && gpio < 12) {
        return false;  // SPI flash pins
      }
    }
    if (gpio == 16) {
      return !psramFound();  // PSRAM pins on modules with off-package or in-package PSRAM
    }
    if (gpio == 17) {
  ...
  for (unsigned i = 0; i < WLED_MAX_ANALOG_CHANNELS; i++) {
    if (bitRead(ledcAlloc, i)) {  // found occupied pin
      ...
      if (channels == 2) {  // will skip odd channel for first channel for phase shifting
        if (ca == 0 && i % 2 == 0) {
          ca++;  // even LEDC channels is 1st PWM channel
        }
        if (ca == 1 && i % 2 == 1) {
          ca++;  // odd LEDC channel is 2nd PWM channel
        }
      } else {
        ca++;
      }
...
static const char* PinManager::getPinOwnerName(uint8_t gpio) {
  ...
  switch (owner) {
    case PinOwner::None:
      return PinManager::isPinAllocated(gpio) ? "System" : "Unknown";
    case PinOwner::Ethernet:
      return "Ethernet";
    case PinOwner::BusDigital:
      return "LED Digital";
    case PinOwner::BusOnOff:
      return "LED On/Off";
    case PinOwner::BusPwm:
      return "LED PWM";
    ...
    case PinOwner::Relay:
      return "Relay";
    case PinOwner::SPI_RAM:
      return "SPI RAM";
    case PinOwner::DebugOut:
      return "Debug";
    case PinOwner::DMX:
      return "DMX Output";
    case PinOwner::HW_I2C:
      return "I2C";
    case PinOwner::HW_SPI:
      return "SPI";
    case PinOwner::DMX_INPUT:
      return "DMX Input";
    case PinOwner::HUB75:
      return "HUB75";
...
#endif
#endif
  return false;  // not an analog pin if it doesn't have ADC capability, ESP8266 has only one ADC pin (A0) which is
                 // handled separately in button.cpp, so return false for all pins here
}
